...
describe('App', () => {
  ...
  it('blocks the main screen when no usable provider and model are configured, while keeping Settings and Spaces accessible', async () => {
    ...
    expect(await screen.findByText('Hermes runtime configuration required')).toBeInTheDocument();
  ...
  it('saves the expanded timeout settings from Settings, including the unrestricted timeout policy', async () => {
    let settingsResponse = createSettingsResponse();
    const savedBodies: Partial<SettingsResponse['settings']>[] = [];
    ...
    vi.stubGlobal(
      ...
      vi.fn(async (input: Parameters<typeof fetch>[0], init?: Parameters<typeof fetch>[1]) => {
        ...
        if (url === '/api/settings' && init?.method === 'PUT') {
          const payload = JSON.parse(String(init.body ?? '{}')) as Partial<SettingsResponse['settings']>;
          savedBodies.push(payload);
          settingsResponse = createSettingsResponse({
            settings: {
  ...
  it('updates theme optimistically from the top-right toggle without immediately flipping back after settings sync', async () => {
    const settingsResponse = createSettingsResponse();
    const settingsSave = createDeferred<Response>();
  ...
  it('drives provider and model selection from structured runtime metadata and uses the provider drawer for extra config', async () => {
    const settingsResponse = createSettingsResponse();
    const initialProviders = createModelProviderResponse();
    const savedProviders = createModelProviderResponse({
      config: {
        ...initialProviders.config,
        defaultModel: 'openai/gpt-5.4-mini'
      }
    });
    const anthropicProviders = createModelProviderResponse({
      inspectedProviderId: 'anthropic'
    });
    ...
    vi.stubGlobal(
      ...
      vi.fn(async (input: Parameters<typeof fetch>[0], init?: Parameters<typeof fetch>[1]) => {
        ...
        if (url === '/api/model-providers?profileId=jbarton') {
          return jsonResponse(initialProviders);
        }
        ...
        if (url === '/api/model-providers?profileId=jbarton&inspectedProviderId=anthropic') {
          return jsonResponse(anthropicProviders);
        }

        if (url === '/api/model-providers' && init?.method === 'PUT') {
          return jsonResponse(savedProviders);
        }
    ...
    expect(within(providerSelect).getByRole('option', { name: 'Anthropic' })).toBeDisabled();
    expect(within(modelSelect).getByRole('option', { name: 'GPT-5.4' })).toBeInTheDocument();
    ...
    await userEvent.click(within(anthropicRow as HTMLTableRowElement).getByRole('button', { name: 'Configure' }));

    const providerDrawer = await screen.findByTestId('provider-config-drawer');
    expect(providerDrawer).toBeInTheDocument();
    expect(within(providerDrawer).getByText('Anthropic')).toBeInTheDocument();
    expect(within(providerDrawer).getByText('missing')).toBeInTheDocument();
    expect(within(providerDrawer).getByText('Guided setup')).toBeInTheDocument();
    expect(within(providerDrawer).getByText('Add API key')).toBeInTheDocument();
    expect(within(providerDrawer).getByPlaceholderText('Paste an Anthropic API key')).toBeInTheDocument();
    expect(within(providerDrawer).getByLabelText('Default model')).toBeDisabled();
    expect(within(providerDrawer).queryByLabelText('Base URL')).not.toBeInTheDocument();
  });

  it('keeps the provider drawer initialized while details load and localizes inspection failures without resetting Settings', async () => {
    const settingsResponse = createSettingsResponse();
    const modelProviderResponse = createModelProviderResponse();
    const inspectionResponse = createDeferred<Response>();
    ...
    vi.stubGlobal(
      ...
      vi.fn(async (input: Parameters<typeof fetch>[0], init?: Parameters<typeof fetch>[1]) => {
        ...
        if (url === '/api/model-providers?profileId=jbarton&inspectedProviderId=openrouter') {
          return inspectionResponse.promise;
        }
    ...
    await userEvent.click(within(openrouterRow as HTMLTableRowElement).getByRole('button', { name: 'Configure' }));

    const providerDrawer = await screen.findByTestId('provider-config-drawer');
    expect(providerDrawer).toBeInTheDocument();
    expect(within(providerDrawer).getByText('OpenRouter')).toBeInTheDocument();
    expect(within(providerDrawer).queryByText('No provider selected')).not.toBeInTheDocument();
    expect(within(providerDrawer).getByText('Loading provider settings')).toBeInTheDocument();

    inspectionResponse.resolve(
      jsonResponse(
        {
          error: {
            code: 'PROVIDER_INSPECT_FAILED',
            message: 'Fixture provider inspection failed.'
          }
        },
        500
      )
    ...
    expect(await within(providerDrawer).findByText('Fixture provider inspection failed.')).toBeInTheDocument();
    expect(screen.getByTestId('page-header')).toBeInTheDocument();
    expect(screen.getByTestId('provider-table-scroll')).toBeInTheDocument();
    expect(screen.queryByText('Runtime configuration failed')).not.toBeInTheDocument();
  ...
  it('prevents freeform OpenRouter model entry when discovery is unavailable', async () => {
    const settingsResponse = createSettingsResponse();
    const modelProviderResponse = createModelProviderResponse({
      providers: createModelProviderResponse().providers.map((provider) =>
        provider.id === 'openrouter'
              ...
              configurationFields: provider.configurationFields.map((field) =>
                field.key === 'defaultModel'
                  ? {
                      ...
                      disabled: true,
                      disabledReason:
                        'Hermes could not discover selectable OpenRouter models yet. Refresh the drawer after runtime discovery completes. Manual model entry is unavailable for this provider.',
                      description: 'Hermes must expose a selectable OpenRouter model list before this provider can be changed here.'
                    }
                  : field
              )
            }
  ...
  it('renders structured provider setup steps and exact verification links without raw CLI output', async () => {
    const settingsResponse = createSettingsResponse();
    const baseProviders = createModelProviderResponse().providers;
    const nousProvider = {
      ...baseProviders[1],
      id: 'nous',
      ...
      stateMessage: 'Authorize Nous Portal before Hermes can use this provider.',
      ready: false,
      ...
      notes: 'Use structured Hermes provider auth.',
      disabled: true,
      disabledReason: 'Connect Nous Portal before selecting it.',
      ...
      setupSteps: [
        ...
        },
        {
          id: 'nous:oauth',
          kind: 'oauth' as const,
          title: 'Start provider sign-in',
          description: 'Start Nous Portal authorization and follow the structured verification link.',
          status: 'action_required' as const,
          command: 'hermes models auth --provider nous --json',
          metadata: {}
        },
        {
          id: 'nous:model',
        ...
        },
        {
          ...
          title: 'Monitor provider readiness',
          description: 'Keep this drawer open after setup. Refresh status rechecks Hermes immediately.',
    ...
    const modelProviderResponse = createModelProviderResponse({
      providers: [baseProviders[0], nousProvider],
      inspectedProviderId: 'nous'
    });
    const startedNousProvider = {
      ...nousProvider,
      status: 'missing' as const,
      state: 'waiting_for_verification' as const,
      stateMessage: 'Open the Nous verification link, approve access, then refresh provider status.',
      supportsDisconnect: true,
      authSession: {
        id: 'nous-session-1',
        providerId: 'nous',
        kind: 'oauth_device_code' as const,
        status: 'pending' as const,
        message: 'Open the Nous verification link, approve access, then refresh provider status.',
        actionUrl: 'https://portal.nousresearch.com/device/verify?user_code=39RS-VCYV',
        actionLabel: 'Open verification link',
        verificationCode: '39RS-VCYV',
        startedAt: '2026-04-08T20:00:00.000Z',
        checkedAt: '2026-04-08T20:00:00.000Z',
      ...
      setupSteps: [
        nousProvider.setupSteps[0],
        {
          id: 'nous:oauth',
          kind: 'oauth' as const,
          title: 'Start provider sign-in',
          description: 'Open the Nous verification link, approve access, then refresh provider status.',
          status: 'pending' as const,
          actionLabel: 'Open verification link',
          actionUrl: 'https://portal.nousresearch.com/device/verify?user_code=39RS-VCYV',
          command: 'hermes models auth --provider nous --json',
          metadata: {
            authSessionId: 'nous-session-1'
          }
        },
        nousProvider.setupSteps[2],
        {
          id: 'nous:refresh',
        ...
        nousProvider.setupSteps[3]
      ]
    };
    const startedNousResponse = createModelProviderResponse({
      providers: [baseProviders[0], startedNousProvider],
      inspectedProviderId: 'nous'
    });
    ...
    vi.stubGlobal(
      ...
      vi.fn(async (input: Parameters<typeof fetch>[0], init?: Parameters<typeof fetch>[1]) => {
        ...
        if (url === '/api/model-providers?profileId=jbarton&inspectedProviderId=nous') {
          return jsonResponse({
            ...modelProviderResponse,
            inspectedProviderId: 'nous'
          });
        ...
        if (url === '/api/provider-auth' && init?.method === 'POST') {
          return jsonResponse(startedNousResponse);
        }

        if (url === '/api/provider-auth/poll' && init?.method === 'POST') {
          return jsonResponse(startedNousResponse);
        }
    ...
    await userEvent.click(within(nousRow as HTMLTableRowElement).getByRole('button', { name: 'Configure' }));

    const providerDrawer = await screen.findByTestId('provider-config-drawer');
    await userEvent.click(within(providerDrawer).getByRole('button', { name: 'Start provider auth' }));
    const verificationLink = within(providerDrawer).getByRole('link', { name: 'Open verification link' });

    expect(within(providerDrawer).getByText('waiting for verification')).toBeInTheDocument();
    expect(verificationLink).toHaveAttribute('href', 'https://portal.nousresearch.com/device/verify?user_code=39RS-VCYV');
    expect(within(providerDrawer).queryByText('Select provider:')).not.toBeInTheDocument();
    expect(within(providerDrawer).queryByText('Traceback (most recent call last):')).not.toBeInTheDocument();
  });

  it('keeps the provider drawer open and shows a visible local error when provider connection fails', async () => {
    const settingsResponse = createSettingsResponse();
    const baseProviders = createModelProviderResponse().providers;
    const minimaxProvider = {
      ...
      displayName: 'MiniMax',
      status: 'missing' as const,
      state: 'needs_api_key' as const,
      stateMessage: 'Add an API key for MiniMax before Hermes can use this provider.',
      ready: false,
      source: 'catalog',
      disabled: true,
      disabledReason: 'Connect MiniMax before selecting it for this profile.',
      ...
      configurationFields: [
        {
          ...
          description: 'Paste the MiniMax API key below.',
          required: true,
          secret: true,
          placeholder: 'Paste the MiniMax API key',
          options: [],
          disabled: false
        },
        {
      ...
      setupSteps: [
        {
          ...
          kind: 'inspect' as const,
          title: 'Inspect Hermes runtime metadata',
          description: 'The bridge loaded current provider status, persisted Hermes config, and the latest discovered model options.',
          status: 'completed' as const,
          metadata: {}
        },
        {
          ...
          title: 'Add API key',
          description: 'Paste the MiniMax API key below. The bridge stores the secret in Hermes, and only non-secret connection metadata is persisted locally.',
          status: 'action_required' as const,
          metadata: {}
        },
        {
    ...
    const modelProviderResponse = createModelProviderResponse({
      providers: [baseProviders[0], minimaxProvider]
    });
    ...
    vi.stubGlobal(
      ...
      vi.fn(async (input: Parameters<typeof fetch>[0], init?: Parameters<typeof fetch>[1]) => {
        ...
        if (url === '/api/model-providers?profileId=jbarton&inspectedProviderId=minimax') {
          return jsonResponse({
            ...modelProviderResponse,
            inspectedProviderId: 'minimax'
          });
        }

        if (url === '/api/provider-connections' && init?.method === 'POST') {
          return jsonResponse(
            {
              error: {
    ...
    await userEvent.click(within(minimaxRow as HTMLTableRowElement).getByRole('button', { name: 'Configure' }));

    const providerDrawer = await screen.findByTestId('provider-config-drawer');
    await userEvent.type(within(providerDrawer).getByPlaceholderText('Paste the MiniMax API key'), 'invalid-minimax-key');
    await userEvent.click(within(providerDrawer).getByRole('button', { name: 'Connect provider' }));

    expect(await within(providerDrawer).findByText('Provider action failed')).toBeInTheDocument();
    expect(within(providerDrawer).getByText('MiniMax rejected the supplied API key.')).toBeInTheDocument();
    expect(providerDrawer).toBeVisible();
    expect(screen.getByTestId('page-header')).toBeInTheDocument();
  });
