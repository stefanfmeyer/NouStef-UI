...
function getWindowsDataDirectory(appName) {
    const base = process.env.APPDATA ?? path.join(os.homedir(), 'AppData', 'Roaming');
    return path.join(base, appName);
}
function getMacDataDirectory(appName) {
    return path.join(os.homedir(), 'Library', 'Application Support', appName);
}
function getLinuxDataDirectory(appName) {
...
export function resolveBridgeDatabasePath(inputPath) {
