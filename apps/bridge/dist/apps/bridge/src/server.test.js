...
async function waitForSessionMessages(baseUrl, profileId, sessionId, predicate, timeoutMs = 4_000) {
    const startedAt = Date.now();
    let latest;
    while (Date.now() - startedAt < timeoutMs) {
        latest = await fetch(`${baseUrl}/api/sessions/${encodeURIComponent(sessionId)}/messages?profileId=${encodeURIComponent(profileId)}`).then((result) => readJson(result));
    ...
    return fetch(`${baseUrl}/api/sessions/${encodeURIComponent(sessionId)}/messages?profileId=${encodeURIComponent(profileId)}`).then((result) => readJson(result));
}
function getPrimaryDynamicDataset(recipe) {
...
function getRecipeContentFormat(recipe) {
...
function hasMeaningfulAttachedRecipe(recipe) {
    const primaryDynamicDataset = getPrimaryDynamicDataset(recipe);
    if (primaryDynamicDataset) {
        return (primaryDynamicDataset.items.length > 0 ||
    ...
    return (getRecipeContentViewData(contentTab, 'markdown').markdown.trim().length > 0 ||
        getRecipeContentViewData(contentTab, 'table').rows.length > 0 ||
        getRecipeContentViewData(contentTab, 'card').cards.length > 0);
}
function expectBaselineHomeRecipe(recipe, expectedSnippet) {
...
function expectLegacyHomeRecipe(recipe, expectedSnippet, options = {}) {
...
function expectPromotedRichHomeRecipe(recipe, expectedSnippet) {
    expectBaselineHomeRecipe(recipe, expectedSnippet);
    if (!recipe) {
        return;
    }
...
describe.sequential('bridge server', () => {
    it('returns real structured bootstrap data from Hermes fixture output', async () => {
        ...
        expect(payload.profiles.some((profile) => profile.id === '8tn')).toBe(true);
        expect(payload.profiles.some((profile) => profile.id === 'jbarton')).toBe(true);
    ...
    it('imports session transcripts from sessions export on demand', async () => {
        ...
        const transcript = await fetch(`${server.baseUrl}/api/sessions/${sessionId}/messages?profileId=${encodeURIComponent(profileId)}`).then((result) => readJson(result));
    ...
    it('returns jobs and tools from Hermes fixture output', async () => {
        ...
        expect(tools.items.some((tool) => tool.id === 'bridge:reviewed-shell')).toBe(true);
        expect(tools.items.some((tool) => tool.id === 'hermes:cli:web')).toBe(true);
        await server.close();
    });
    it('allows trusted local origins and rejects non-local origins for JSON and SSE routes', async () => {
        ...
        const forbiddenPayload = await readJson(forbidden);
    ...
    it('returns skills and model/provider state and persists provider updates', async () => {
        ...
        tempRoots.push(tempRoot);
        const server = await startServer(tempRoot);
        const skills = await fetch(`${server.baseUrl}/api/skills?profileId=8tn`).then((result) => readJson(result));
        const providers = await fetch(`${server.baseUrl}/api/model-providers?profileId=8tn`).then((result) => readJson(result));
        ...
        const otherProfileProviders = await fetch(`${server.baseUrl}/api/model-providers?profileId=jbarton`).then((result) => readJson(result));
        const settings = await fetch(`${server.baseUrl}/api/settings`).then((result) => readJson(result));
        ...
        expect(providers.providers.some((item) => item.id === 'openrouter' &&
    ...
    it('persists expanded timeout settings through the bridge settings API', async () => {
        ...
        const reloaded = await fetch(`${server.baseUrl}/api/settings`).then((result) => readJson(result));
    ...
    it('uses discovery and login for provider auth routes in v0.9.0', async () => {
        ...
        tempRoots.push(tempRoot);
        const server = await startServer(tempRoot);
        const openrouterProviders = await fetch(`${server.baseUrl}/api/model-providers?profileId=jbarton&inspectedProviderId=openrouter`).then((result) => readJson(result));
    ...
    it('falls back to dump when model --json is unavailable in v0.9.0', async () => {
        ...
        const payload = await readJson(providers);
    ...
    it('renames and deletes sessions through the bridge API and keeps deleted sessions hidden', async () => {
        ...
        tempRoots.push(tempRoot);
        const server = await startServer(tempRoot);
        const sessionsBeforeRename = await fetch(`${server.baseUrl}/api/sessions?page=1&pageSize=25&search=&profileId=jbarton`).then((result) => readJson(result));
        ...
        const sessionsAfterRename = await fetch(`${server.baseUrl}/api/sessions?page=1&pageSize=25&search=&profileId=jbarton`).then((result) => readJson(result));
        ...
        const sessionsAfterDelete = await fetch(`${server.baseUrl}/api/sessions?page=1&pageSize=25&search=&profileId=jbarton`).then((result) => readJson(result));
    ...
    it('deletes hub skills through the bridge API and persists the removal', async () => {
        ...
        tempRoots.push(tempRoot);
        const server = await startServer(tempRoot);
        const skillsBeforeDelete = await fetch(`${server.baseUrl}/api/skills?profileId=jbarton`).then((result) => readJson(result));
        ...
        const skillsAfterDelete = await fetch(`${server.baseUrl}/api/skills?profileId=jbarton`).then((result) => readJson(result));
...
`
        ...
        const initialSkills = await fetch(`${first.baseUrl}/api/skills?profileId=8tn`).then((result) => readJson(result));
        ...
        const cachedSkills = await fetch(`${degraded.baseUrl}/api/skills?profileId=8tn`).then((result) => readJson(result));
        ...
        const afterDelete = await fetch(`${finalServer.baseUrl}/api/skills?profileId=8tn`).then((result) => readJson(result));
    ...
    it('creates, updates, persists, reopens, and deletes session-attached spaces through the bridge API', async () => {
        ...
        const attachedSessionMessages = await fetch(`${server.baseUrl}/api/sessions/${encodeURIComponent(session.id)}/messages?profileId=${encodeURIComponent('jbarton')}`).then((result) => readJson(result));
        ...
        const listedBeforeRestart = await fetch(`${server.baseUrl}/api/recipes?profileId=jbarton`).then((result) => readJson(result));
        ...
        const reopenedSessionMessages = await fetch(`${reopened.baseUrl}/api/sessions/${encodeURIComponent(session.id)}/messages?profileId=${encodeURIComponent('jbarton')}`).then((result) => readJson(result));
        ...
        const sessionsAfterDelete = await fetch(`${reopened.baseUrl}/api/sessions?page=1&pageSize=25&search=&profileId=jbarton`).then((result) => readJson(result));
        const messagesAfterDelete = await fetch(`${reopened.baseUrl}/api/sessions/${encodeURIComponent(session.id)}/messages?profileId=${encodeURIComponent('jbarton')}`).then((result) => readJson(result));
    ...
    it('keeps chat as the persisted page when opening and loading an attached-recipe session', async () => {
        ...
        const beforeMessagesBootstrap = await fetch(`${server.baseUrl}/api/bootstrap`).then((result) => readJson(result));
        ...
        const loadedMessages = await fetch(`${server.baseUrl}/api/sessions/${encodeURIComponent(openedChat.session.id)}/messages?profileId=${encodeURIComponent('jbarton')}`).then((result) => readJson(result));
        expect(loadedMessages.attachedRecipe?.id).toBe(created.recipe.id);
        const afterMessagesBootstrap = await fetch(`${server.baseUrl}/api/bootstrap`).then((result) => readJson(result));
    ...
    it('strips legacy Hermes recipe-operation blocks during chat streams and keeps the baseline Home recipe attached to the session', async () => {
        ...
        const afterCreate = await fetch(`${server.baseUrl}/api/sessions/${encodeURIComponent(session.id)}/messages?profileId=${encodeURIComponent(profileId)}`).then((result) => readJson(result));
        ...
        const updatedSession = await fetch(`${server.baseUrl}/api/sessions/${encodeURIComponent(session.id)}/messages?profileId=${encodeURIComponent(profileId)}`).then((result) => readJson(result));
        ...
        const afterDelete = await fetch(`${server.baseUrl}/api/sessions/${encodeURIComponent(session.id)}/messages?profileId=${encodeURIComponent(profileId)}`).then((result) => readJson(result));
    ...
    it('creates stable Home recipes for legacy-style fixture prompts and preserves their baseline or rich state across restart', async () => {
        ...
        for (const item of prompts) {
            ...
            const sessionMessages = await fetch(`${server.baseUrl}/api/sessions/${encodeURIComponent(session.id)}/messages?profileId=${encodeURIComponent(profileId)}`).then((result) => readJson(result));
        ...
        for (const created of createdSessions) {
            const sessionMessages = await fetch(`${reopened.baseUrl}/api/sessions/${encodeURIComponent(created.sessionId)}/messages?profileId=${encodeURIComponent(profileId)}`).then((result) => readJson(result));
            expect(sessionMessages.session.attachedRecipeId).toBe(sessionMessages.attachedRecipe?.id ?? null);
    ...
    it('auto-creates Home recipes for structured-result prompts and promotes richer declarative recipes without requiring the word recipe', async () => {
        ...
        for (const prompt of prompts) {
            ...
            const completeEvent = events.find((event) => event.type === 'complete');
            await server.bridge.waitForRecipeEnrichmentQueues();
            const sessionMessages = await waitForSessionMessages(server.baseUrl, profileId, session.id, (payload) => Boolean(payload.attachedRecipe &&
                (payload.attachedRecipe.dynamic?.recipeTemplate ||
                    payload.attachedRecipe.dynamic?.activeBuild?.phase === 'failed')), 10_000);
            expect(completeEvent?.type).toBe('complete');
            expect(events.some((event) => event.type === 'recipe_event')).toBe(true);
    ...
    it('keeps recovered legacy recipe blocks stable while promoting richer declarative recipes only where structured intent warrants them', async () => {
        ...
        for (const prompt of prompts) {
            ...
            await server.bridge.waitForRecipeEnrichmentQueues();
            const sessionMessages = await fetch(`${server.baseUrl}/api/sessions/${encodeURIComponent(session.id)}/messages?profileId=${encodeURIComponent(profileId)}`).then((result) => readJson(result));
            const telemetry = await fetch(`${server.baseUrl}/api/telemetry?profileId=${encodeURIComponent(profileId)}&sessionId=${encodeURIComponent(session.id)}&limit=20`).then((result) => readJson(result));
            const completeEvent = events.find((event) => event.type === 'complete');
            expect(events.some((event) => event.type === 'recipe_event')).toBe(true);
            expect(events.some((event) => event.type === 'activity' && event.activity.label === 'Hermes UI Recipes block' && event.activity.state === 'failed')).toBe(false);
    ...
    it('keeps malformed legacy recipe blocks as baseline Home recipes while preserving the assistant answer', async () => {
        ...
        const spacesBefore = await fetch(`${server.baseUrl}/api/recipes?profileId=${encodeURIComponent(profileId)}`).then((result) => readJson(result));
        const events = await fetch(`${server.baseUrl}/api/chat/stream`, {
            method: 'POST',
            headers: {
                'content-type': 'application/json'
        ...
        const sessionMessages = await fetch(`${server.baseUrl}/api/sessions/${encodeURIComponent(session.id)}/messages?profileId=${encodeURIComponent(profileId)}`).then((result) => readJson(result));
        const spacesAfter = await fetch(`${server.baseUrl}/api/recipes?profileId=${encodeURIComponent(profileId)}`).then((result) => readJson(result));
        const telemetry = await fetch(`${server.baseUrl}/api/telemetry?profileId=${encodeURIComponent(profileId)}&sessionId=${encodeURIComponent(session.id)}&limit=20`).then((result) => readJson(result));
        const errorEvent = events.find((event) => event.type === 'error');
        const completeEvent = events.find((event) => event.type === 'complete');
        expect(events.some((event) => event.type === 'recipe_event')).toBe(true);
        ...
        expect(sessionMessages.messages.some((message) => message.role === 'assistant' && message.content.includes('Tried to create a malformed recipe block.'))).toBe(true);
        expect(completeEvent?.type).toBe('complete');
        if (completeEvent?.type === 'complete') {
            expect(completeEvent.assistantMessage.content).not.toContain('hermes-ui-recipes');
        }
    ...
    it('reopens the existing primary session for attached-recipe chat and keeps the session after deleting the recipe', async () => {
        ...
        const sessions = await fetch(`${server.baseUrl}/api/sessions?page=1&pageSize=25&search=&profileId=jbarton`).then((result) => readJson(result));
    ...
    it('converts attached-recipe content formats across markdown/table/cards and drops legacy todo state', async () => {
        ...
        const jobs = await fetch(`${server.baseUrl}/api/jobs?profileId=jbarton`).then((result) => readJson(result));
    ...
    it('filters synthetic sessions and keeps listings scoped to the selected profile', async () => {
        ...
        expect(bootstrap.recentSessions.some((session) => /fixture|smoke/i.test(`${session.id} ${session.title} ${session.summary}`))).toBe(false);
    ...
    it('rejects transcripts from a different profile instead of leaking session associations', async () => {
        ...
        tempRoots.push(tempRoot);
        const server = await startServer(tempRoot);
        const eightSessions = await fetch(`${server.baseUrl}/api/sessions?page=1&pageSize=25&search=&profileId=8tn`).then((result) => readJson(result));
        const foreignSessionId = eightSessions.items[0]?.id;
        expect(foreignSessionId).toBeTruthy();
        const crossProfileResponse = await fetch(`${server.baseUrl}/api/sessions/${encodeURIComponent(foreignSessionId ?? '')}/messages?profileId=jbarton`);
        const crossProfilePayload = await readJson(crossProfileResponse);
        const jbartonSessions = await fetch(`${server.baseUrl}/api/sessions?page=1&pageSize=25&search=&profileId=jbarton`).then((result) => readJson(result));
    ...
    it('streams chat replies and persists the new session transcript', async () => {
        ...
        const transcript = await fetch(`${server.baseUrl}/api/sessions/${session.id}/messages?profileId=${encodeURIComponent(bootstrap.activeProfileId ?? 'jbarton')}`).then((result) => readJson(result));
        const reloadedTranscript = await fetch(`${server.baseUrl}/api/sessions/${session.id}/messages?profileId=${encodeURIComponent(bootstrap.activeProfileId ?? 'jbarton')}`).then((result) => readJson(result));
        const exported = await fetch(`${server.baseUrl}/api/sessions/${session.id}/export?profileId=${encodeURIComponent(bootstrap.activeProfileId ?? 'jbarton')}`).then((result) => readJson(result));
        const latestRuntimeRequest = transcript.runtimeRequests.at(-1);
        expect(events.some((event) => event.type === 'assistant_snapshot')).toBe(true);
        ...
        expect(latestRuntimeRequest?.messageIds).toEqual(expect.arrayContaining([
        ...
        expect(exported.messages.map((message) => message.content).join('\n')).not.toMatch(/Bridge execution note|Raw system message|python scripts\/check_mail\.py|Built-in toolsets|session_id:|Traceback|No such file or directory/u);
        await server.close();
    });
    it('stores chat failure telemetry and finalizes failed runtime requests', async () => {
        ...
        const events = await collectStreamEvents(streamResponse);
        const transcript = await fetch(`${server.baseUrl}/api/sessions/${session.id}/messages?profileId=${encodeURIComponent(bootstrap.activeProfileId ?? 'jbarton')}`).then((result) => readJson(result));
        const telemetry = await fetch(`${server.baseUrl}/api/telemetry?profileId=${encodeURIComponent(bootstrap.activeProfileId ?? 'jbarton')}&sessionId=${encodeURIComponent(session.id)}&limit=20`).then((result) => readJson(result));
        const requestTelemetry = await fetch(`${server.baseUrl}/api/telemetry?profileId=${encodeURIComponent(bootstrap.activeProfileId ?? 'jbarton')}&sessionId=${encodeURIComponent(session.id)}&requestId=${encodeURIComponent(transcript.runtimeRequests.at(-1)?.requestId ?? '')}&limit=20`).then((result) => readJson(result));
        const telemetryByProfile = await fetch(`${server.baseUrl}/api/telemetry?profileId=${encodeURIComponent(bootstrap.activeProfileId ?? 'jbarton')}&limit=20`).then((result) => readJson(result));
        const telemetryAll = await fetch(`${server.baseUrl}/api/telemetry?limit=20`).then((result) => readJson(result));
    ...
    it('supports unrestricted access mode and audits when it is enabled and used', async () => {
        ...
        const events = await collectStreamEvents(streamResponse);
        const settingsAfterChat = await fetch(`${server.baseUrl}/api/settings`).then((result) => readJson(result));
        expect(updatedSettings.settings.unrestrictedAccessEnabled).toBe(true);
        expect(events.some((event) => event.type === 'complete')).toBe(true);
        expect(settingsAfterChat.accessAudit.latestEvents.some((event) => event.type === 'unrestricted_access_enabled' || event.type === 'unrestricted_access_used')).toBe(true);
    ...
    it('keeps unread-email replies focused on the final assistant answer', async () => {
        ...
        if (completeEvent?.type === 'complete') {
            ...
            expect(completeEvent.assistantMessage.content).not.toMatch(/Reached maximum iterations|Tool gmail_unread_count|Built-in toolsets|profile 8tn|AUTH_SCOPE_MISMATCH|session_id:/i);
        }
        expect(assistantMessages.some((content) => /jbarton|google recipe/i.test(content))).toBe(true);
        expect(assistantMessages.join('\n')).not.toMatch(/Reached maximum iterations|Tool gmail_unread_count|Built-in toolsets|profile 8tn|AUTH_SCOPE_MISMATCH|session_id:/i);
        await server.close();
    });
    ...
    it('keeps nearby-search chats responsive, emits search progress, and persists runtime tool history', async () => {
        ...
        const history = await fetch(`${server.baseUrl}/api/tool-history?page=1&pageSize=25&profileId=${encodeURIComponent(profileId)}`).then((result) => readJson(result));
        const telemetry = await fetch(`${server.baseUrl}/api/telemetry?profileId=${encodeURIComponent(profileId)}&sessionId=${encodeURIComponent(session.id)}&limit=20`).then((result) => readJson(result));
        ...
        expect(history.runtimeItems.some((item) => item.label === 'nearby_restaurants_search' && item.state === 'completed' && item.sessionId === session.id)).toBe(true);
    ...
    it('records reviewed tool history for approvals and rejections', async () => {
        ...
        const history = await fetch(`${server.baseUrl}/api/tool-history?page=1&pageSize=25&profileId=${encodeURIComponent(bootstrap.activeProfileId ?? 'jbarton')}`).then((result) => readJson(result));
        expect(history.items.some((item) => item.status === 'completed')).toBe(true);
        expect(history.items.some((item) => item.status === 'rejected')).toBe(true);
        await server.close();
    });
    ...
    it('surfaces degraded cached bootstrap state when Hermes profile discovery fails', async () => {
        ...
        const degradedBootstrap = await fetch(`${server.baseUrl}/api/bootstrap`).then((result) => readJson(result));
    ...
    it('migrates a legacy Electron snapshot into SQLite before serving the browser bridge', async () => {
        ...
        const bootstrap = await fetch(`${first.baseUrl}/api/bootstrap`).then((result) => readJson(result));
        const importedTranscript = await fetch(`${first.baseUrl}/api/sessions/legacy-runtime-session/messages?profileId=default`).then((result) => readJson(result));
        const localTranscript = await fetch(`${first.baseUrl}/api/sessions/fallback-local-session/messages?profileId=default`).then((result) => readJson(result));
        process.env.HERMES_FIXTURE_FAIL = 'cron';
        const jobs = await fetch(`${first.baseUrl}/api/jobs?profileId=default`).then((result) => readJson(result));
        const history = await fetch(`${first.baseUrl}/api/tool-history?page=1&pageSize=25&profileId=default`).then((result) => readJson(result));
        ...
        const reopenedSessions = await fetch(`${reopened.baseUrl}/api/sessions?page=1&pageSize=25&search=Legacy&profileId=default`).then((result) => readJson(result));
