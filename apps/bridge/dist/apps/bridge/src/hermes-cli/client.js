...
function createDefaultHermesCliRunner(cliPath) {
    return {
...
function isCliTableDivider(line) {
    return /^[\s─-]+$/u.test(line.trim());
}
function isCliHeaderLine(line, headerLabel) {
...
function parseProfileShow(profileId, output, isActive) {
...
function parseRelativeSessionTimestamp(value, referenceTime) {
    ...
    const match = normalized.match(/^(?<amount>\d+)\s*(?<unit>s|sec|secs|second|seconds|m|min|mins|minute|minutes|h|hr|hrs|hour|hours|d|day|days|w|wk|wks|week|weeks)\s+ago$/u);
    ...
    const millisecondsPerUnit = unit.startsWith('s')
        ? 1_000
        : unit.startsWith('m')
            ? 60_000
            : unit.startsWith('h')
...
function parseSessionListTimestamp(value, referenceTime) {
    const parsed = Date.parse(value);
    if (!Number.isNaN(parsed)) {
        return new Date(parsed).toISOString();
    }
...
function parseCronJobs(listOutput, statusOutput, profileId, timestamp) {
    ...
    const pushCurrent = () => {
        ...
        const baseStatus = current.state === 'paused' || current.state === 'disabled'
            ? 'paused'
...
function extractRuntimeSessionId(output) {
...
function extractAssistantBoxContent(output) {
...
function collapseRepeatedParagraphSequences(content) {
...
function extractAssistantBody(output, options = {}) {
...
function parseChatActivity(line, timestamp, requestId = null) {
    ...
    const toolActivityMatch = trimmed.match(/^┊\s+(?<icon>📞|📖|🐍)\s+(?<verb>tool|read|exec)\s+(?<label>.+?)\s+\d+(?:\.\d+)?s(?:\s+\[exit\s+(?<exitCode>-?\d+)\])?$/u);
    if (toolActivityMatch?.groups?.label && toolActivityMatch.groups.verb) {
        const rawLabel = toolActivityMatch.groups.label.trim();
        const toolLabel = toolActivityMatch.groups.verb === 'read'
            ? 'read_file'
...
function progressMessageForActivity(activity) {
...
function defaultProgressMessageForContent(content, normalizedOutput, assistantChunk) {
...
function parseToolsList(output, profileId, timestamp) {
    const normalized = normalizeOutput(output);
    if (!normalized) {
        return [];
    }
...
function parseSkillsList(output, profileId, timestamp) {
    const normalized = normalizeOutput(output);
    if (!normalized) {
        return [];
    }
...
function isRecipeWorkflowIntent(content, hasRecipeContext = false) {
    ...
    return /\b(create|build|make|update|refresh|revise|rename|delete|remove|organize|convert|turn|rerun|repeat)\b.*\b(recipe|recipe|table|card|cards|markdown|note|board|tracker)\b/i.test(content);
}
function isSimpleRecipeWorkflowIntent(content, hasRecipeContext = false) {
    ...
    return !/\b(research|compare|analy[sz]e|investigate|search|discover|nearby|near me|around me|restaurants?|hotels?|coffee|cafe)\b/i.test(content);
}
function resolveIntentContent(content, refreshContext) {
    const refreshPrompt = refreshContext?.intentPrompt?.trim();
    return refreshPrompt && refreshPrompt.length > 0 ? refreshPrompt : content;
}
export function resolveHermesChatTimeoutMs(options) {
...
function buildBridgeChatQuery(profile, content, spaceContext, refreshContext, options = {}) {
...
Do not emit normalizedData, uiSpec, actionSpec, testSpec, or other app-final recipe structures.`;
    ...
    if (options.recipeAppletStage) {
        const stageInstruction = options.recipeAppletStage === 'source'
...
function buildRecipeAppletArtifactQuery(profile, prompt, stage) {
...
function buildRecipeDslArtifactQuery(profile, prompt, stage) {
...
function buildRecipeTemplateArtifactQuery(profile, prompt, stage) {
    const outputInstruction = stage === 'select'
...
function buildRecipeTemplateCorrectionQuery(profile, correctionPrompt, stage) {
    const outputInstruction = stage === 'select'
        ? 'Return only one JSON object with kind "recipe_template_selection" and schemaVersion "recipe_template_selection/v2".'
        : stage === 'text' || stage === 'text_repair'
            ? 'Return only one JSON object with kind "recipe_template_text" and schemaVersion "recipe_template_text/v1".'
...
function validateRecipeAppletArtifactPrompt(prompt) {
...
function validateRecipeDslArtifactPrompt(prompt) {
    const trimmed = prompt.trim();
    if (!trimmed) {
...
function validateRecipeTemplateArtifactPrompt(prompt) {
    const trimmed = prompt.trim();
    if (!trimmed) {
...
function isRecipeAppletLiveTaskActivity(activity) {
    return activity.kind === 'tool' || activity.kind === 'skill' || activity.kind === 'command' || activity.kind === 'approval';
}
function describeRecipeAppletActivity(activity) {
...
export class HermesCli {
    ...
    run(args, env = process.env, signal) {
    ...
    async runExecutable(command, args, env = process.env, signal, timeoutMs = this.timeoutMs) {
        return new Promise((resolve, reject) => {
    ...
    resolveGoogleRecipePaths() {
        ...
        const pythonPath = process.env.HERMES_PYTHON ??
    ...
    async listProfiles(signal) {
        ...
        for (const listedProfile of listedProfiles) {
            ...
            detailedProfiles.push(toProfile({
                ...detailedProfile,
    ...
    async deleteSession(profile, session, signal) {
        const env = buildProfileEnvironment(profile);
    ...
    async exportSessionMessages(profile, session, signal) {
    ...
    async listTools(profile, signal) {
        const timestamp = this.now();
        const env = buildProfileEnvironment(profile);
    ...
    async listSkills(profile, signal) {
        const timestamp = this.now();
        const env = buildProfileEnvironment(profile);
    ...
    async deleteSkill(profile, skillName, signal) {
        const env = buildProfileEnvironment(profile);
    ...
    parseDumpProviders(dumpOutput, profileId, activeProvider, now) {
        ...
        if (apiKeysMatch?.[1]) {
            ...
            for (const line of lines) {
                ...
                if (!id || NON_MODEL_PROVIDERS.has(id))
                    continue;
                ...
                providers.push({
                    ...
                    supportsOAuth,
                    lastSyncedAt: now,
                    state: connected ? 'connected' : (supportsOAuth && !meta?.supportsApiKey) ? 'needs_oauth' : 'needs_api_key',
    ...
    async getRuntimeModelConfig(profile) {
    ...
    async pollProviderAuth(profile, providerId, _authSessionId, signal) {
        return this.discoverRuntimeProviderState(profile, providerId, signal);
    }
    ...
    async deleteGmailMessages(profile, messageIds, signal) {
        ...
        for (const messageId of uniqueMessageIds) {
            const result = await this.runExecutable(pythonPath, [scriptPath, 'gmail', 'modify', messageId, '--add-labels', 'TRASH', '--remove-labels', 'INBOX,UNREAD'], env, signal, Math.max(this.timeoutMs, 30_000));
            if (result.exitCode !== 0) {
    ...
    async generateRecipeAppletArtifact(options) {
                    ...
                    while (newlineIndex >= 0) {
                        ...
                        if (activity) {
                            ...
                            if (isRecipeAppletLiveTaskActivity(activity)) {
                                ...
                                liveTaskViolation = new HermesCliRecipeAppletViolationError(`Recipe applet generation attempted live task activity (${violationDetail}).`, violationDetail);
                                controller.abort();
                                return;
                            }
        ...
        if (/Reached maximum iterations/i.test(normalizedOutput)) {
            throw new HermesCliRecipeAppletViolationError('Recipe applet generation hit the restricted turn limit instead of staying inside the artifact-only contract.');
        }
        return {
    ...
    async generateRecipeDslArtifact(options) {
                ...
                onStdout: (chunk) => {
                    ...
                    while (newlineIndex >= 0) {
                        ...
                        if (activity) {
                            ...
                            if (isRecipeAppletLiveTaskActivity(activity)) {
                                const violationDetail = describeRecipeAppletActivity(activity);
                                liveTaskViolation = new HermesCliRecipeAppletViolationError(`Recipe DSL generation attempted live task activity (${violationDetail}).`, violationDetail);
                                controller.abort();
                                return;
                            }
                            const progressMessage = progressMessageForActivity(activity);
        ...
        const normalizedOutput = stripAnsi(result.stdout).replace(/\r/g, '');
        if (/Reached maximum iterations/i.test(normalizedOutput)) {
            throw new HermesCliRecipeAppletViolationError('Recipe DSL generation hit the restricted turn limit instead of staying inside the artifact-only contract.');
        }
        return {
            assistantMarkdown: extractAssistantBody(normalizedOutput, {
    ...
    async generateRecipeTemplateArtifact(options) {
                ...
                onStdout: (chunk) => {
                    ...
                    while (newlineIndex >= 0) {
                        ...
                        if (activity) {
                            ...
                            if (isRecipeAppletLiveTaskActivity(activity)) {
                                const violationDetail = describeRecipeAppletActivity(activity);
                                liveTaskViolation = new HermesCliRecipeAppletViolationError(`Recipe template generation attempted live task activity (${violationDetail}).`, violationDetail);
                                controller.abort();
                                return;
                            }
                            const progressMessage = progressMessageForActivity(activity);
        ...
        const normalizedOutput = stripAnsi(result.stdout).replace(/\r/g, '');
        if (/Reached maximum iterations/i.test(normalizedOutput)) {
            throw new HermesCliRecipeAppletViolationError('Recipe template generation hit the restricted turn limit instead of staying inside the artifact-only contract.');
        }
        return {
            assistantMarkdown: extractAssistantBody(normalizedOutput, {
                structuredArtifactOnly: true
    ...
    async generateRecipeTemplateArtifactCorrection(options) {
                ...
                onStdout: (chunk) => {
                    ...
                    while (newlineIndex >= 0) {
                        ...
                        if (activity) {
                            ...
                            if (isRecipeAppletLiveTaskActivity(activity)) {
                                const violationDetail = describeRecipeAppletActivity(activity);
                                liveTaskViolation = new HermesCliRecipeAppletViolationError(`Recipe template correction attempted live task activity (${violationDetail}).`, violationDetail);
                                controller.abort();
                                return;
                            }
                            const progressMessage = progressMessageForActivity(activity);
        ...
        const normalizedOutput = stripAnsi(result.stdout).replace(/\r/g, '');
        if (/Reached maximum iterations/i.test(normalizedOutput)) {
            throw new HermesCliRecipeAppletViolationError('Recipe template correction hit the restricted turn limit instead of staying inside the artifact-only contract.');
        }
        return {
            assistantMarkdown: extractAssistantBody(normalizedOutput, {
                structuredArtifactOnly: true
    ...
    async streamChat(options) {
        if (options.recipeAppletStage) {
            throw new HermesCliRecipeAppletContextError('Recipe applet generation must use generateRecipeAppletArtifact() instead of the live Hermes chat path.');
        }
        const env = buildProfileEnvironment(options.profile);
        const args = ['chat', '-Q'];
        ...
        try {
            ...
            if (error instanceof Error && error.name === 'TimeoutError' && isEmailIntent(options.content)) {
                throw new Error(`Hermes timed out while checking email for the active profile ${options.profile.id}. The bridge kept the request scoped to ${options.profile.id}; if Google Recipe is not fully authenticated for this profile, re-run the Google Recipe setup for ${options.profile.id} and try again.`);
            }
            if (error instanceof Error && error.name === 'TimeoutError' && isLocalSearchIntent(options.content)) {
                throw new Error('Hermes timed out while searching for nearby results. The bridge now gives local-search requests more time; retrying with a tighter area or cuisine usually succeeds.');
            }
            throw error;
        }
        if (result.exitCode !== 0) {
        ...
        if (!options.unrestrictedAccessEnabled && /Reached maximum iterations/i.test(normalizedOutput)) {
            throw new Error('Hermes stopped before finishing because the restricted turn limit was reached. Increase the restricted turn limit or enable Unrestricted Access.');
        }
        return {
            assistantMarkdown: extractAssistantBody(normalizedOutput, {
                structuredArtifactOnly: options.structuredArtifactOnly,
