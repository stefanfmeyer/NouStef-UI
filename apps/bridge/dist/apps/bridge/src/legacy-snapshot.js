...
function extractMarkdownBlocks(message) {
...
function summarizeMessages(messages, fallback) {
...
function normalizeLegacyMessage(sessionId, rawMessage, fallbackTimestamp, index) {
...
function parseLegacyProfile(rawProfile, activeProfileId) {
...
function inferRecentSessionIds(snapshot, fallbackSessionIds) {
    ...
    if (activeProfileId) {
        for (const sessionId of asArray(recentByProfile[activeProfileId])) {
    ...
    for (const value of Object.values(recentByProfile)) {
        for (const sessionId of asArray(value)) {
            if (readString(sessionId)) {
                orderedIds.push(sessionId);
            }
        }
...
function inferSessionProfileId(snapshot, sessionId) {
    const pageState = asRecord(snapshot.pageState);
    const profilesState = asRecord(pageState.profiles);
    const recentByProfile = asRecord(profilesState.recentSessionIdsByProfile);
    for (const [profileId, value] of Object.entries(recentByProfile)) {
        if (asArray(value).includes(sessionId)) {
            return profileId;
        }
    }
    return null;
}
function inferSessionAssociationProfileIds(snapshot, sessionId, fallbackProfileId) {
    const pageState = asRecord(snapshot.pageState);
    const profilesState = asRecord(pageState.profiles);
    const recentByProfile = asRecord(profilesState.recentSessionIdsByProfile);
    const associatedProfileIds = [];
    for (const [profileId, value] of Object.entries(recentByProfile)) {
        if (asArray(value).includes(sessionId) && !associatedProfileIds.includes(profileId)) {
...
function parseLegacySession(rawSession, snapshot, fallbackTimestamp) {
...
function createOrphanSession(sessionId, snapshot, fallbackTimestamp, messages) {
...
function normalizeLegacyMessages(sessionId, rawMessages, fallbackTimestamp) {
...
function normalizeLegacyJobs(snapshot, profileId, importedAt) {
...
function normalizeLegacyJobsFreshness(snapshot, profileId, importedAt) {
    ...
    return {
        profileId,
        status: status === 'connected' || status === 'disconnected' || status === 'error' || status === 'refreshing'
            ? status
            : 'disconnected',
...
function normalizeLegacyToolExecutions(snapshot, importedAt, fallbackProfileId) {
    const executions = [];
    for (const [index, rawExecution] of asArray(snapshot.toolExecutionHistory).entries()) {
        ...
        executions.push({
            ...
            status: status === 'pending' || status === 'approved' || status === 'rejected' || status === 'completed' || status === 'failed'
                ? status
                : 'completed',
            requestedAt,
...
function inferUiState(snapshot, profileIds, sessionIds) {
    const pageState = asRecord(snapshot.pageState);
    ...
    const currentPage = activeTab === 'jobs'
        ? 'jobs'
        : activeTab === 'tools'
            ? 'tools'
    ...
    const recentSessionIdsByProfile = activeProfileId && sessionIds.length > 0
        ? {
...
function scoreSnapshot(snapshot) {
    ...
    const messagesCount = Object.values(asRecord(snapshot.sessionMessages)).reduce((total, value) => total + asArray(value).length, 0);
    return messagesCount * 1000 + sessionsCount * 100 + toolHistoryCount * 10 + jobsCount * 5 + profilesCount;
}
function loadBestLegacySnapshot(snapshotPaths) {
    const candidates = [];
...
export function migrateLegacySnapshotIfNeeded(database, options) {
    ...
    const orderedSessions = [...sessionRecords.values()].sort((left, right) => right.session.lastUpdatedAt.localeCompare(left.session.lastUpdatedAt));
    ...
    for (const record of orderedSessions) {
        ...
        if (dedupedMessages.length > 0) {
            database.replaceSessionMessages(record.session.id, dedupedMessages, record.lastMessageSyncAt ?? record.updatedAt);
