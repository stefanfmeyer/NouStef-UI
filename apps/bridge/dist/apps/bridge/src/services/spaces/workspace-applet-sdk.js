...
export function useWorkspace() {
    if (!currentContext) {
        throw new Error('useWorkspace() can only be used while rendering an applet.');
    }
...
export function refreshSpace(actionId = 'refresh-space') {
    return { actionId };
}
export function updateLocalState(patch) {
    return patch;
}
export function patchWorkspace(operations) {
    return operations;
}
...
export const Fragment = Symbol.for('workspace-applet-fragment');
export function jsx(type, props) {
    ...
    throw new Error('Workspace applets only support SDK components, not intrinsic HTML elements.');
}
