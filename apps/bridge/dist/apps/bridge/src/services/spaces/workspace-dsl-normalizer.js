import { LegacyWorkspaceDslSchema, WorkspaceAuthoringDslSchema, WorkspaceModelSchema } from '@hermes-workspaces/protocol';
import { synthesizeWorkspaceModelPatch } from './workspace-model';
...
function convertLegacyWorkspaceDsl(raw, repairs) {
    ...
    return {
        ...
        subtitle: raw.workspace.subtitle,
        summary: raw.summary,
        status: raw.workspace.status,
        tabs: tabs.length > 0
            ? tabs
            : [
                {
                    id: raw.workspace.primaryTabId ?? 'tab-overview',
                    label: 'Overview',
                    summary: undefined,
...
function buildCanonicalWorkspaceDsl(input, repairs, warnings, errors) {
    const legacyParse = LegacyWorkspaceDslSchema.safeParse(input.dsl);
    if (legacyParse.success) {
        warnings.push('Converted a legacy strict workspace DSL artifact into the v2 authoring DSL locally.');
        return convertLegacyWorkspaceDsl(legacyParse.data, repairs);
    }
    if (!isRecord(input.dsl)) {
        errors.push('Workspace DSL payload must be a JSON object.');
        return null;
    }
    const hasMeaningfulAuthoringSignal = Array.isArray(input.dsl.tabs) ||
        ...
        isRecord(input.dsl.workspace);
    if (!hasMeaningfulAuthoringSignal) {
        errors.push('Workspace DSL payload was empty or missing required authoring content.');
        return null;
    }
    const workspace = toRecord(input.dsl.workspace);
    ...
    const datasetsSource = Array.isArray(input.dsl.datasets)
        ...
        : Array.isArray(input.dsl.collections)
            ...
            : [];
    const title = asString(input.dsl.title) ?? asString(workspace.title);
    const subtitle = asString(input.dsl.subtitle) ?? asString(workspace.subtitle);
    ...
    const draft = {
        ...
        status: asString(input.dsl.status) === 'idle' ||
            ...
            : asString(workspace.status) === 'idle' ||
                asString(workspace.status) === 'active' ||
                asString(workspace.status) === 'changed' ||
                asString(workspace.status) === 'archived' ||
                asString(workspace.status) === 'error'
                ? asString(workspace.status)
                : undefined,
        tabs,
        datasets,
        sections,
    ...
    const parsed = WorkspaceAuthoringDslSchema.safeParse({
        ...draft,
...
function buildWorkspaceSectionBody(section) {
    if (section.body) {
        return section.body;
    }
...
function materializeWorkspaceModel(dsl, baseModel, stateEffects) {
    ...
    nextModel.sections = dsl.sections.map((section) => {
        ...
        return {
            ...
            status: 'ready',
            body: buildWorkspaceSectionBody(section),
            emptyState: section.emptyState,
            metadata: {
                ...section.metadata,
                links: section.links,
    ...
    if (stateEffects.statusMessage) {
        nextModel.state.localState.workspaceStatusMessage = stateEffects.statusMessage;
    }
    nextModel.revision = Math.max(baseModel.revision + 1, nextModel.revision + 1);
    return nextModel;
}
export function normalizeWorkspaceDsl(input) {
    const warnings = [];
    const errors = [];
    const repairs = createRepairSummary();
    const canonicalDsl = buildCanonicalWorkspaceDsl(input, repairs, warnings, errors);
    if (!canonicalDsl) {
        return {
            dsl: null,
            model: null,
    ...
    const parsedDsl = WorkspaceAuthoringDslSchema.safeParse(filteredDsl);
    ...
    const nextModel = materializeWorkspaceModel(parsedDsl.data, input.baseModel, operationResult.state);
    let parsedModel;
    try {
        parsedModel = WorkspaceModelSchema.parse(nextModel);
    }
    catch (error) {
        errors.push(error instanceof Error ? error.message : 'Workspace DSL normalization failed.');
        return {
            dsl: parsedDsl.data,
            model: null,
            patch: null,
    ...
    if (parsedModel.sections.length === 0 || parsedModel.views.length === 0 || parsedModel.tabs.length === 0) {
        errors.push('Workspace DSL normalization produced an empty section, view, or tab set.');
        return {
            dsl: parsedDsl.data,
            model: null,
            patch: null,
    ...
    return {
        dsl: parsedDsl.data,
        model: parsedModel,
        patch: synthesizeWorkspaceModelPatch(input.baseModel, parsedModel),
        warnings,
        errors,
        repairs
    };
