...
async function main() {
    ...
    if (!contextPath || !manifestPath || !sourcePath) {
        throw new Error('Workspace applet verifier runner requires context, manifest, source, and tests paths.');
    }
    ...
    if (!definition || typeof definition.render !== 'function') {
        throw new Error('Workspace applet module must default-export defineApplet(...).');
    }
    ...
    if (testsPath && fs.existsSync(testsPath)) {
        ...
        if (factory) {
            const generatedTests = typeof factory === 'function'
...
main()
    ...
    process.stdout.write(JSON.stringify({
        ok: false,
        error: error instanceof Error ? error.message : 'Workspace applet runner failed.'
    }));
});
