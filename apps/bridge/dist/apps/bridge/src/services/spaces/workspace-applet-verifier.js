...
import { SpaceAppletRenderTreeSchema } from '../../../../../packages/protocol/src/schemas';
import { collectWorkspaceAppletActionIds, collectWorkspaceAppletNodeKinds, validateWorkspaceAppletSmallPane } from './workspace-applet-render-tree';
import { validateWorkspaceAppletModule } from './workspace-applet-static-validation';
...
function verificationCheck(status, message) {
    return {
        status,
        message
    };
}
function createSkippedVerification(message) {
...
function resolveDatasetReference(datasetId, normalizedData) {
    if (datasetId === 'primary') {
        return normalizedData?.primaryDatasetId ?? normalizedData?.datasets[0]?.id ?? null;
    }
    return datasetId;
}
function buildCapabilityValidation(manifest, normalizedData) {
    const errors = [];
    const warnings = [];
    const declaredDatasets = new Set(normalizedData?.datasets.map((dataset) => dataset.id) ?? []);
    const capabilityMap = new Map(manifest.requestedCapabilities.map((capability) => [capability.id, capability]));
...
function validateRenderTreeAgainstManifest(manifest, renderTree, normalizedData) {
    const errors = [];
    const warnings = [];
    ...
    for (const actionId of collectWorkspaceAppletActionIds(renderTree)) {
        if (!actionIds.has(actionId)) {
            errors.push(`Applet render tree references undeclared action ${actionId}.`);
        }
    }
    const visitDatasetReference = (node) => {
        switch (node.kind) {
            case 'table':
            case 'list':
            case 'detail_panel':
    ...
    if (manifest.usesTabs && !collectWorkspaceAppletNodeKinds(renderTree).includes('tabs')) {
    ...
    if (manifest.usesPagination && !collectWorkspaceAppletNodeKinds(renderTree).includes('paginator')) {
        errors.push('Manifest declared usesPagination=true but the applet render tree does not contain a paginator.');
    }
    if (manifest.usesImages && !collectWorkspaceAppletNodeKinds(renderTree).includes('image')) {
        errors.push('Manifest declared usesImages=true but the applet render tree does not contain an image.');
    }
    return { errors, warnings };
}
function createWorkspaceFiles(tempDirectory, input, sdkSource, jsxRuntimeSource, testSdkSource) {
    writeFile(path.join(tempDirectory, 'package.json'), JSON.stringify({
        name: 'workspace-applet-build',
        private: true,
        type: 'module'
    }, null, 2));
    writeFile(path.join(tempDirectory, 'tsconfig.json'), JSON.stringify({
        compilerOptions: {
            target: 'ES2022',
            module: 'ESNext',
            ...
            strict: true,
            jsx: 'react-jsx',
            jsxImportSource: 'workspace-applet-sdk',
            skipLibCheck: true,
            noEmit: true,
    ...
    writeFile(path.join(tempDirectory, 'node_modules/workspace-applet-sdk/package.json'), JSON.stringify({
        name: 'workspace-applet-sdk',
        private: true,
        type: 'module',
        exports: {
            '.': './index.ts',
    ...
    writeFile(path.join(tempDirectory, 'node_modules/workspace-applet-sdk/index.ts'), sdkSource);
    writeFile(path.join(tempDirectory, 'node_modules/workspace-applet-sdk/jsx-runtime.ts'), jsxRuntimeSource);
    writeFile(path.join(tempDirectory, 'node_modules/workspace-applet-test-sdk/package.json'), JSON.stringify({
        name: 'workspace-applet-test-sdk',
        private: true,
        type: 'module',
        exports: {
            '.': './index.ts'
    ...
    writeFile(path.join(tempDirectory, 'node_modules/workspace-applet-test-sdk/index.ts'), testSdkSource);
}
export async function verifyWorkspaceApplet(input) {
    const checkedAt = new Date().toISOString();
    const sourceValidation = validateWorkspaceAppletModule(input.sourceArtifact.source, {
        fileName: 'applet.tsx',
        kind: 'source'
    });
    const testsValidation = validateWorkspaceAppletModule(input.testArtifact.source, {
    ...
    const tempDirectory = fs.mkdtempSync(path.join(os.tmpdir(), 'workspace-applet-'));
    try {
        const sdkSource = fs.readFileSync(new URL('./workspace-applet-sdk.ts', import.meta.url), 'utf8');
        const jsxRuntimeSource = "export { Fragment, jsx, jsxs } from './index';\n";
        const testSdkSource = fs.readFileSync(new URL('./workspace-applet-test-sdk.ts', import.meta.url), 'utf8');
        createWorkspaceFiles(tempDirectory, input, sdkSource, jsxRuntimeSource, testSdkSource);
        ...
        if (typecheckResult.exitCode !== 0) {
            const detail = typecheckResult.stderr.trim() || typecheckResult.stdout.trim() || 'Workspace applet typecheck failed.';
            return {
                renderTree: null,
                verification: {
                    kind: 'applet_verification',
        ...
        const runnerResult = await runCommand(process.execPath, [
            tsxCliPath,
            fileURLToPath(new URL('./workspace-applet-verifier-runner.ts', import.meta.url)),
        ...
        const runnerOutput = (runnerResult.stdout.trim() || runnerResult.stderr.trim());
        if (!runnerOutput) {
            return {
                renderTree: null,
                verification: createSkippedVerification('Workspace applet runner produced no output.')
            };
        }
        let parsedRunnerResult;
        try {
            parsedRunnerResult = JSON.parse(runnerOutput);
        }
        catch (error) {
            return {
                renderTree: null,
                verification: createSkippedVerification(error instanceof Error ? error.message : 'Workspace applet runner returned invalid JSON.')
            };
        }
        if (!parsedRunnerResult.ok) {
            const errorMessage = parsedRunnerResult.error ?? 'Workspace applet render smoke failed.';
            return {
                renderTree: null,
                verification: {
                    kind: 'applet_verification',
        ...
        let renderTree;
        try {
            renderTree = SpaceAppletRenderTreeSchema.parse(parsedRunnerResult.renderTree);
        }
        catch (error) {
            const detail = error instanceof Error ? error.message : 'Workspace applet render output did not match the render-tree schema.';
            return {
                renderTree: null,
                verification: {
                    kind: 'applet_verification',
        ...
        const smallPaneErrors = validateWorkspaceAppletSmallPane(renderTree);
        ...
        const verification = {
            ...
            capabilityValidation: verificationCheck('passed', 'Capability validation passed.'),
            typecheck: verificationCheck('passed', 'Typecheck passed.'),
            generatedTests: generatedTestFailures.length === 0
