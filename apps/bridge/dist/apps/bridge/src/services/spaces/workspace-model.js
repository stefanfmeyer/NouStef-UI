import { WorkspaceModelSchema, WorkspacePatchSchema } from '@hermes-workspaces/protocol';
...
function mapPreferredView(dataset) {
...
function mapSectionKind(preferredView) {
...
function resolveCollectionSelectionMode(actions, collectionId) {
    ...
    if (relevantActions.some((action) => action.visibility.requiresSelection === 'one_or_more' || action.visibility.requiresSelection === 'any')) {
        return 'multiple';
    }
    return 'multiple';
}
function toWorkspaceEntity(dataset, item) {
    return {
        id: item.id,
        kind: dataset.kind,
        title: item.title,
...
function toWorkspaceCollection(dataset, actions) {
    return {
        id: dataset.id,
        label: dataset.label,
...
function buildOverviewSections(input) {
    ...
    sections.push({
        id: 'section-summary',
        kind: 'summary',
        title: input.summary?.title ?? 'Workspace summary',
...
function buildDatasetSections(dataset, actions) {
...
function nextCollectionState(normalizedData) {
    return Object.fromEntries((normalizedData?.datasets ?? []).map((dataset) => [
        dataset.id,
        {
...
export function applyWorkspacePatchOperations(model, patchOrOperations) {
    ...
    for (const operation of operations) {
        switch (operation.op) {
            case 'upsert_entities': {
                const byId = new Map(nextModel.entities.map((entity) => [entity.id, entity]));
            ...
            case 'update_collection': {
                const byId = new Map(nextModel.collections.map((collection) => [collection.id, collection]));
            ...
            case 'reorder_collection':
                nextModel.collections = nextModel.collections.map((collection) => collection.id === operation.collectionId ? { ...collection, entityIds: [...operation.entityIds] } : collection);
                break;
            case 'create_tab': {
                const byId = new Map(nextModel.tabs.map((tab) => [tab.id, tab]));
            ...
            case 'replace_section':
                nextModel.sections = nextModel.sections.map((section) => section.id === operation.section.id ? operation.section : section);
            ...
            case 'append_notes':
                if (operation.target === 'entity' && operation.entityId) {
                    nextModel.entities = nextModel.entities.map((entity) => entity.id === operation.entityId
                        ? {
                            ...entity,
                    ...
                    const existingNotes = Array.isArray(nextModel.metadata.notes)
                        ? nextModel.metadata.notes.filter((note) => typeof note === 'string')
                        : [];
                    nextModel.metadata.notes = [...existingNotes, ...operation.notes];
                }
                break;
            case 'set_status':
                if (operation.target === 'workspace') {
                    nextModel.status = operation.status;
                }
                else if (operation.target === 'entity' && operation.targetId) {
                    nextModel.entities = nextModel.entities.map((entity) => entity.id === operation.targetId ? { ...entity, status: operation.status } : entity);
                }
                else if (operation.target === 'section' && operation.targetId) {
                    nextModel.sections = nextModel.sections.map((section) => section.id === operation.targetId
                        ? {
                            ...section,
                            status: operation.status === 'loading' || operation.status === 'error' ? operation.status : 'ready',
                            description: operation.message ?? section.description
                        }
                        : section);
                }
                else if (operation.target === 'tab' && operation.targetId) {
                    nextModel.tabs = nextModel.tabs.map((tab) => tab.id === operation.targetId
                        ? {
                            ...tab,
                            status: operation.status === 'loading' || operation.status === 'error' ? operation.status : 'ready'
                        }
                    ...
                    nextModel.state.actionState[operation.targetId] = {
                        status: operation.status === 'running' || operation.status === 'completed' || operation.status === 'error'
                            ? operation.status
                            : 'idle',
    ...
    return WorkspaceModelSchema.parse(nextModel);
}
export function synthesizeWorkspaceModelPatch(previousModel, nextModel) {
    const operations = [];
    const previous = previousModel;
    const previousEntities = new Map(previous?.entities.map((entity) => [entity.id, entity]) ?? []);
    const nextEntities = new Map(nextModel.entities.map((entity) => [entity.id, entity]));
    ...
    const removedCollectionIds = [...previousCollections.keys()].filter((collectionId) => !nextModel.collections.some((collection) => collection.id === collectionId));
    ...
    const previousViews = new Map(previous?.views.map((view) => [view.id, view]) ?? []);
    ...
    return WorkspacePatchSchema.parse({
        ...
        summary: operations.length > 0
            ? `Applied ${operations.length} workspace graph update${operations.length === 1 ? '' : 's'}.`
            : 'Workspace graph was unchanged.',
        operations
    });
}
export function buildWorkspaceModel(input) {
    ...
    const entities = dedupeById(datasets.flatMap((dataset) => dataset.items.map((item) => toWorkspaceEntity(dataset, item))));
    const collections = datasets.map((dataset) => toWorkspaceCollection(dataset, actions));
    const linksMarkdown = buildLinksMarkdown(dedupeLinks([...(input.summary?.links ?? []), ...(input.assistantContext?.links ?? []), ...(normalizedData?.links ?? [])]));
    ...
    const nextModel = WorkspaceModelSchema.parse({
        kind: 'workspace_model',
        schemaVersion: 'space_workspace_model/v1',
        sdkVersion: 'workspace_sdk/v1',
        revision: nextRevision,
        title: input.summary?.title ?? input.space.title,
        subtitle: input.summary?.subtitle,
        description: input.space.description ?? input.assistantContext?.summary,
    ...
    const patch = synthesizeWorkspaceModelPatch(input.previousModel ?? null, nextModel);
    const appliedModel = input.previousModel && patch.operations.length > 0 ? applyWorkspacePatchOperations(input.previousModel, patch) : nextModel;
    return {
        model: appliedModel,
        patch
    };
