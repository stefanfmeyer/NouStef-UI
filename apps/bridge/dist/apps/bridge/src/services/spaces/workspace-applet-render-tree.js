export function visitWorkspaceAppletNodes(nodes, visitor) {
    for (const node of nodes) {
        ...
        switch (node.kind) {
            ...
            case 'grid':
            case 'card':
                visitWorkspaceAppletNodes(node.children, visitor);
                break;
            case 'tabs':
                for (const tab of node.tabs) {
                    visitWorkspaceAppletNodes(tab.children, visitor);
                }
                break;
            case 'button_group':
                visitWorkspaceAppletNodes(node.children, visitor);
...
export function collectWorkspaceAppletNodeKinds(tree) {
    const kinds = [];
    visitWorkspaceAppletNodes(tree.root, (node) => {
        kinds.push(node.kind);
    });
    return kinds;
}
export function collectWorkspaceAppletActionIds(tree) {
...
export function validateWorkspaceAppletSmallPane(tree) {
    const errors = [];
    visitWorkspaceAppletNodes(tree.root, (node) => {
        switch (node.kind) {
            case 'grid':
