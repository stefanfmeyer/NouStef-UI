import { z } from 'zod';
import { SpaceAssistantContextSchema, SpaceIntentCategorySchema, SpaceIntentPresentationSchema, SpaceNormalizedDataSchema, SpaceRawDataSchema, SpaceStatusSchema } from '@hermes-workspaces/protocol';
import { extractRecoverableJsonObject } from './structured-json-recovery';
export const HermesSpaceTargetSchema = z
    .object({
    ...
    preferredPresentation: SpaceIntentPresentationSchema.optional()
})
    .strict();
export const HermesSpaceIntentHintsSchema = z
    .object({
    category: SpaceIntentCategorySchema.optional(),
    ...
    .strict();
export const HermesSpaceSemanticHintsSchema = z
    .object({
    ...
    .strict();
export const HermesSpaceActionHintsSchema = z
    .object({
    allowRefresh: z.boolean().optional(),
    suggestedActionLabels: z.array(z.string().min(1)).default([])
})
    .strict();
const HermesSpaceSeedBaseSchema = z
    .object({
    schemaVersion: z.enum(['hermes_space_seed/v1', 'hermes_space_data/v1']),
    space: HermesSpaceTargetSchema,
    rawData: SpaceRawDataSchema,
    assistantContext: SpaceAssistantContextSchema
})
    .passthrough();
const spaceDataFenceOpenPattern = /```hermes-space-data[^\n]*\n?/iu;
...
function parseHermesSpaceEnvelopeObject(rawValue) {
    const warnings = [];
    const diagnostics = [];
    const extensionDiagnostics = [];
    let baseEnvelope;
    try {
        baseEnvelope = HermesSpaceSeedBaseSchema.parse(rawValue);
    }
    catch (error) {
        return {
            envelope: null,
            ...
            schemaError: error instanceof Error ? error.message : 'Invalid Hermes space-data payload.'
        };
    }
    ...
    const parseOptional = (extension, value, schema, apply) => {
        ...
        diagnostics.push({
            code: 'SPACE_DATA_OPTIONAL_EXTENSION_DROPPED',
            severity: 'warning',
    ...
    parseOptional('intentHints', rawRecord.intentHints, HermesSpaceIntentHintsSchema, (parsed) => {
        envelope.intentHints = parsed;
    });
    parseOptional('semanticHints', rawRecord.semanticHints, HermesSpaceSemanticHintsSchema, (parsed) => {
        envelope.semanticHints = parsed;
    });
    parseOptional('actionHints', rawRecord.actionHints, HermesSpaceActionHintsSchema, (parsed) => {
        envelope.actionHints = parsed;
    });
    parseOptional('normalizedData', rawRecord.normalizedData ?? rawExtensions.normalizedData, SpaceNormalizedDataSchema, (parsed) => {
...
function parseHermesSpaceDataPayload(payloadText, options) {
    ...
    const addMissingFenceUnrecoverableDiagnostic = (detail) => {
        diagnostics.push({
            code: 'SPACE_DATA_BLOCK_MISSING_FENCE_UNRECOVERABLE',
            severity: 'warning',
            detail
        });
    };
    ...
    if (!trimmed) {
        return fail('SPACE_DATA_BLOCK_EMPTY', 'The Hermes space-data block was empty.', 'empty_payload');
    }
    const addMissingFenceRecoveryDiagnostic = () => {
        const detail = 'Recovered a Hermes space-data block that was missing the closing fence.';
        warnings.push(detail);
        diagnostics.push({
            code: 'SPACE_DATA_BLOCK_MISSING_FENCE_RECOVERED',
            severity: 'warning',
            detail
        });
    };
    const addIgnoredProseDiagnostic = () => {
        const detail = 'Ignored non-JSON prose inside the Hermes space-data block.';
        warnings.push(detail);
        diagnostics.push({
            code: 'SPACE_DATA_BLOCK_PROSE_IGNORED',
            severity: 'info',
            detail
        });
    };
        ...
        const detail = extractedJson.errors[0] ??
            (nextFailureKind === 'json_unbalanced'
                ? options.missingClosingFence
                    ? 'The Hermes space-data block did not close properly and no balanced JSON payload could be recovered.'
                    : 'The Hermes space-data block did not contain a balanced JSON payload.'
                : nextFailureKind === 'json_invalid'
                    ? 'The Hermes space-data block did not contain valid JSON.'
                    : 'The Hermes space-data block did not contain a JSON payload.');
        return fail(nextFailureKind === 'json_unbalanced'
            ? 'SPACE_DATA_BLOCK_JSON_UNBALANCED'
            : nextFailureKind === 'json_invalid'
                ? 'SPACE_DATA_BLOCK_JSON_INVALID'
                : nextFailureKind === 'schema_invalid'
                    ? 'SPACE_DATA_BLOCK_SCHEMA_INVALID'
                    : 'SPACE_DATA_BLOCK_JSON_MISSING', detail, nextFailureKind);
    }
    try {
        parserDiagnostics.schemaValidationAttempted = true;
        const parsedEnvelope = parseHermesSpaceEnvelopeObject(extractedJson.rawValue);
        if (!parsedEnvelope.envelope) {
            return fail('SPACE_DATA_BLOCK_SCHEMA_INVALID', parsedEnvelope.schemaError ?? 'Invalid Hermes space-data block.', 'schema_invalid');
        }
        ...
        if (extractedJson.diagnostics.recoveryStrategiesSucceeded.includes('complete_trailing_delimiters')) {
            ...
            warnings.push(detail);
            diagnostics.push({
                code: 'SPACE_DATA_BLOCK_JSON_DELIMITER_COMPLETED',
                severity: 'warning',
                detail
            });
        }
        ...
        if (extractedError instanceof SyntaxError) {
            return fail('SPACE_DATA_BLOCK_JSON_INVALID', extractedError.message, 'json_invalid');
        }
        const detail = extractedError instanceof Error ? extractedError.message : 'Invalid Hermes space-data block.';
        return fail('SPACE_DATA_BLOCK_SCHEMA_INVALID', detail, 'schema_invalid');
    }
}
export function extractHermesSpaceDataEnvelope(markdown, options = {}) {
    const mode = options.mode ?? 'compat';
    if (mode === 'structured_only') {
        const trimmed = markdown.trim();
        if (trimmed.startsWith('{') || trimmed.startsWith('[')) {
            const parsedPayload = parseHermesSpaceDataPayload(trimmed, {
        ...
        if (markerMatch && markerMatch.index !== undefined) {
            ...
            if (closingFenceIndex < payloadStart) {
                const parsedPayload = parseHermesSpaceDataPayload(markdown.slice(payloadStart), {
            ...
            const parsedPayload = parseHermesSpaceDataPayload(markdown.slice(payloadStart, closingFenceIndex), {
                missingClosingFence: false,
                mode
            });
            return buildExtractionResult(markdown, {
        ...
        if (!trimmed) {
            const parsedPayload = parseHermesSpaceDataPayload('', {
                missingClosingFence: false,
                mode: 'structured_only'
            });
        ...
        return buildExtractionResult(markdown, null, {
            ...
            errors: ['The structured-only response did not contain a JSON artifact or Hermes space-data start marker.'],
            diagnostics: [
                {
                    code: 'SPACE_DATA_BLOCK_PARSE_FAILED',
                    severity: 'warning',
                    detail: 'The structured-only response did not contain a JSON artifact or Hermes space-data start marker.'
                }
            ],
            parserDiagnostics: {
