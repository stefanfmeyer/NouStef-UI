...
function actionIdFromCall(expression, helper, errors, fileName) {
    if (helper === 'refreshSpace' && expression.arguments.length === 0) {
        return 'refresh-space';
    }
...
function analyzePatchWorkspaceCall(expression, fileName, errors, patchUsages, collectionIds, entityIds, tabIds, actionUsages) {
    const [operationsArgument] = expression.arguments;
    if (!operationsArgument || !ts.isArrayLiteralExpression(operationsArgument)) {
        errors.push(`patchWorkspace() in ${fileName} requires a literal array of patch operations.`);
        return;
    }
    for (const element of operationsArgument.elements) {
        if (!ts.isObjectLiteralExpression(element)) {
            errors.push(`patchWorkspace() in ${fileName} requires each patch operation to be an object literal.`);
            continue;
        }
        const operation = literalObjectPropertyValue(element, 'op');
        if (!operation) {
            errors.push(`patchWorkspace() in ${fileName} requires each patch operation to declare a literal op value.`);
            continue;
        }
            ...
            case 'update_collection':
                {
                    ...
                    if (!collectionInitializer || !ts.isObjectLiteralExpression(collectionInitializer)) {
                        errors.push(`patchWorkspace() update_collection in ${fileName} requires a literal collection object.`);
                        break;
                    }
                    const collectionId = literalObjectPropertyValue(collectionInitializer, 'id');
                    if (!collectionId) {
                        errors.push(`patchWorkspace() update_collection in ${fileName} requires a literal collection id.`);
                        break;
                    }
            ...
            case 'set_selection':
                {
                    ...
                    if (!collectionId) {
                        errors.push(`patchWorkspace() ${operation} in ${fileName} requires a literal collectionId.`);
                        break;
                    }
                    collectionIds.add(collectionId);
                }
            ...
            case 'update_tab':
                {
                    ...
                    if (!tabInitializer || !ts.isObjectLiteralExpression(tabInitializer)) {
                        errors.push(`patchWorkspace() ${operation} in ${fileName} requires a literal tab object.`);
                        break;
                    }
                    const tabId = literalObjectPropertyValue(tabInitializer, 'id');
                    if (!tabId) {
                        errors.push(`patchWorkspace() ${operation} in ${fileName} requires a literal tab id.`);
                        break;
                    }
                    tabIds.add(tabId);
                }
            ...
            case 'remove_tab':
                {
                    ...
                    if (!tabId) {
                        errors.push(`patchWorkspace() remove_tab in ${fileName} requires a literal tabId.`);
                        break;
                    }
                    tabIds.add(tabId);
                }
            ...
            case 'replace_view':
                {
                    ...
                    if (!viewInitializer || !ts.isObjectLiteralExpression(viewInitializer)) {
                        errors.push(`patchWorkspace() replace_view in ${fileName} requires a literal view object.`);
                    }
                }
                break;
            case 'replace_section':
                {
                    ...
                    if (!sectionInitializer || !ts.isObjectLiteralExpression(sectionInitializer)) {
                        errors.push(`patchWorkspace() replace_section in ${fileName} requires a literal section object.`);
                        break;
                    }
...
export function analyzeWorkspaceAppletModule(source, options) {
    const errors = [];
    const warnings = [];
    const imports = [];
    const sdkImports = [];
    ...
    const allowedImports = options.kind === 'source'
        ? new Set(['workspace-applet-sdk'])
        : new Set(['workspace-applet-sdk', 'workspace-applet-test-sdk']);
    ...
    const visit = (node) => {
        if (ts.isImportDeclaration(node)) {
            ...
            if (specifier === 'workspace-applet-sdk') {
                ...
                if (node.importClause?.namedBindings && ts.isNamedImports(node.importClause.namedBindings)) {
                    for (const element of node.importClause.namedBindings.elements) {
                        ...
                        if (!allowedSdkNamedExports.has(importedName)) {
                            errors.push(`Unsupported workspace SDK export "${importedName}" in ${options.fileName}.`);
                        }
        ...
        if (ts.isCallExpression(node)) {
            ...
            if (ts.isIdentifier(node.expression)) {
                ...
                if (sdkExport) {
                    ...
                    if (workspaceMutationExports.has(sdkExport)) {
                        analyzePatchWorkspaceCall(node, options.fileName, errors, patchUsages, collectionIds, entityIds, tabIds, actionUsages);
                    }
        ...
        if (options.kind === 'source' && (ts.isJsxOpeningElement(node) || ts.isJsxSelfClosingElement(node))) {
            ...
            if (ts.isIdentifier(node.tagName)) {
                ...
                if (sdkExport) {
                    ...
                    errors.push(`Intrinsic HTML element "${normalizedTagName}" is not allowed in workspace applets.`);
                }
            }
            else if (normalizedTagName.length > 0 && normalizedTagName[0] === normalizedTagName[0]?.toLowerCase()) {
                errors.push(`Intrinsic HTML element "${normalizedTagName}" is not allowed in workspace applets.`);
...
export function validateWorkspaceAppletModule(source, options) {
    return analyzeWorkspaceAppletModule(source, options);
}
