function createDiagnostics(trimmed) {
    return {
...
function appendTrailingClosers(stack) {
    return stack
        .slice()
        .reverse()
        .map((entry) => (entry === '{' ? '}' : ']'))
...
function extractJsonCandidate(text, diagnostics) {
...
export function extractRecoverableJsonObject(responseText, options = {}) {
    ...
    diagnostics.leadingProse = extracted.leadingText.length > 0;
    diagnostics.trailingProse = extracted.trailingText.length > 0;
    const changedPayload = extracted.status === 'completed' || diagnostics.leadingProse || diagnostics.trailingProse;
