...
const DEFAULT_TEMPLATE_ACTIONS = {
    'refresh-space': bridgeAction('refresh-space', 'Refresh workspace', 'refresh_space', {
        description: 'Regenerate the workspace from the latest session context.',
        intent: 'secondary',
        visibility: {
    ...
    'retry-build': bridgeAction('retry-build', 'Rebuild workspace', 'retry_build', {
...
export const WORKSPACE_TEMPLATE_RUNTIME_REGISTRY = {
    'price-comparison-grid': withDefaults({
        ...
        slots: [
            ...
            { id: 'quick-actions', kind: 'action-bar', required: true }
        ],
    ...
    'restaurant-finder': withDefaults({
        ...
        slots: [
            ...
            { id: 'quick-actions', kind: 'action-bar' }
        ],
...
export function getWorkspaceTemplateRuntimeDefinition(templateId) {
    return WORKSPACE_TEMPLATE_RUNTIME_REGISTRY[templateId] ?? null;
}
