import { WorkspaceDslSchema } from '@hermes-workspaces/protocol';
import { extractBalancedJsonValue } from './dynamic-space-contract';
export const WORKSPACE_DSL_TOP_LEVEL_KEYS = [
    'kind',
    'schemaVersion',
    'sdkVersion',
    'title',
...
export const WORKSPACE_DSL_REQUIRED_FIELDS = ['kind', 'schemaVersion', 'summary', 'tabs'];
export const WORKSPACE_DSL_ACTION_KINDS = ['existing_action'];
export const WORKSPACE_DSL_OPERATION_NAMES = [
    'update_dataset',
    'replace_section',
    'remove_section',
    'update_tab',
...
export const WORKSPACE_DSL_SECTION_TYPES = [
    'summary',
    'dataset',
    'comparison',
    'detail',
...
export const WORKSPACE_DSL_DATASET_DISPLAYS = [
    'table',
    'list',
    'cards',
    'detail',
...
function normalizeDslPayload(responseText) {
    const trimmed = responseText.trim();
    if (!trimmed) {
        return '';
    }
    const fenceMatch = trimmed.match(/^```(?:hermes-workspace-dsl|json)?\s*\n?/iu);
    if (fenceMatch) {
    ...
    return [
        {
            ...
            sections: [
                ...
                },
                {
                    ...
                    fields: [],
                    entityIds: [],
                    actionIds: ['refresh-space', 'retry-build'],
                    links: [],
                    media: [],
                    stats: [],
                    metadata: {}
...
function createInvalidExamples() {
    return [
        {
            reason: 'Wrong top-level shape. The DSL must use tabs and sections, not views plus strict workspace graph objects.',
            invalid: {
                kind: 'workspace_dsl',
                schemaVersion: 'space_workspace_dsl/v2',
...
export function createWorkspaceDslContractPacket(input) {
    return {
        summary: 'Emit a smaller authoring DSL for Hermes Home. Do not emit the final strict workspace model, raw entities/collections/views graph, or TSX applet source.',
        allowedTopLevelKeys: [...WORKSPACE_DSL_TOP_LEVEL_KEYS],
        requiredFields: [...WORKSPACE_DSL_REQUIRED_FIELDS],
        allowedActionKinds: [...WORKSPACE_DSL_ACTION_KINDS],
        allowedOperationNames: [...WORKSPACE_DSL_OPERATION_NAMES],
        allowedSectionTypes: [...WORKSPACE_DSL_SECTION_TYPES],
        allowedDatasetDisplays: [...WORKSPACE_DSL_DATASET_DISPLAYS],
        allowedReferences: {
        ...
        guidance: [
            ...
            'Sections stay semantic and compact; the bridge translates them into the strict workspace model.',
...
export function createWorkspaceDslRepairExcerpt(errors) {
    ...
    const include = {
        ...
        sections: normalizedErrors.some((error) => error.includes('section') || error.includes('body') || error.includes('field') || error.includes('type')),
        datasets: normalizedErrors.some((error) => error.includes('dataset') || error.includes('collection') || error.includes('entity')),
        ...
        topLevel: normalizedErrors.some((error) => error.includes('summary') || error.includes('schema') || error.includes('kind')) ||
            normalizedErrors.length === 0
    };
    return {
        summary: 'Repair the JSON so it matches the compact workspace authoring DSL. Do not introduce TSX, strict graph keys, or live-task instructions.',
        topLevel: include.topLevel || include.tabs || include.sections || include.datasets || include.actions || include.operations
            ? {
                allowedTopLevelKeys: [...WORKSPACE_DSL_TOP_LEVEL_KEYS],
                requiredFields: [...WORKSPACE_DSL_REQUIRED_FIELDS]
            }
        ...
        sections: include.sections
            ? {
                allowedTypes: [...WORKSPACE_DSL_SECTION_TYPES],
                shape: {
                    id: 'section-summary',
                    type: 'summary',
                    title: 'Summary',
        ...
        datasets: include.datasets
            ? {
                allowedDisplays: [...WORKSPACE_DSL_DATASET_DISPLAYS],
                shape: {
        ...
        actions: include.actions
            ? {
                allowedKinds: [...WORKSPACE_DSL_ACTION_KINDS],
                shape: {
                    kind: 'existing_action',
        ...
        operations: include.operations
            ? {
                allowedNames: [...WORKSPACE_DSL_OPERATION_NAMES],
                examples: [
                    {
                        op: 'set_active_tab',
                        tabId: 'tab-overview'
...
export function extractWorkspaceDslArtifact(responseText) {
    const warnings = [];
    const errors = [];
    ...
    if (!normalized) {
        return {
            ...
            jsonText: null,
            warnings,
            errors: ['Hermes did not emit a workspace DSL artifact.'],
            recovered: false
        };
    }
    ...
    if (!balancedJson) {
        return {
            ...
            jsonText: null,
            warnings,
            errors: ['Hermes did not emit a balanced workspace DSL JSON object.'],
            recovered: false
        };
    }
    const recovered = balancedJson.jsonText.trim() !== normalized.trim();
    if (recovered) {
        warnings.push('Recovered the first balanced workspace DSL JSON object from the Hermes response.');
    }
    let parsedJson;
    try {
        ...
        return {
            ...
            warnings,
            errors: [error instanceof Error ? error.message : 'Workspace DSL JSON parsing failed.'],
            recovered
        };
    }
    const parsedArtifact = WorkspaceDslSchema.safeParse(parsedJson);
