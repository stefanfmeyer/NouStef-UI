import { SpaceActionSpecSchema, SpaceAnalysisSchema, SpaceAnalysisDatasetSchema, SpaceFallbackStateSchema, SpaceIntentSchema, SpaceNormalizedDataSchema, SpaceRawDataSchema, SpaceSummarySchema, SpaceTestResultsSchema, SpaceTestSpecSchema, SpaceUiSpecSchema, SpaceUserPromptArtifactSchema } from '@hermes-workspaces/protocol';
...
function buildCollectionDataset(rawRecords, label, path, timestamp, context, parentDatasetId, parentFieldKey) {
    ...
    context.datasets.push(SpaceAnalysisDatasetSchema.parse(dataset));
...
export function analyzeSpaceData(rawData, assistantContext, options) {
    const context = {
        datasets: []
    };
    ...
    return SpaceAnalysisSchema.parse({
        kind: 'analysis',
        schemaVersion: 'space_analysis/v1',
...
export function deriveNormalizedDataFromAnalysis(analysis, options = {}) {
    ...
    return SpaceNormalizedDataSchema.parse({
...
export function deriveNormalizedDataFromRawData(rawData, options) {
    const analysis = analyzeSpaceData(rawData, {
...
function buildSpaceIntent(input, analysis, normalizedData) {
    const normalizedPrompt = normalizePrompt(input.prompt);
    return SpaceIntentSchema.parse({
        kind: 'intent',
        schemaVersion: 'space_intent/v1',
        category: input.envelope.intentHints?.category ?? input.intentHint?.category ?? 'general',
        label: input.envelope.intentHints?.label ?? input.intentHint?.label ?? input.envelope.space.title,
        ...
        updateTarget: input.currentSpace ? 'current_space' : 'new_space',
        metadata: {
...
function buildSummary(title, subtitle, analysis, assistantContext, timestamp, category) {
    return SpaceSummarySchema.parse({
        kind: 'summary',
        schemaVersion: 'space_summary/v1',
        title,
        ...
        note: 'Locally analyzed and compiled for the attached Hermes Home workspace.'
    });
}
function buildFallback(summary, assistantContext, normalizedData) {
    const primaryDataset = normalizedData.datasets.find((dataset) => dataset.id === normalizedData.primaryDatasetId) ?? normalizedData.datasets[0];
    return SpaceFallbackStateSchema.parse({
        kind: 'fallback',
        schemaVersion: 'space_fallback/v1',
        title: summary.title,
        message: 'The rich workspace is unavailable. Showing a safe summary preview instead.',
...
function buildActionSpec(userPrompt, intent, normalizedData, assistantContext) {
    ...
    if (intent.allowOutboundRequests) {
        actions.push({
            ...
            kind: 'prompt',
            intent: 'primary',
            description: 'Refresh this workspace using the original scope and the latest available data.',
            visibility: {
                requiresSelection: 'none',
                whenBuildReady: true
            },
            prompt: {
                promptTemplate: `Refresh the workspace for "${userPrompt.originalPrompt}" and replace stale items with the latest relevant results.`,
                includeInputs: ['original_prompt', 'intent', 'space_summary', 'raw_data', 'normalized_data', 'assistant_context', 'page_state', 'filter_state'],
    ...
    if ((primaryDataset?.items.length ?? 0) > 0) {
        actions.push({
            ...
            kind: 'prompt',
            intent: 'secondary',
            description: 'Focus the workspace around the selected item and update the attached results.',
            visibility: {
            ...
            prompt: {
                promptTemplate: `Refine the current workspace around the selected item from "${assistantContext.summary ?? userPrompt.originalPrompt}".`,
                includeInputs: ['original_prompt', 'intent', 'space_summary', 'raw_data', 'normalized_data', 'selected_items', 'assistant_context'],
    ...
    actions.push({
        ...
        kind: 'bridge',
        intent: 'secondary',
        description: 'Retry the latest richer workspace enrichment from persisted Home artifacts.',
        visibility: {
            requiresSelection: 'none',
...
function datasetActionIds(actionSpec) {
    return {
        refresh: actionSpec.actions.some((action) => action.id === 'refresh-space') ? ['refresh-space'] : [],
...
function buildDatasetNodes(dataset, actionSpec, options = {}) {
    ...
    if (dataset.itemCount === 0) {
        nodes.push({
            ...
            description: 'Try broadening the request or refreshing the workspace.'
        });
        return nodes;
    }
    ...
    const sharedLoadingState = {
        ...
        description: 'Compiling a compact workspace view.'
    };
...
function buildUiSpec(summary, analysis, actionSpec, assistantContext, options = {}) {
    ...
    const primaryActions = actionSpec.actions.some((action) => action.id === 'refresh-space') ? ['refresh-space'] : [];
    ...
    return SpaceUiSpecSchema.parse({
        kind: 'ui_spec',
        schemaVersion: 'space_ui/v2',
        compact: {
...
function buildTestSpec(uiSpec, actionSpec) {
    ...
    return SpaceTestSpecSchema.parse({
        kind: 'test_spec',
        schemaVersion: 'space_test_spec/v1',
        cases
    });
}
...
export function runSpaceTestHarness(input) {
    ...
    const results = input.testSpec.cases.map((testCase) => {
        ...
        switch (testCase.kind) {
            case 'schema_valid': {
                try {
                    SpaceNormalizedDataSchema.parse(input.normalizedData);
                    SpaceUiSpecSchema.parse(input.uiSpec);
                    SpaceActionSpecSchema.parse(input.actionSpec);
                    SpaceTestSpecSchema.parse(input.testSpec);
                    message = 'The dynamic space schemas validated successfully.';
                }
                catch (error) {
                    passed = false;
            ...
            case 'render_smoke': {
                if (input.uiSpec.schemaVersion === 'space_ui/v2') {
                    ...
                    message = passed ? 'The dynamic space contains a renderable header and node tree.' : 'The dynamic space is missing a required header or node.';
                }
                else {
                    ...
                    message = passed ? 'The dynamic space contains a renderable header and section set.' : 'The dynamic space is missing a required header or section.';
                }
                break;
            }
            case 'compact_layout': {
                ...
                message = passed
                    ? 'The dynamic space stays within the compact pane layout constraints.'
                    : 'The dynamic space exceeds the compact pane layout constraints.';
                break;
            }
    ...
    return SpaceTestResultsSchema.parse({
        kind: 'test_results',
        schemaVersion: 'space_test_results/v1',
...
export function buildWorkspaceAppletBaseArtifacts(input) {
    const userPrompt = SpaceUserPromptArtifactSchema.parse({
        kind: 'user_prompt',
        schemaVersion: 'space_user_prompt/v1',
        originalPrompt: input.prompt,
        normalizedPrompt: normalizePrompt(input.prompt),
        requestMode: input.requestMode
    });
    const rawData = SpaceRawDataSchema.parse(input.envelope.rawData);
    const assistantContext = input.envelope.assistantContext;
    const analysis = analyzeSpaceData(rawData, assistantContext, {
        datasetLabel: input.envelope.space.title,
    ...
    const intent = buildSpaceIntent(input, analysis, normalizedData);
    const summary = buildSummary(input.envelope.space.title || input.currentSpace?.title || 'Workspace', input.envelope.space.subtitle || input.envelope.space.description || input.currentSpace?.description, analysis, assistantContext, input.timestamp, intent.category);
    ...
    const workspaceModelArtifacts = buildWorkspaceModel({
        space: input.currentSpace ?? {
            id: 'pending-space',
            title: input.envelope.space.title || 'Workspace',
            description: input.envelope.space.description,
            status: input.envelope.space.status ?? 'active',
            metadata: {
                changeVersion: 1,
                auditTags: [],
                homeWorkspace: true
        ...
        actionSpec,
        fallback,
        previousModel: input.currentSpace?.dynamic?.workspaceModel ?? null
    });
    return {
        userPrompt,
        intent,
        ...
        fallback,
        actionSpec,
        workspaceModel: workspaceModelArtifacts.model,
        workspacePatch: workspaceModelArtifacts.patch
    };
}
export function buildDynamicSpaceArtifacts(input) {
    const baseArtifacts = buildWorkspaceAppletBaseArtifacts(input);
    const preferredPresentationHint = resolvePrimaryPresentationHint(input);
