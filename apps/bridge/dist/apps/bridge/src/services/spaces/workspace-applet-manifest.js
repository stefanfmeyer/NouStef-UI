import { SpaceAppletManifestSchema } from '@hermes-workspaces/protocol';
...
function resolveDatasetId(datasetId, normalizedData) {
    if (datasetId === 'primary') {
        return normalizedData?.primaryDatasetId ?? normalizedData?.datasets[0]?.id ?? null;
    }
    return datasetId;
}
function datasetExists(datasetId, normalizedData) {
...
function collectionExists(collectionId, workspaceModel, normalizedData) {
    if (workspaceModel?.collections.some((collection) => collection.id === collectionId)) {
        return true;
    }
    return datasetExists(collectionId, normalizedData);
}
function inferSmallPaneStrategy(analysis) {
    if (analysis.features.usesTabs) {
        return 'tabbed';
    }
    if (analysis.usedComponents.some((component) => ['Card', 'Callout', 'DetailPanel', 'Grid', 'List', 'Paginator', 'Table'].includes(component)) ||
        analysis.datasetIds.length > 1) {
        return 'sectioned';
    }
    return 'single_column';
}
function inferManifestSummary(input) {
    const source = input.summary?.subtitle ??
        input.assistantContext?.summary ??
        input.intent.summary ??
        `Interactive Home workspace applet for ${input.space.title}.`;
    return truncateText(source, 160);
}
function buildImageCapabilities(analysis) {
    if (!analysis.features.usesImages || analysis.imageUsages.length === 0) {
        return [];
    }
    const imageCapabilityIds = new Set();
    const capabilities = [];
    for (const imageUsage of analysis.imageUsages) {
        ...
        capabilities.push({
            ...
            description: imageUsage.kind === 'local'
                ? 'The workspace applet renders image nodes from local or bundled sources.'
                : 'The workspace applet renders image nodes that may require remote image loading.',
...
function buildCapabilityForAction(action) {
    ...
    return {
        ...
        metadata: {
            actionId: action.id,
            operation: typeof action.bridge.payload.operation === 'string' ? String(action.bridge.payload.operation) : undefined
        }
    };
}
export function synthesizeWorkspaceAppletManifest(input) {
    ...
    const actionMap = new Map((input.actionSpec?.actions ?? []).map((action) => [action.id, action]));
    const capabilityActionMap = new Map((input.actionSpec?.actions ?? [])
        .filter((action) => action.bridge?.capabilityId)
        .map((action) => [action.bridge?.capabilityId ?? '', action]));
    ...
    for (const collectionId of collectionIds) {
        if (!collectionExists(collectionId, input.workspaceModel, input.normalizedData)) {
    ...
    for (const usage of input.analysis.actionUsages) {
        const action = actionMap.get(usage.actionId) ?? (usage.helper === 'callApprovedApi' ? capabilityActionMap.get(usage.actionId) : undefined);
        if (!action) {
            errors.push(usage.helper === 'callApprovedApi'
    ...
    const requestedCapabilities = uniqueById([
        ...manifestActions.map((action) => buildCapabilityForAction(action)).filter((capability) => Boolean(capability)),
    ...
    const manifest = SpaceAppletManifestSchema.parse({
        kind: 'applet_manifest',
        schemaVersion: 'space_applet_manifest/v1',
        title: truncateText(`${input.space.title} applet`, 96),
        summary: inferManifestSummary(input),
        description: input.space.description?.trim() ||
            truncateText(`Locally synthesized from the persisted Home workspace artifacts for ${input.space.title}.`, 160),
