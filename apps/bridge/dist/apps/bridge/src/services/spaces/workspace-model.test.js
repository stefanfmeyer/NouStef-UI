...
import { applyWorkspacePatchOperations, buildWorkspaceModel } from './workspace-model';
        ...
        actions: [
            {
                ...
                prompt: {
                    ...
                    allowedMutations: ['workspace_model', 'workspace_patch'],
...
describe('workspace model synthesis', () => {
    it('builds a deterministic workspace graph with collections, tabs, sections, and actions', () => {
        ...
        expect(result.model.sdkVersion).toBe('workspace_sdk/v1');
        ...
        expect(result.model.actions.map((action) => action.id)).toEqual(['refresh-space', 'refine-selection']);
    ...
    it('produces patch-style updates instead of forcing a full rewrite', () => {
        const initial = buildWorkspaceModel({
            space: {
                id: 'space-inbox',
                title: 'Inbox triage',
                description: 'Unread messages',
        ...
        const updated = buildWorkspaceModel({
            space: {
                id: 'space-inbox',
                title: 'Inbox triage',
                description: 'Unread messages',
        ...
        const applied = applyWorkspacePatchOperations(initial.model, updated.patch);
