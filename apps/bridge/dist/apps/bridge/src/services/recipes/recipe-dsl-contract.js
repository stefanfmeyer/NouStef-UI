...
export const WORKRECIPE_DSL_REQUIRED_FIELDS = ['kind', 'schemaVersion', 'summary', 'tabs'];
export const WORKRECIPE_DSL_ACTION_KINDS = ['existing_action'];
...
export function createRecipeDslContractPacket(input) {
    return {
        summary: 'Emit a smaller authoring DSL for Hermes Home. Do not emit the final strict recipe model, raw entities/collections/views graph, or TSX applet source.',
...
export function createRecipeDslRepairExcerpt(errors) {
    ...
    return {
        summary: 'Repair the JSON so it matches the compact recipe authoring DSL. Do not introduce TSX, strict graph keys, or live-task instructions.',
...
export function extractRecipeDslArtifact(responseText) {
    const warnings = [];
    const errors = [];
