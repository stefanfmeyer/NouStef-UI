import { RecipeModelSchema, RecipePatchSchema } from '@hermes-recipes/protocol';
...
function toRecipeEntity(dataset, item) {
    return {
        id: item.id,
        kind: dataset.kind,
        title: item.title,
...
function toRecipeCollection(dataset, actions) {
    return {
        id: dataset.id,
        label: dataset.label,
...
export function applyRecipePatchOperations(model, patchOrOperations) {
...
export function synthesizeRecipeModelPatch(previousModel, nextModel) {
    const operations = [];
...
export function buildRecipeModel(input) {
    ...
    const appliedModel = input.previousModel && patch.operations.length > 0 ? applyRecipePatchOperations(input.previousModel, patch) : nextModel;
    return {
        model: appliedModel,
        patch
    };
