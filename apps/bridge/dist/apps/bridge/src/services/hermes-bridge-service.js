...
import { extractHermesRecipeDataEnvelope } from './recipes/dynamic-recipe-contract';
import { createRecipeDslContractPacket, createRecipeDslRepairExcerpt, extractRecipeDslArtifact } from './recipes/recipe-dsl-contract';
import { normalizeRecipeDsl } from './recipes/recipe-dsl-normalizer';
import { assembleRecipeTemplateFill, compileRecipeTemplatePreviewState, compileRecipeTemplateState, createRecipeTemplateGhostState, createRecipeTemplateActionSpec, createRecipeTemplateActionsPacket, createRecipeTemplateFillFromHydration, createRecipeTemplateFillFromText, createRecipeTemplateHydrationArtifact, createRecipeTemplateHydrationPacket, createRecipeTemplateSelectionPacket, createRecipeTemplateTextArtifact, createRecipeTemplateTextPacket, extractRecipeTemplateActionsArtifact, extractRecipeTemplateHydrationArtifact, extractRecipeTemplateSelectionArtifact, extractRecipeTemplateTextArtifact, findCardById, mapSections, migrateRecipeTemplateFill, normalizeRecipeTemplateActions, normalizeRecipeTemplateHydration, normalizeRecipeTemplateSelection, normalizeRecipeTemplateText, validateRecipeTemplateSelection } from './recipes/recipe-template-contract';
...
const HermesRecipeUpdateOperationSchema = z
    ...
    value.linkCurrentChat !== undefined, 'At least one update field must be provided.');
...
function normalizeRecipeMetadata(metadata, current) {
    return {
...
function createRecipePipelineSegment(status, updatedAt, options = {}) {
    return {
        status,
        updatedAt,
        ...options
...
function createInitialRecipePipeline(timestamp) {
    return {
...
function createRecipeAppletRetryPipeline(timestamp) {
    return {
...
function updateRecipePipelineSegment(pipeline, segment, currentStage, update) {
    return {
        ...pipeline,
        currentStage,
        [segment]: {
...
function isSyntheticRecipeRefreshMessage(content) {
    return content.trim() === RECIPE_REFRESH_USER_MESSAGE;
}
function withRefreshableRecipeMetadata(metadata, options) {
    const nextRefreshPrompt = options.requestMode !== 'recipe_refresh' && options.requestPreview && options.requestPreview.trim().length > 0
        ? options.requestPreview.trim()
        : undefined;
    return normalizeRecipeMetadata(nextRefreshPrompt
        ? {
            ...metadata,
        ...
        : metadata, options.current);
}
function withRecipeAttemptMetadata(metadata, options) {
...
function normalizeLegacyBlankContent(data) {
...
function normalizeRecipeTabsForInput(input, currentRecipe) {
    ...
    if (input.data !== undefined) {
        const inferredContentFormat = input.contentFormat ??
...
function inferRecipeContentFormatFromData(data) {
    if (!data) {
        return null;
    }
...
function parseRecipeOperationsJson(rawValue) {
...
function parseRecipeOperationsPayload(payloadText) {
    ...
    try {
        const parsedPayload = parseRecipeOperationsJson(JSON.parse(trimmed));
        return {
        ...
        try {
            const parsedPayload = parseRecipeOperationsJson(JSON.parse(extractedJson.jsonText));
            const warnings = [];
...
function stripTextRanges(markdown, ranges) {
    if (ranges.length === 0) {
...
function findRecipeFenceEnd(markdown, searchStart, maxEnd) {
...
function extractRecipeOperations(markdown) {
    ...
    for (let index = 0; index < openingMatches.length; index += 1) {
        ...
        const parsedFenceEnd = parsedPayload.extractedRange && parsedPayload.extractedRange.endIndex > parsedPayload.extractedRange.startIndex
            ? findRecipeFenceEnd(markdown, payloadStart + parsedPayload.extractedRange.endIndex, nextBlockStart)
            : -1;
        const blockEnd = parsedFenceEnd > payloadStart
            ? parsedFenceEnd
...
export class HermesBridge {
    ...
    recordTelemetryIfLatestDifferent(event, dedupeBy = {}) {
    ...
    formatTimeoutMs(timeoutMs) {
        return `${timeoutMs.toLocaleString()}ms`;
    }
    upsertRuntimeRequestRecipePipeline(requestId, pipeline) {
        return this.options.database.updateRuntimeRequestRecipePipeline(requestId, pipeline);
    }
    updateRecipeRecipePipeline(recipe, pipeline) {
        return (this.options.database.updateRecipe(recipe.id, {
    ...
    persistRecipePipeline(requestId, pipeline, recipe) {
    ...
    describeTaskTimeoutContext(prompt, requestMode, structuredRecipeIntent) {
    ...
    classifyTaskFailure(input) {
        ...
        if (/timed out/i.test(diagnostic)) {
            const category = configuredTimeoutMs < timeoutContext.recommendedTimeoutMs ? 'timeout_user_config' : 'timeout_runtime';
            const timeoutDetail = category === 'timeout_user_config'
    ...
    classifyBaselineFailure(detail) {
    ...
    classifyRecipeAppletVerificationFailure(verification) {
    ...
    classifyRecipeAppletFailure(input) {
        ...
        const configuredTimeoutMs = input.configuredTimeoutMsOverride ??
            (input.kind === 'seed'
        ...
        if (input.failureReason === 'live_task_violation') {
            return {
                ...
                userFacingMessage: 'The Home recipe baseline is ready, but the recipe applet generation attempted blocked live task work and was stopped. The applet path can only use persisted artifacts.',
                diagnostic,
                retryable: true,
        ...
        if (input.failureReason === 'context_invalid' || input.kind === 'context') {
            return {
                ...
                userFacingMessage: 'The Home recipe baseline is ready, but the recipe applet context is incomplete. Refresh the baseline artifacts before retrying the applet upgrade.',
                diagnostic,
                retryable: false,
                failureStage: 'applet_generating'
            };
        ...
        if (/timed out/i.test(diagnostic)) {
            ...
            const stageLabel = input.kind === 'seed'
    ...
    resolveRecipeAppletFailureEvent(failure, fallbackCode, fallbackMessage) {
    ...
    classifyRecipeDslFailure(input) {
        ...
        if (input.failureReason === 'live_task_violation') {
            return {
                ...
                userFacingMessage: 'The Home recipe baseline is ready, but recipe enrichment attempted blocked live task work and was stopped. Enrichment can only use persisted artifacts.',
                diagnostic,
                retryable: true,
        ...
        if (input.failureReason === 'context_invalid' || input.kind === 'context') {
            return {
                ...
                userFacingMessage: 'The Home recipe baseline is ready, but the enrichment context is incomplete. Refresh the baseline artifacts before retrying recipe enrichment.',
                diagnostic,
                retryable: false,
                failureStage: 'enrichment_generating'
            };
        ...
        if (input.kind === 'repair') {
            return {
                ...
                userFacingMessage: 'The Home recipe baseline is ready, but one bounded recipe DSL repair attempt still could not produce a valid enrichment artifact. You can retry recipe enrichment.',
                diagnostic,
                retryable: true,
        ...
        return {
            ...
            userFacingMessage: 'The Home recipe baseline is ready, but the bridge could not normalize the richer recipe DSL safely. You can retry recipe enrichment.',
            diagnostic,
            retryable: true,
            failureStage: 'enrichment_validating'
        };
    }
    resolveRecipeDslFailureEvent(failure, fallbackCode, fallbackMessage) {
    ...
    classifyRecipeTemplateFailure(input) {
        ...
        if (input.failureReason === 'live_task_violation') {
            return {
                ...
                userFacingMessage: 'Recipe generation failed because Hermes attempted live task work. Template generation may only use persisted artifacts.',
                diagnostic,
                retryable: true,
        ...
        if (/timed out/i.test(diagnostic)) {
            const category = configuredTimeoutMs < recommendedTimeoutMs ? 'timeout_user_config' : 'timeout_runtime';
            const stageLabel = input.kind === 'selection'
                ? 'template selection'
                : input.kind === 'text'
        ...
        if (input.kind === 'text_repair') {
            return {
                ...
                userFacingMessage: 'Recipe generation failed because the bounded text/content repair pass could not produce a valid staged artifact.',
                diagnostic,
                retryable: true,
                failureStage: 'enrichment_validating'
            };
        ...
        if (input.kind === 'actions_repair') {
            return {
                ...
                userFacingMessage: 'Recipe generation failed because the bounded actions/buttons repair pass could not produce a valid staged artifact.',
                diagnostic,
                retryable: true,
                failureStage: 'enrichment_validating'
            };
    ...
    normalizeRequestPreview(content) {
        const trimmed = content.trim();
        return trimmed.length > 0 ? trimmed : 'Hermes request';
    }
    resolveRecipeTemplateSelectionMode(selection, currentTemplate) {
        if (!currentTemplate) {
            return 'fill';
        }
    ...
    resolveUnexpectedQueuedTemplateFailureKind(phase) {
        switch (phase) {
    ...
    resolveUnexpectedQueuedTemplateFailureEvent(kind) {
        switch (kind) {
            case 'selection':
                return {
    ...
    serializeStructuredContext(value, maxLength = 4_000) {
    ...
    getSelectedRecipeItems(normalizedData, selectedItemIds) {
    ...
    buildRecipeActionTranscriptContent(action, selectedItems) {
    ...
    resolveRecipeActionArtifactBuildId(recipe, actionId) {
        const candidateIds = [
    ...
    resolveRecipeActionArtifactContext(recipe, actionId) {
        ...
        if (recipe.renderMode !== 'dynamic_v1' && action.kind !== 'bridge') {
            throw new BridgeError(409, 'RECIPE_ACTION_UNAVAILABLE', 'Prompt-bound actions are only available after richer recipe enrichment is ready.');
        }
        return {
    ...
    resolveRecipeAppletArtifactBuildId(recipe) {
        const candidateIds = [
            recipe.dynamic?.applet?.activeBuild?.id ?? null,
            recipe.dynamic?.applet?.promotedBuildId ?? null,
            recipe.dynamic?.activeBuild?.id ?? null,
    ...
    resolveRecipeAppletArtifactContext(recipe) {
        const artifactBuildId = this.resolveRecipeAppletArtifactBuildId(recipe);
        if (!artifactBuildId) {
            throw new BridgeError(409, 'RECIPE_DSL_CONTEXT_MISSING', 'The Home recipe is missing the persisted artifacts required for artifact-only recipe enrichment.');
        }
        ...
        if (!build ||
            ...
            throw new BridgeError(409, 'RECIPE_DSL_CONTEXT_MISSING', 'The Home recipe is missing one or more persisted enrichment artifacts.');
        }
        return {
            buildId: artifactBuildId,
            build,
    ...
    resolveRecipeTemplateArtifactContext(recipe) {
        const artifactBuildId = this.resolveRecipeAppletArtifactBuildId(recipe);
        if (!artifactBuildId) {
            throw new BridgeError(409, 'RECIPE_TEMPLATE_CONTEXT_MISSING', 'The Home recipe is missing the persisted artifacts required for template generation.');
        }
        const build = this.options.database.getRecipeBuild(artifactBuildId);
        const userPromptArtifact = this.options.database.getRecipeBuildArtifact(artifactBuildId, 'user_prompt');
        const intentArtifact = this.options.database.getRecipeBuildArtifact(artifactBuildId, 'intent');
        ...
        if (!build ||
            ...
            !analysisArtifact ||
            !normalizedDataArtifact) {
            throw new BridgeError(409, 'RECIPE_TEMPLATE_CONTEXT_MISSING', 'The Home recipe is missing one or more persisted template-generation artifacts.');
        }
        return {
            buildId: artifactBuildId,
            build,
    ...
    resolveTemplateRecipeActionArtifactContext(recipe, actionId) {
        const artifactBuildId = this.resolveRecipeActionArtifactBuildId(recipe, actionId);
        if (!artifactBuildId) {
    ...
    findFirstTemplateSection(sections, predicate) {
    ...
    persistTemplateRecipeActionUpdate(recipe, buildId, templateState, onEvent, sessionId, requestId, actionLabel) {
        ...
        if (build) {
            ...
            if (definition) {
                this.persistRecipeBuildArtifact(build, this.withRetryBuildActionSpec(createRecipeTemplateActionSpec(templateState, definition), true), this.now());
            }
        }
        const updatedRecipe = this.options.database.updateRecipe(recipe.id, {
            profileId: recipe.profileId,
        ...
        return this.emitRecipeEvent(this.createRecipeEvent(updatedRecipe, 'updated', `${actionLabel} updated "${updatedRecipe.title}".`, 'bridge', sessionId, {
            renderMode: 'dynamic_v1',
            templateId: templateState.templateId,
            requestId
        }), onEvent, this.options.database.getRecipe(updatedRecipe.id) ?? updatedRecipe);
    }
    cloneRecipeTemplateState(templateState) {
        return RecipeTemplateStateSchema.parse(JSON.parse(JSON.stringify(templateState)));
    }
    getTemplateSelectedItems(templateState, selectedItemIds) {
        const selectedItems = [];
    ...
    validateTemplateActionSelection(action, requestedSelectedItemIds, templateState) {
    ...
    buildTemplateActionTranscriptContent(action, selectedItems) {
        if (selectedItems.length === 1) {
    ...
    buildPromptBoundTemplateActionContent(recipe, context, input, selectedItems) {
    ...
    appendTemplateNoteLine(templateState, line) {
        const noteTarget = this.findFirstTemplateSection(templateState.sections, (section) => section.kind === 'notes') ?? null;
        if (!noteTarget || noteTarget.kind !== 'notes') {
            return templateState;
        }
        const nextState = this.cloneRecipeTemplateState(templateState);
        nextState.sections = mapSections(nextState.sections, (section) => section.slotId === noteTarget.slotId && section.kind === 'notes'
            ? {
                ...section,
    ...
    markTemplateSelection(templateState, selectedId, labels) {
        const nextState = this.cloneRecipeTemplateState(templateState);
    ...
    saveTemplatePlaceSelection(templateState, selectedItem) {
        ...
        return this.appendTemplateNoteLine(selectedState, `Saved place: ${selectedItem.title}${selectedItem.subtitle ? ` — ${selectedItem.subtitle}` : ''}`);
    }
    selectEventVenue(templateState, selectedItem) {
        const selectedState = this.markTemplateSelection(templateState, selectedItem.id, { selected: 'Selected venue' });
        return this.appendTemplateNoteLine(selectedState, `Selected venue: ${selectedItem.title}${selectedItem.subtitle ? ` — ${selectedItem.subtitle}` : ''}`);
    }
    addEventGuest(templateState, guestName) {
        const guestsSection = this.findFirstTemplateSection(templateState.sections, (section) => section.slotId === 'guests' && section.kind === 'grouped-list');
    ...
    toggleTemplateChecklistItem(templateState, itemId) {
        const checklistSection = this.findFirstTemplateSection(templateState.sections, (section) => section.slotId === 'checklist' && section.kind === 'grouped-list');
    ...
    moveTemplateCardStage(templateState, itemId, targetStageId) {
        ...
        nextState.sections = mapSections(nextState.sections, (section) => {
            ...
            const explicitTargetIndex = targetStageId && targetStageId.length > 0
                ? nextColumns.findIndex((column) => (column.id ?? column.label) === targetStageId)
                : -1;
            const destinationIndex = explicitTargetIndex >= 0
                ? explicitTargetIndex
    ...
    buildEventPlannerTemplateFillFromPlace(place) {
    ...
    async emitTemplateActionComplete(session, requestId, onEvent, content) {
    ...
    validateRecipeActionSelection(action, requestedSelectedItemIds, selectedItems) {
        if (requestedSelectedItemIds.length > 0 && selectedItems.length !== requestedSelectedItemIds.length) {
    ...
    buildPromptBoundRecipeActionContent(recipe, context, input, selectedItems) {
        ...
        appendSection('Normalized intent', includeInputs.has('intent')
            ...
            : null);
        appendSection('Recipe summary', includeInputs.has('recipe_summary')
            ? this.serializeStructuredContext({
            ...
            : null);
        appendSection('Raw data', includeInputs.has('raw_data')
            ? this.serializeStructuredContext({
            ...
            : null);
        appendSection('Normalized data', includeInputs.has('normalized_data')
            ? this.serializeStructuredContext({
            ...
            : null);
        appendSection('Selected items', includeInputs.has('selected_items') ? this.serializeStructuredContext(selectedItems, 3_500) : null);
        appendSection('Page state', includeInputs.has('page_state') && Object.keys(input.pageState).length > 0 ? this.serializeStructuredContext(input.pageState) : null);
        appendSection('Filter state', includeInputs.has('filter_state') && Object.keys(input.filterState).length > 0
            ? this.serializeStructuredContext(input.filterState)
            : null);
        appendSection('Form values', includeInputs.has('form_values') && Object.keys(input.formValues).length > 0 ? this.serializeStructuredContext(input.formValues) : null);
        appendSection('Assistant response context', includeInputs.has('assistant_context')
            ? this.serializeStructuredContext({
    ...
    resolveRecipeAppletManifest(recipe) {
        return recipe.dynamic?.applet?.manifest ?? null;
    }
    isRecipeActionBuildReady(recipe) {
        return recipe.dynamic?.activeBuild?.phase === 'ready';
    }
    resolveAsyncRecipeAppletTimeoutMs(_settings) {
    ...
    resolveRecipePipelineSnapshot(requestId, recipe, fallback) {
        return this.options.database.getRuntimeRequest(requestId)?.recipePipeline ?? recipe.metadata.recipePipeline ?? fallback;
    }
    assertExperimentalRecipeAppletRuntimeEnabled() {
        if (EXPERIMENTAL_WORKRECIPE_APPLET_RUNTIME_ENABLED) {
            return;
        }
        throw new BridgeError(409, 'RECIPE_EXPERIMENTAL_APPLET_RUNTIME_DISABLED', 'Runtime TSX recipe applets are disabled in production. Hermes Home enrichment uses the DSL-only renderer path.');
    }
    async queueRecipeAppletBuild(input) {
        ...
        const build = this.options.database.startRecipeBuild({
            ...
            triggerKind: input.triggerKindOverride ??
        ...
        await this.emitRecipePipelineStateEvent(recipeWithPipeline, input.session.id, input.onEvent, input.pipelineEventMessage, {
    ...
    enqueueRecipeAppletEnrichmentJob(input) {
        ...
        const nextQueue = currentQueue
            ...
            try {
                ...
                let buildPhase = null;
                let finalizedFailureCategory = null;
                try {
                    ...
                    if (recipe &&
                        ...
                        await this.finalizeRecipeAppletBuildFailure({
                            ...
                            build,
                            pipeline: this.resolveRecipePipelineSnapshot(input.requestId, recipe, input.kind === 'retry' ? createRecipeAppletRetryPipeline(this.now()) : createInitialRecipePipeline(this.now())),
                ...
                try {
                    this.recordTelemetry({
                        ...
                        severity: 'error',
                        category: 'recipes',
                        code: finalizedFailureCategory || buildPhase
                            ? 'RECIPE_TEMPLATE_ASYNC_JOB_FAILED'
                            : 'RECIPE_DSL_ASYNC_JOB_FAILED',
                        message: finalizedFailureCategory || buildPhase
    ...
    isRecipeEnrichmentSuperseded(recipeId, requestId) {
        ...
        const latestAttemptedAtMs = typeof currentRecipe.metadata.latestRecipeAttemptedAt === 'string'
            ? Date.parse(currentRecipe.metadata.latestRecipeAttemptedAt)
            : Number.NaN;
        const activeEnrichmentBuild = currentRecipe.dynamic?.activeBuild &&
    ...
    async markRecipeEnrichmentSuperseded(input) {
    ...
    async executeRecipeAppletCapabilityAction(profileId, sessionId, recipe, context, onEvent) {
        ...
        if (capability.kind === 'approved_api') {
            const operation = typeof capability.metadata.operation === 'string'
            ...
            throw new BridgeError(400, 'RECIPE_APPLET_CAPABILITY_UNSUPPORTED', `Approved applet capability ${capabilityId} requested unsupported operation ${operation || 'unknown'}.`);
        }
        throw new BridgeError(400, 'RECIPE_APPLET_CAPABILITY_UNSUPPORTED', `Applet capability ${capabilityId} with kind ${capability.kind} is not executable by the bridge.`);
    }
    async retryRecipeAppletUpgradeFromArtifacts(profileId, sessionId, recipe, triggerActionId, onEvent) {
        ...
        const useTemplateRetry = Boolean(recipe.dynamic?.recipeTemplate) ||
        ...
        await this.emitRecipePipelineStateEvent(activeRecipe, session.id, onEvent, useTemplateRetry
        ...
        try {
            ...
            const detail = error instanceof BridgeError && (error.code === 'RECIPE_DSL_CONTEXT_MISSING' || error.code === 'RECIPE_TEMPLATE_CONTEXT_MISSING')
                ? error.message
            ...
            const failureEvent = useTemplateRetry
                ...
                : this.resolveRecipeDslFailureEvent(failure, 'RECIPE_WORKRECIPE_DSL_CONTEXT_INVALID', 'Recipe enrichment could not start because the artifact-only context was invalid.');
            const queuedBuild = await this.queueRecipeAppletBuild({
                profile,
                session,
                currentRecipe: activeRecipe,
        ...
        const completionContent = queuedRecipeEnrichmentJob
        ...
        const completionMessage = this.options.database.appendMessage(ChatMessageSchema.parse({
            id: `message-${randomUUID()}`,
            sessionId: session.id,
            role: 'system',
        ...
        this.appendPersistedRuntimeActivity(profile.id, session.id, requestId, requestPreview, {
            ...
            detail: queuedRecipeEnrichmentJob
    ...
    ensurePersistedRuntimeRequest(profileId, sessionId, requestId, preview, startedAt) {
    ...
    appendPersistedRuntimeActivity(profileId, sessionId, requestId, preview, activity) {
        return this.options.database.appendRuntimeActivity(requestId, sessionId, profileId, activity, {
            preview
        });
    }
    finalizePersistedRuntimeRequest(profileId, sessionId, requestId, preview, status, timestamp, lastError) {
    ...
    backfillRuntimeRequestsFromMessages(profileId, sessionId, messages) {
    ...
    buildImportedRequestAliasMap(messages, runtimeRequests) {
        ...
        const normalizedPreview = (value) => this.normalizeRequestPreview(value);
        const usedTargetIds = new Set();
        const aliases = new Map();
        let targetCursor = 0;
        const findCandidateIndex = (preview) => {
    ...
    remapMessageRequestIds(messages, aliases) {
        let changed = false;
    ...
    normalizeSessionMessages(profileId, sessionId, syncedAt) {
        ...
        const missingRequestMessages = sessionMessages.filter((message) => message.requestId && !existingRuntimeRequestIds.has(message.requestId));
    ...
    candidateSkillSummaryFiles(profile, skill) {
    ...
    generateSkillSummary(profile, skill) {
    ...
    resolveActiveProfileId() {
        ...
        return ((uiState.activeProfileId && profiles.some((profile) => profile.id === uiState.activeProfileId) ? uiState.activeProfileId : null) ??
            profiles.find((profile) => profile.isActive)?.id ??
            profiles[0]?.id ??
            null);
    }
    resolveCachedBootstrap(detail) {
        const profiles = this.options.database.listProfiles();
        const uiState = this.options.database.getUiState();
        const settings = this.options.database.getSettings();
    ...
    async syncProfilesAndSessions(targetProfileId) {
        const timestamp = this.now();
        try {
            ...
            const profilesToSync = targetProfileId && syncedProfiles.some((profile) => profile.id === targetProfileId)
    ...
    ensureScopedSession(profileId, sessionId) {
    ...
    ensureRecipe(profileId, recipeId) {
    ...
    resolveRecipeTarget(profileId, currentRecipeId, operation) {
    ...
    createRecipeEvent(recipe, type, message, source, sessionId, metadata = {}) {
    ...
    buildRecipeRefreshContext(sessionId, recipe) {
    ...
    normalizeRecipeTabsForMutation(input, options) {
        try {
    ...
    flattenRecipeTemplateSections(sections) {
        const flattened = [];
        const visit = (items) => {
    ...
    persistRecipeTemplateProgressState(input) {
        const timestamp = this.now();
    ...
    async advanceRecipeBuild(build, requestPreview, onEvent, update) {
        const timestamp = this.now();
        const nextBuild = this.options.database.updateRecipeBuild(build.id, {
            phase: update.phase,
    ...
    createRecipeGenerationTrace(build, currentTemplateId) {
        return {
            buildId: build.id,
    ...
    beginRecipeGenerationStage(trace, input) {
        return {
            stage: input.stage,
            phase: input.phase,
    ...
    describeRecipeGenerationStageLabel(stage) {
        switch (stage) {
    ...
    appendStructuredRecipeBuildLog(build, phase, level, message, detail) {
    ...
    createBoundedRecipePayloadSnapshot(value, options = {}) {
    ...
    persistRecipeGenerationRawPayloadLog(input) {
        const snapshot = this.createBoundedRecipePayloadSnapshot(input.rawPayload);
        return this.appendStructuredRecipeBuildLog(input.build, input.phase, input.status === 'succeeded' ? 'info' : input.status === 'timed_out' ? 'warning' : 'error', `${this.describeRecipeGenerationStageLabel(input.stage)} raw payload snapshot.`, {
    ...
    persistRecipeGenerationAttemptLog(input) {
        return this.appendStructuredRecipeBuildLog(input.build, input.phase, input.status === 'succeeded' ? 'info' : input.status === 'timed_out' ? 'warning' : 'error', `${this.describeRecipeGenerationStageLabel(input.stage)} attempt ${input.attemptNumber} ${input.status}.`, {
    ...
    summarizeRecipeRetryReason(detail) {
    ...
    applyRecipeStageRetryPrompt(basePrompt, priorFailureDetail) {
...
- Do not regenerate unrelated content or invent new unsupported structure.`;
    ...
    mapRecipeStructuredFailureKind(failureKind, detail) {
    ...
    mapRecipeRequestFailureKind(detail) {
        return /timed out/i.test(detail) ? 'timeout' : 'request_error';
    }
    buildRecipeStageInvalidJson(input) {
    ...
    isRetryableRecipeStageAttempt(input) {
        if (input.failureReason === 'context_invalid') {
            return false;
        }
        if (input.failureReason === 'live_task_violation') {
    ...
    async runRecipeGenerationStageWithRetries(input) {
        const maxAttempts = input.maxAttempts ?? 3;
        let priorFailureDetail = null;
        for (let attemptNumber = 1; attemptNumber <= maxAttempts; attemptNumber += 1) {
            const attemptResult = await input.executeAttempt(attemptNumber, priorFailureDetail);
            const status = attemptResult.value !== null
                ? 'succeeded'
            ...
            const rawPayloadLogId = attemptResult.assistantMarkdown.trim().length > 0 &&
            ...
            const retryable = attemptResult.value === null
            ...
            this.appendStructuredRecipeBuildLog(input.build, input.phase, 'warning', `Retrying ${this.describeRecipeGenerationStageLabel(input.stage)} after attempt ${attemptNumber} failed.`, {
    ...
    completeRecipeGenerationStage(build, trace, run, input) {
        const endedAt = input.endedAt ?? this.now();
        const timeoutCategory = input.status === 'timed_out'
        ...
        const message = input.status === 'timed_out'
        ...
        this.appendStructuredRecipeBuildLog(build, run.phase, input.status === 'succeeded' ? 'info' : input.status === 'timed_out' ? 'warning' : 'error', message, observation);
        return observation;
    }
    findLatestRecipeGenerationTimeout(trace) {
    ...
    recordRecipeGenerationTimeoutTelemetry(build, trace, observation) {
        this.recordTelemetry({
            profileId: trace.profileId,
            sessionId: trace.sessionId,
            requestId: trace.requestId,
    ...
    recordRecipeGenerationSummary(build, trace) {
        const totalElapsedMs = this.calculateElapsedMs(build.startedAt, build.completedAt ?? this.now());
        const attemptsByStage = trace.stageObservations.reduce((summary, observation) => {
            summary[observation.stage] = (summary[observation.stage] ?? 0) + 1;
            return summary;
        }, {});
        const rawPayloadLogIdsByStage = trace.stageObservations.reduce((summary, observation) => {
        ...
        const recoveryByStage = trace.stageObservations.reduce((summary, observation) => {
        ...
        const perStageElapsedMs = trace.stageObservations.reduce((summary, observation) => {
        ...
        this.appendStructuredRecipeBuildLog(build, build.phase, build.phase === 'ready' ? 'info' : 'warning', 'Recipe generation summary.', summary);
        this.recordTelemetry({
            profileId: trace.profileId,
            sessionId: trace.sessionId,
            requestId: trace.requestId,
    ...
    recordRecipeDataEnvelopeTelemetry(profileId, sessionId, requestId, requestPreview, currentRecipeId, extracted, options = {}) {
        const payload = {
    ...
    normalizeRecipeArtifactRequestMode(mode) {
        if (mode === 'recipe_refresh') {
            return 'recipe_refresh';
        }
    ...
    buildRecipeAppletArtifactContext(input) {
    ...
    summarizeAssistantAnswer(markdown, fallback = 'Hermes completed the conversational answer.') {
    ...
    isRetryBuildAttempt(triggerKind, triggerActionId) {
        return triggerKind === 'retry' || triggerActionId === 'retry-build';
    }
    stripHermesRecipeDataMarkerRemnants(markdown) {
    ...
    sanitizeAssistantMarkdownForFallback(markdown) {
    ...
    deriveStructuredRecipeFailureTitle(currentRecipe, intent) {
    ...
    deriveMarkdownFallbackTitle(currentRecipe, session, intent) {
        if (currentRecipe?.title.trim()) {
            return currentRecipe.title.trim();
        }
        if (intent) {
    ...
    buildStructuredRecipeArtifactPrompt(input) {
...
${currentRecipeSummary}`;
    ...
    async emitStructuredRecipeArtifactProgress(profileId, sessionId, requestId, requestPreview, message, onEvent) {
    ...
    emitStructuredRecipeArtifactCompleted(profileId, sessionId, requestId, requestPreview, succeeded) {
        this.appendPersistedRuntimeActivity(profileId, sessionId, requestId, requestPreview, {
            kind: 'status',
    ...
    describeStructuredRecipeArtifactFailure(extracted, fallbackDetail) {
        if (!extracted) {
            return fallbackDetail;
        }
        return (extracted.errors.join('; ') ||
    ...
    getStructuredRecipeArtifactFailureTelemetry(extracted, fallbackDetail) {
    ...
    async requestStructuredRecipeArtifactAttempt(profile, session, currentRecipe, requestId, requestPreview, requestMode, prompt, refreshContext, settings, options) {
    ...
    async generateStructuredRecipeArtifact(build, generationTrace, profile, session, currentRecipe, requestId, requestPreview, requestMode, originalPrompt, assistantMarkdown, structuredRecipeIntent, refreshContext, settings, onEvent, options = {}) {
        await this.emitStructuredRecipeArtifactProgress(profile.id, session.id, requestId, requestPreview, options.isRetryBuild ? 'Retrying structured recipe generation…' : 'Generating structured recipe data…', onEvent);
        this.recordTelemetry({
            ...
            executeAttempt: async (attemptNumber, priorFailureDetail) => {
                if (attemptNumber > 1) {
                    this.recordTelemetry({
                        ...
                        detail: this.summarizeRecipeRetryReason(priorFailureDetail) ??
                    ...
                    await this.emitStructuredRecipeArtifactProgress(profile.id, session.id, requestId, requestPreview, `Retrying structured recipe data (${attemptNumber}/3)…`, onEvent);
                }
                const attempt = await this.requestStructuredRecipeArtifactAttempt(profile, session, currentRecipe, requestId, requestPreview, requestMode, this.buildStructuredRecipeArtifactPrompt({
                    originalPrompt,
                    assistantMarkdown,
                    intent: structuredRecipeIntent,
                    currentRecipe,
                    reissueFailureDetail: priorFailureDetail
                }), refreshContext, settings, {
                ...
                const failureTelemetry = this.getStructuredRecipeArtifactFailureTelemetry(attempt.extracted, attempt.errorDetail ?? 'The structured artifact generation call failed.');
                this.recordTelemetry({
                    profileId: profile.id,
                    sessionId: session.id,
                    requestId,
                ...
                return {
                    ...
                    recommendedTimeoutMs: attempt.recommendedTimeoutMs,
                    failureReason: null,
                    failureKind: this.mapRecipeStructuredFailureKind(attempt.extracted?.failureKind ?? null, failureTelemetry.detail),
                    retryable: true,
    ...
    createRecipeAppletRenderContext(recipe, context) {
        return {
            recipe: {
                id: recipe.id,
                title: recipe.title,
    ...
    persistRecipeAppletBaseArtifacts(build, context, timestamp) {
    ...
    persistRecipeTemplateBaseArtifacts(build, context, timestamp) {
        for (const artifact of [
            context.userPrompt,
            context.intent,
            context.rawData,
    ...
    buildRecipeTemplateAssistantContextPacket(assistantContext) {
        return {
    ...
    resolveRecipeTemplateAssistantMarkdown(context) {
        const fullResponseMarkdown = typeof context.assistantContext.metadata.fullResponseMarkdown === 'string'
    ...
    buildRecipeDslPrompt(input) {
        ...
        const contractPacket = createRecipeDslContractPacket({
            ...
            allowedDatasetIds: recipeModel?.collections.map((collection) => collection.id) ??
...
Allowed actions:
${this.serializeStructuredContext({
            actions: input.context.actionSpec?.actions.map((action) => ({
                id: action.id,
                label: action.label,
                kind: action.kind,
...
${this.serializeStructuredContext(input.context.assistantContext, 2_500)}`;
    ...
    buildRecipeDslRepairPrompt(input) {
...
${this.serializeStructuredContext(createRecipeDslRepairExcerpt(input.errors), 6_000)}`;
    ...
    async requestRecipeDslStage(profile, requestId, prompt, stage, settings) {
        ...
        try {
            ...
            return {
                ...
                failureReason: error instanceof Error && error.name === 'HermesCliRecipeAppletViolationError'
                    ? 'live_task_violation'
...
${this.serializeStructuredContext(input.context.currentTemplate, 6_000)}`;
    ...
    buildRecipeTemplateTextPrompt(input) {
        const assistantMarkdown = this.resolveRecipeTemplateAssistantMarkdown(input.context);
...
${this.serializeStructuredContext(input.context.currentTemplate, 6_000)}`;
    ...
    buildRecipeTemplateHydrationPrompt(input) {
        const assistantMarkdown = this.resolveRecipeTemplateAssistantMarkdown(input.context);
...
${this.serializeStructuredContext(input.context.currentTemplate, 8_000)}`;
    ...
    buildRecipeTemplateActionsPrompt(input) {
        const assistantMarkdown = this.resolveRecipeTemplateAssistantMarkdown(input.context);
...
${this.serializeStructuredContext(assistantMarkdown, 8_000)}`;
    ...
    buildRecipeTemplateTextRepairPrompt(input) {
...
${this.serializeStructuredContext(createRecipeTemplateTextPacket(input.templateId), 8_000)}`;
    ...
    buildRecipeTemplateActionsRepairPrompt(input) {
...
${this.serializeStructuredContext(createRecipeTemplateActionsPacket(input.hydratedTextArtifact), 8_000)}`;
    ...
    buildRecipeSameSessionCorrectionPrompt(input) {
        const stageLabel = input.stage === 'select'
            ? 'template selection'
            : input.stage === 'text'
                ? 'text/content'
                : input.stage === 'hydrate'
...
- The corrected JSON must pass the canonical schema validation exactly.`;
    ...
    describeRecipeTemplateStageTimeout(settings, stage) {
        void stage;
    ...
    async requestRecipeTemplateSameSessionCorrection(profile, requestId, correctionPrompt, stage, sessionId, settings) {
        const timeoutPolicy = this.describeRecipeTemplateStageTimeout(settings, stage);
        const startedAt = this.now();
        try {
    ...
    async attemptSameSessionCorrection(input) {
        ...
        for (let turn = 1; turn <= maxCorrectionTurns; turn += 1) {
            ...
            this.appendStructuredRecipeBuildLog(input.build, input.build.phase, 'info', `Same-session correction turn ${turn} for ${input.stage} stage.`, {
            ...
            const correctionResponse = await this.requestRecipeTemplateSameSessionCorrection(input.profile, input.requestId, correctionPrompt, input.stage, input.sessionId, input.settings);
            ...
            if (result.value !== null) {
                this.appendStructuredRecipeBuildLog(input.build, input.build.phase, 'info', `Same-session correction succeeded on turn ${turn} for ${input.stage} stage.`, {
        ...
        this.appendStructuredRecipeBuildLog(input.build, input.build.phase, 'warning', `Same-session correction exhausted ${maxCorrectionTurns} turns for ${input.stage} stage without fixing schema.`, {
            recordType: 'recipe_same_session_correction_result',
            buildId: input.build.id,
            stage: input.stage,
    ...
    async attemptRecipeTemplateTextRepair(input) {
        const repairResponse = await this.requestRecipeTemplateStage(input.profile, input.requestId, this.buildRecipeTemplateTextRepairPrompt({
            templateId: input.templateId,
            invalidJson: input.invalidJson,
            validationErrors: input.validationErrors
        }), 'text_repair', input.settings);
        const extractedRepair = extractRecipeTemplateTextArtifact(repairResponse.assistantMarkdown);
        const normalizedRepair = extractedRepair.rawValue !== null
        ...
        return {
            ...
            detail: (repairResponse.errorDetail ?? [...extractedRepair.errors, ...normalizedRepair.errors].join('; ')) ||
                'The bounded template text/content repair did not return valid JSON.'
        };
    }
    async attemptRecipeTemplateActionsRepair(input) {
        const repairResponse = await this.requestRecipeTemplateStage(input.profile, input.requestId, this.buildRecipeTemplateActionsRepairPrompt({
            templateId: input.templateId,
            invalidJson: input.invalidJson,
            validationErrors: input.validationErrors,
            hydratedTextArtifact: input.hydratedTextArtifact
        }), 'actions_repair', input.settings);
    ...
    synthesizeRecipeAppletArtifacts(input) {
    ...
    buildRecipeAppletSourcePrompt(input) {
        const recipeModel = input.context.recipeModel;
...
- Use only these action ids: ${input.context.actionSpec?.actions.map((action) => action.id).join(', ') || '(none)'}.
...
${this.serializeStructuredContext({
            ...
            revision: recipeModel?.revision ?? null,
            activeTabId: recipeModel?.state.activeTabId ?? null,
            tabs: recipeModel?.tabs.map((tab) => ({
                id: tab.id,
                label: tab.label,
                viewId: tab.viewId,
                status: tab.status
            })) ?? [],
            collections: recipeModel?.collections.map((collection) => ({
                id: collection.id,
            ...
            sections: recipeModel?.sections.map((section) => ({
                id: section.id,
                kind: section.kind,
                title: section.title,
...
${this.serializeStructuredContext({
    ...
    buildRecipeAppletRepairPrompt(input) {
...
Emit one corrected TSX module now.`;
    ...
    async emitRecipeAppletProgress(profileId, sessionId, requestId, requestPreview, message, onEvent) {
        this.appendPersistedRuntimeActivity(profileId, sessionId, requestId, requestPreview, {
            kind: 'status',
            state: 'updated',
            label: 'Recipe applet',
    ...
    async requestRecipeAppletStage(profile, requestId, prompt, stage, settings) {
        this.assertExperimentalRecipeAppletRuntimeEnabled();
        const timeoutMs = this.resolveAsyncRecipeAppletTimeoutMs(settings);
        try {
    ...
    async finalizeRecipeAppletBuildFailure(input) {
        ...
        await this.emitRecipePipelineStateEvent(failedRecipe, input.session.id, input.onEvent, input.pipelineEventMessage, {
            buildPhase: build.phase,
            failureCategory: input.failure.category,
            asyncEnrichment: true
        });
    ...
    createRetryBuildActionSpec(templateDriven) {
    ...
    computeAlternativeTemplates(selectedId, intentCategory) {
        ...
        for (const id of [...categoryIds, ...Object.keys(TEMPLATE_TO_LABEL)]) {
            if (id === selectedId || results.some(r => r.id === id))
                continue;
            const def = definitions.find(d => d.id === id);
            const label = TEMPLATE_TO_LABEL[id];
            if (def && label)
                results.push({ id: id, name: def.name, intentLabel: label });
            if (results.length >= 3)
                break;
        }
        return results;
    }
    withRetryBuildActionSpec(actionSpec, templateDriven) {
    ...
    async continueRecipeTemplateEnrichment(profile, session, currentRecipe, requestId, requestPreview, settings, onEvent, context, pipeline, build, generationTrace, mutationIntent) {
        ...
        const persistTemplatePreviewState = (stage, phase, fill, options = {}) => {
        ...
        const persistGhostTemplateState = (phase, options = {}) => {
        ...
        let latestTemplatePreviewState = persistGhostTemplateState('selected');
        ...
        let stagedTextArtifact = null;
        let hydrationArtifact = null;
        let hydratedTextArtifact = null;
        let actionsArtifact = null;
        let assembledFill = null;
        ...
        const textOutcome = await this.runRecipeGenerationStageWithRetries({
            ...
            currentTemplateId: currentTemplate?.templateId ?? null,
            executeAttempt: async (_attemptNumber, priorFailureDetail) => {
                const textResponse = await this.requestRecipeTemplateStage(profile, requestId, this.applyRecipeStageRetryPrompt(this.buildRecipeTemplateTextPrompt({
                    context,
                    currentRecipe,
                    selection
                ...
                let correctionResult = null;
                if (textResponse.runtimeSessionId &&
                ...
                const detail = (textResponse.errorDetail ?? [...latestTextFailure.validationErrors, ...(correctionResult?.correctionErrors ?? [])].join('; ')) ||
        ...
        if (!textOutcome.ok) {
            ...
            const textRepairOutcome = await this.runRecipeGenerationStageWithRetries({
                build,
                trace: generationTrace,
                ...
                currentTemplateId: currentTemplate?.templateId ?? null,
                executeAttempt: async (_attemptNumber, priorFailureDetail) => {
                    const repairResponse = await this.requestRecipeTemplateStage(profile, requestId, this.applyRecipeStageRetryPrompt(this.buildRecipeTemplateTextRepairPrompt({
                        templateId: selection.templateId,
                        invalidJson: latestTextRepairFailure.invalidJson,
                        validationErrors: latestTextRepairFailure.validationErrors
                    }), priorFailureDetail), 'text_repair', settings);
                    const extractedRepair = extractRecipeTemplateTextArtifact(repairResponse.assistantMarkdown);
                    const normalizedRepair = extractedRepair.rawValue !== null
                        ? normalizeRecipeTemplateText({
                            templateId: selection.templateId,
                    ...
                    const detail = (repairResponse.errorDetail ?? latestTextRepairFailure.validationErrors.join('; ')) ||
                        'The bounded template text/content repair did not return valid JSON.';
                    return {
                        value: null,
                        assistantMarkdown: repairResponse.assistantMarkdown,
            ...
            if (!textRepairOutcome.ok) {
                const failure = this.classifyRecipeTemplateFailure({
                    ...
                    settings,
                    configuredTimeoutMsOverride: textRepairOutcome.observation.configuredTimeoutMs ?? textRepairTimeoutPolicy.configuredTimeoutMs,
                    recommendedTimeoutMsOverride: textRepairOutcome.observation.recommendedTimeoutMs ?? textRepairTimeoutPolicy.recommendedTimeoutMs,
                ...
                return this.finalizeRecipeAppletBuildFailure({
                    ...
                    failedTemplateState: persistGhostTemplateState('failed', {
        ...
        const hydrationOutcome = await this.runRecipeGenerationStageWithRetries({
            ...
            currentTemplateId: currentTemplate?.templateId ?? null,
            executeAttempt: async (_attemptNumber, priorFailureDetail) => {
                const hydrationResponse = await this.requestRecipeTemplateStage(profile, requestId, this.applyRecipeStageRetryPrompt(this.buildRecipeTemplateHydrationPrompt({
                    context,
                    currentRecipe,
                    selection,
                    textArtifact: stagedTextArtifact
                }), priorFailureDetail), 'hydrate', settings);
                const extractedHydration = extractRecipeTemplateHydrationArtifact(hydrationResponse.assistantMarkdown);
                const normalizedHydration = extractedHydration.rawValue !== null
                ...
                let candidateFill = null;
                let candidateCompiled = null;
                ...
                const hydrationAllErrors = validationErrors.length > 0
                    ? validationErrors
                ...
                let correctionResult = null;
                if (hydrationResponse.runtimeSessionId &&
                ...
                const detail = (hydrationResponse.errorDetail ?? [...latestHydrationFailure.validationErrors, ...(correctionResult?.correctionErrors ?? [])].join('; ')) ||
        ...
        if (!hydrationOutcome.ok) {
            const failure = this.classifyRecipeTemplateFailure({
                ...
                recommendedTimeoutMsOverride: hydrationOutcome.observation.recommendedTimeoutMs ?? this.describeRecipeTemplateStageTimeout(settings, 'hydrate').recommendedTimeoutMs,
            ...
            return this.finalizeRecipeAppletBuildFailure({
                ...
                failedTemplateState: persistTemplatePreviewState('template_validation', 'failed', assembledFill ?? createRecipeTemplateFillFromText(stagedTextArtifact), {
                    failureCategory: failure.category,
        ...
        const actionsOutcome = await this.runRecipeGenerationStageWithRetries({
            ...
            currentTemplateId: currentTemplate?.templateId ?? null,
            executeAttempt: async (_attemptNumber, priorFailureDetail) => {
                const actionsResponse = await this.requestRecipeTemplateStage(profile, requestId, this.applyRecipeStageRetryPrompt(this.buildRecipeTemplateActionsPrompt({
                    context,
                    currentRecipe,
                    selection,
                    hydratedTextArtifact: hydratedTextArtifact
                }), priorFailureDetail), 'actions', settings);
                const extractedActions = extractRecipeTemplateActionsArtifact(actionsResponse.assistantMarkdown);
                const normalizedActions = extractedActions.rawValue !== null
                    ? normalizeRecipeTemplateActions({
                        templateId: selection.templateId,
                ...
                const actionsAllErrors = validationErrors.length > 0
                    ? validationErrors
                    : candidateCompiled?.errors.length
                        ? candidateCompiled.errors
                ...
                let correctionResult = null;
                if (actionsResponse.runtimeSessionId &&
                    ...
                    const invalidJson = this.buildRecipeStageInvalidJson({
                        ...
                        rawValue: extractedActions.rawValue,
                        fallbackJson: JSON.stringify(normalizedActions.actions ?? { kind: 'recipe_template_actions', templateId: selection.templateId, actions: [], links: [] }, null, 2)
                    });
                    const correction = await this.attemptSameSessionCorrection({
                        profile,
                        requestId,
                ...
                latestActionsFailure = {
                    invalidJson: this.buildRecipeStageInvalidJson({
                        ...
                        jsonText: extractedActions.jsonText,
                        rawValue: extractedActions.rawValue,
                        fallbackJson: JSON.stringify(normalizedActions.actions ?? {
                ...
                const detail = (actionsResponse.errorDetail ?? [...latestActionsFailure.validationErrors, ...(correctionResult?.correctionErrors ?? [])].join('; ')) ||
        ...
        if (!actionsOutcome.ok) {
            ...
            const actionsRepairOutcome = await this.runRecipeGenerationStageWithRetries({
                ...
                currentTemplateId: currentTemplate?.templateId ?? null,
                executeAttempt: async (_attemptNumber, priorFailureDetail) => {
                    const repairResponse = await this.requestRecipeTemplateStage(profile, requestId, this.applyRecipeStageRetryPrompt(this.buildRecipeTemplateActionsRepairPrompt({
                        templateId: selection.templateId,
                        invalidJson: latestActionsRepairFailure.invalidJson,
                        validationErrors: latestActionsRepairFailure.validationErrors,
                        hydratedTextArtifact: hydratedTextArtifact
                    }), priorFailureDetail), 'actions_repair', settings);
                    const extractedRepair = extractRecipeTemplateActionsArtifact(repairResponse.assistantMarkdown);
                    const normalizedRepair = extractedRepair.rawValue !== null
                        ? normalizeRecipeTemplateActions({
                            templateId: selection.templateId,
                    ...
                    latestActionsRepairFailure = {
                        invalidJson: this.buildRecipeStageInvalidJson({
                            ...
                            jsonText: extractedRepair.jsonText,
                            rawValue: extractedRepair.rawValue,
                            fallbackJson: JSON.stringify(normalizedRepair.actions ?? {
                                kind: 'recipe_template_actions',
                                templateId: selection.templateId,
                                actions: [],
                                links: []
                    ...
                    const detail = (repairResponse.errorDetail ?? latestActionsRepairFailure.validationErrors.join('; ')) ||
                        'The bounded template actions/buttons repair did not return valid JSON.';
                    return {
                        value: null,
                        assistantMarkdown: repairResponse.assistantMarkdown,
            ...
            if (!actionsRepairOutcome.ok) {
                const failure = this.classifyRecipeTemplateFailure({
                    ...
                    settings,
                    configuredTimeoutMsOverride: actionsRepairOutcome.observation.configuredTimeoutMs ?? actionsRepairTimeoutPolicy.configuredTimeoutMs,
                    recommendedTimeoutMsOverride: actionsRepairOutcome.observation.recommendedTimeoutMs ?? actionsRepairTimeoutPolicy.recommendedTimeoutMs,
                ...
                return this.finalizeRecipeAppletBuildFailure({
                    ...
                    failedTemplateState: persistTemplatePreviewState('template_validation', 'failed', assembledFill, {
                        failureCategory: failure.category,
        ...
        nextActionSpec = this.withRetryBuildActionSpec(createRecipeTemplateActionSpec(compiledTemplate.state, compiledTemplate.definition), true);
        if (!nextTemplateState || !nextActionSpec) {
            ...
            this.completeRecipeGenerationStage(build, generationTrace, this.beginRecipeGenerationStage(generationTrace, {
                stage: 'template_validation',
        ...
        const promotedRecipeWithTemplate = this.options.database.updateRecipe(currentRecipe.id, {
        ...
        const promotedRecipeWithPipeline = this.persistRecipePipeline(requestId, nextPipeline, promotedRecipeWithTemplate) ?? promotedRecipeWithTemplate;
        ...
        await this.emitRecipeEvent(this.createRecipeEvent(promotedRecipe, 'updated', normalizedMode === 'switch'
            ...
            : normalizedMode === 'update'
                ...
                : `Hermes generated the ${nextTemplateState.templateId} recipe template for "${promotedRecipe.title}".`, 'bridge', session.id, {
            renderMode: 'dynamic_v1',
            buildPhase: 'ready',
            recipePipelineStage: nextPipeline.currentStage,
            templateId: nextTemplateState.templateId,
            asyncEnrichment: true
        }), onEvent, promotedRecipe);
    ...
    async continueRecipeDslEnrichment(profile, session, currentRecipe, requestId, requestPreview, settings, onEvent, context, pipeline, build) {
        if (this.isRecipeEnrichmentSuperseded(currentRecipe.id, requestId)) {
            await this.markRecipeEnrichmentSuperseded({
                profile,
                session,
        ...
        const dslResponse = await this.requestRecipeDslStage(profile, requestId, this.buildRecipeDslPrompt({
            context,
            currentRecipe
        }), 'generate', settings);
        ...
        if (!context.recipeModel) {
            ...
            return this.finalizeRecipeAppletBuildFailure({
                ...
                onEvent,
                failure,
                failureEvent: this.resolveRecipeDslFailureEvent(failure, 'RECIPE_WORKRECIPE_DSL_CONTEXT_INVALID', 'The richer recipe DSL context was invalid.'),
                detail,
        ...
        let normalizedDsl = extractedDsl.rawValue ?? extractedDsl.artifact
            ? normalizeRecipeDsl({
        ...
        if ((!normalizedDsl.model || !normalizedDsl.patch) && (extractedDsl.jsonText || dslResponse.assistantMarkdown.trim().length > 0)) {
            ...
            this.recordTelemetry({
                ...
                detail: normalizedDsl.errors.join('; ') ||
            ...
            const repairedResponse = await this.requestRecipeDslStage(profile, requestId, this.buildRecipeDslRepairPrompt({
        ...
        if (!normalizedDsl.model || !normalizedDsl.patch) {
            ...
            return this.finalizeRecipeAppletBuildFailure({
                ...
                onEvent,
                failure,
                failureEvent: this.resolveRecipeDslFailureEvent(failure, repairAttempted ? 'RECIPE_WORKRECIPE_DSL_REPAIR_FAILED' : 'RECIPE_WORKRECIPE_DSL_NORMALIZATION_FAILED', repairAttempted
        ...
        this.persistRecipeBuildArtifact(build, RecipeActionSpecSchema.parse({
            kind: 'action_spec',
            schemaVersion: 'recipe_action_spec/v1',
        ...
        this.recordTelemetry({
            ...
            detail: normalizedDsl.warnings.join('; ') ||
        ...
        const promotedRecipe = this.options.database.getRecipe(promotedRecipeWithPipeline.id) ?? promotedRecipeWithPipeline;
        const promotedEvent = this.createRecipeEvent(promotedRecipe, 'updated', `Hermes upgraded "${promotedRecipe.title}" with a richer declarative recipe.`, 'bridge', session.id, {
            renderMode: 'dynamic_v1',
            buildPhase: 'ready',
            recipePipelineStage: nextPipeline.currentStage,
            asyncEnrichment: true
    ...
    async continueRecipeAppletUpgrade(profile, session, currentRecipe, requestId, requestPreview, settings, onEvent, context, pipeline, build) {
        this.assertExperimentalRecipeAppletRuntimeEnabled();
        if (this.isRecipeEnrichmentSuperseded(currentRecipe.id, requestId)) {
            await this.markRecipeEnrichmentSuperseded({
                profile,
        ...
        let sourceResponse = await this.requestRecipeAppletStage(profile, requestId, this.buildRecipeAppletSourcePrompt({
            context,
            currentRecipe
        }), 'source', settings);
        ...
        if (!extractedSource.artifact && !sourceResponse.failureReason && !sourceResponse.errorDetail) {
            ...
            this.recordTelemetry({
                ...
                detail: sourceResponse.errorDetail ??
            ...
            sourceResponse = await this.requestRecipeAppletStage(profile, requestId, this.buildRecipeAppletRepairPrompt({
                context,
                currentRecipe,
                sourceArtifact: {
        ...
        if (!extractedSource.artifact) {
            const detail = sourceResponse.errorDetail ??
            ...
            const failureEvent = this.resolveRecipeAppletFailureEvent(failure, 'RECIPE_APPLET_SOURCE_FAILED', 'The recipe applet source could not be generated.');
            return this.finalizeRecipeAppletBuildFailure({
                profile,
                session,
                currentRecipe,
        ...
        if (!synthesizedArtifacts.manifest || !synthesizedArtifacts.plan || !synthesizedArtifacts.testArtifact) {
            const detail = synthesizedArtifacts.errors.join('; ') ||
            ...
            const failureEvent = this.resolveRecipeAppletFailureEvent(failure, 'RECIPE_APPLET_MANIFEST_FAILED', 'The recipe applet manifest could not be synthesized locally.');
            return this.finalizeRecipeAppletBuildFailure({
                profile,
                session,
                currentRecipe,
        ...
        this.recordTelemetry({
            ...
            detail: synthesizedArtifacts.warnings.join('; ') ||
        ...
        currentRecipe = this.persistRecipePipeline(requestId, nextPipeline, currentRecipe) ?? currentRecipe;
        await this.emitRecipePipelineStateEvent(currentRecipe, session.id, onEvent, `Hermes is validating the optional recipe applet for "${currentRecipe.title}" in the background.`, {
            buildPhase: build.phase,
            asyncEnrichment: true
        });
                ...
                if (!synthesizedArtifacts.manifest || !synthesizedArtifacts.plan || !synthesizedArtifacts.testArtifact) {
                    verificationResult = {
                        ...
                        verification: {
                            ...
                            staticValidation: {
                                status: 'failed',
                                message: synthesizedArtifacts.errors.join('; ') ||
        ...
        const promotedRecipeWithPipeline = this.persistRecipePipeline(requestId, nextPipeline, currentRecipe) ?? currentRecipe;
        const promotedRecipe = this.options.database.getRecipe(promotedRecipeWithPipeline.id) ?? promotedRecipeWithPipeline;
        const promotedEvent = this.createRecipeEvent(promotedRecipe, 'updated', `Hermes upgraded "${promotedRecipe.title}" with a verified recipe applet.`, 'bridge', session.id, {
            renderMode: 'dynamic_v1',
            buildPhase: 'ready',
            recipePipelineStage: nextPipeline.currentStage,
            asyncEnrichment: true
    ...
    async runQueuedRecipeAppletEnrichmentFromRequest(input) {
        ...
        const structuredArtifact = await this.generateStructuredRecipeArtifact(nextBuild, generationTrace, input.profile, input.session, currentRecipe, input.requestId, input.requestPreview, input.requestMode, input.structuredIntentPrompt, input.conversationalAssistantMarkdown, input.structuredRecipeIntent, input.refreshContext, input.settings, onEvent);
        if (!structuredArtifact.extracted?.envelope) {
            const failure = this.classifyRecipeTemplateFailure({
                kind: 'selection',
                detail: structuredArtifact.errorDetail ??
        ...
        await this.emitRecipePipelineStateEvent(updatedRecipe, input.session.id, onEvent, `Hermes prepared the template-generation artifacts for "${updatedRecipe.title}" from persisted seed data.`, {
    ...
    async runQueuedRecipeAppletEnrichmentFromArtifacts(input, onEvent = async () => undefined) {
        ...
        try {
            ...
            const detail = error instanceof BridgeError && error.code === 'RECIPE_TEMPLATE_CONTEXT_MISSING'
                ? error.message
                : error instanceof Error
                    ? error.message
        ...
        const updatedRecipe = this.persistRecipePipeline(input.requestId, pipeline, currentRecipe) ?? currentRecipe;
        await this.emitRecipePipelineStateEvent(updatedRecipe, input.session.id, onEvent, `Hermes resumed background recipe generation for "${updatedRecipe.title}" from persisted Home artifacts.`, {
            buildPhase: nextBuild.phase,
            asyncEnrichment: true
        });
    ...
    async recordFailedRecipeAppletSeedBuild(profile, session, currentRecipe, requestId, requestPreview, requestMode, pipeline, settings, onEvent, options) {
        this.assertExperimentalRecipeAppletRuntimeEnabled();
        const failure = this.classifyRecipeAppletFailure({
            kind: 'seed',
        ...
        let build = this.options.database.startRecipeBuild({
            ...
            buildKind: 'applet',
            triggerKind: options.triggerKindOverride ??
        ...
        const failedRecipe = this.options.database.getRecipe(failedRecipeWithPipeline.id) ?? failedRecipeWithPipeline;
        await this.emitRecipePipelineStateEvent(failedRecipe, session.id, onEvent, `Hermes kept the baseline Home recipe for "${failedRecipe.title}" because the applet seed could not be generated.`, {
            buildPhase: build.phase,
            failureCategory: failure.category
        });
        return {
    ...
    async attemptRecipeAppletUpgrade(profile, session, currentRecipe, requestId, requestPreview, requestMode, settings, onEvent, context, pipeline, options = {}) {
        this.assertExperimentalRecipeAppletRuntimeEnabled();
        let nextPipeline = updateRecipePipelineSegment(pipeline, 'applet', 'applet_generating', {
            status: 'running',
            stage: 'applet_generating',
        ...
        if (!extractedSource.artifact) {
            ...
            const failedRecipe = this.options.database.getRecipe(failedRecipeWithPipeline.id) ?? failedRecipeWithPipeline;
            await this.emitRecipePipelineStateEvent(failedRecipe, session.id, onEvent, `Hermes kept the baseline Home recipe because the applet source for "${failedRecipe.title}" could not be generated.`, {
                buildPhase: build.phase,
                failureCategory: failure.category
            });
            return {
        ...
        if (!synthesizedArtifacts.manifest || !synthesizedArtifacts.plan || !synthesizedArtifacts.testArtifact) {
            ...
            const failedRecipe = this.options.database.getRecipe(failedRecipeWithPipeline.id) ?? failedRecipeWithPipeline;
            await this.emitRecipePipelineStateEvent(failedRecipe, session.id, onEvent, `Hermes kept the baseline Home recipe because the applet manifest for "${failedRecipe.title}" could not be synthesized locally.`, {
                buildPhase: build.phase,
                failureCategory: failure.category
            });
            return {
        ...
        currentRecipe = this.persistRecipePipeline(requestId, nextPipeline, currentRecipe) ?? currentRecipe;
        await this.emitRecipePipelineStateEvent(currentRecipe, session.id, onEvent, `Hermes is validating the optional recipe applet for "${currentRecipe.title}".`, {
            buildPhase: build.phase
        });
        let verificationResult = await verifyRecipeApplet({
            manifest,
        ...
        if (verificationResult.verification.status !== 'passed' || !verificationResult.renderTree) {
            ...
            const failedRecipe = this.options.database.getRecipe(failedRecipeWithPipeline.id) ?? failedRecipeWithPipeline;
            await this.emitRecipePipelineStateEvent(failedRecipe, session.id, onEvent, `Hermes kept the baseline Home recipe because the applet for "${failedRecipe.title}" failed verification.`, {
                buildPhase: 'failed',
                failureCategory: failure.category
            });
            return {
    ...
    async createOrUpdateHomeBaselineRecipe(profileId, session, currentRecipeId, requestId, requestPreview, requestMode, assistantMarkdown, structuredRecipeIntent, recipePipeline, onEvent) {
        const currentRecipe = (currentRecipeId ? this.options.database.getRecipe(currentRecipeId) : null) ??
        ...
        const markdownContent = safeAssistantMarkdown.trim() ||
        ...
        const metadata = withRecipeAttemptMetadata(withRefreshableRecipeMetadata({
            ...(currentRecipe?.metadata ?? {}),
            homeRecipe: true,
        ...
        const event = this.createRecipeEvent(targetRecipe, created ? 'created' : 'updated', created
            ? `Hermes created the Home recipe "${title}" from the latest answer.`
            : `Hermes updated the Home recipe "${title}" from the latest answer.`, 'bridge', session.id, {
            contentFormat: 'markdown',
            renderMode: 'legacy_content_v1',
    ...
    ensureStructuredRecipeRequestCompleted(profileId, sessionId, requestId, requestPreview, intent, explicitRecipeId, resolvedRecipeId, createdRecipeIds = []) {
        void profileId;
        void sessionId;
        void requestId;
        void requestPreview;
    ...
    async emitRecipePipelineStateEvent(recipe, sessionId, onEvent, message, metadata = {}) {
    ...
    async applyRecipeOperations(profileId, session, currentRecipeId, requestId, requestPreview, requestMode, assistantMarkdown, onEvent) {
        ...
        for (const operation of extracted.operations) {
            await onEvent({
                type: 'progress',
                requestId,
                message: operation.type === 'delete_space'
            ...
            if (operation.type === 'create_space') {
                ...
                const event = this.createRecipeEvent(recipe, 'created', `Hermes created the recipe "${recipe.title}".`, 'hermes', session.id, {
                    contentFormat
                });
            ...
            if (operation.type === 'update_space') {
                ...
                const event = this.createRecipeEvent(nextRecipe, renamed ? 'renamed' : 'updated', renamed
                    ? `Hermes renamed the recipe "${currentRecipe.title}" to "${nextRecipe.title}".`
                    : `Hermes updated the recipe "${nextRecipe.title}".`, 'hermes', session.id, {
                    contentFormat,
            ...
            if (operation.type === 'mark_space_changed') {
                ...
                const event = this.createRecipeEvent(nextRecipe, 'changed', operation.summary
                    ? `Hermes marked "${nextRecipe.title}" as changed: ${operation.summary}`
                    : `Hermes marked "${nextRecipe.title}" as changed.`, 'hermes', session.id, {
            ...
            const event = this.createRecipeEvent(currentRecipe, 'deleted', `Hermes deleted the recipe "${currentRecipe.title}".`, 'hermes', session.id);
            this.appendPersistedRuntimeActivity(profileId, session.id, requestId, requestPreview, {
                kind: 'status',
                state: 'completed',
                label: 'Recipe event',
        ...
        const assistantMarkdownWithFallback = unrecoveredParseErrors.length > 0 && summaries.length === 0
    ...
    createJobsFreshness(profileId, status, lastError, usingCache = false) {
    ...
    describeModelTestError(raw, model) {
    ...
    async listSessions(profileId, page, pageSize, search) {
    ...
    async getSessionMessages(profileId, sessionId) {
        const profile = await this.ensureProfile(profileId);
        const session = this.ensureScopedSession(profile.id, sessionId);
        ...
        const shouldRefreshFromHermes = Boolean(session.runtimeSessionId) &&
    ...
    async exportSession(profileId, sessionId) {
    ...
    async getJobs(profileId) {
        ...
        try {
            ...
            const freshness = this.createJobsFreshness(profile.id, cachedJobs.items.length > 0 ? 'error' : 'disconnected', detail, cachedJobs.items.length > 0);
            this.recordTelemetry({
                profileId: profile.id,
                sessionId: null,
                requestId: null,
    ...
    async getSkills(profileId) {
        const profile = await this.ensureProfile(profileId);
        try {
            const cachedSkillSummaries = new Map(this.options.database.listSkills(profile.id).map((skill) => [skill.id, skill.summary]));
    ...
    async getRecipe(profileId, recipeId) {
    ...
    async updateRecipe(recipeId, input) {
        ...
        this.createRecipeEvent(updatedRecipe, renamed ? 'renamed' : 'updated', renamed ? `Renamed "${currentRecipe.title}" to "${updatedRecipe.title}".` : `Updated the recipe "${updatedRecipe.title}".`, 'user', updatedRecipe.primarySessionId, {
    ...
    async applyRecipeEntryAction(recipeId, input) {
    ...
    async deleteRecipe(recipeId, input) {
    ...
    async openRecipeChat(recipeId, input) {
        const profile = await this.ensureProfile(input.profileId);
    ...
    persistRuntimeProviderState(profileId, runtimeState) {
    ...
    buildModelProviderResponseFromRuntimeState(runtimeState, connection) {
        return ModelProviderResponseSchema.parse({
            ...
            inspectedProviderId: runtimeState.inspectedProviderId ?? runtimeState.config.provider ?? runtimeState.providers[0]?.id ?? null,
    ...
    async updateRuntimeModelConfig(input) {
    ...
    async connectProvider(input) {
        ...
        try {
            ...
            const detail = error instanceof HermesCliStructuredActionError
                ? error.detail ?? error.message
                : error instanceof Error
                    ? error.message
                    : 'Failed to connect the provider.';
            const userMessage = error instanceof HermesCliStructuredActionError ? error.message : 'Hermes could not connect the provider. Check the credential and try again.';
            this.recordTelemetry({
                profileId: profile.id,
                sessionId: null,
                requestId: null,
    ...
    async beginProviderAuth(input) {
        ...
        try {
            ...
            const userMessage = error instanceof HermesCliStructuredActionError
                ? error.message
    ...
    async listToolHistory(page, pageSize, profileId) {
        await this.ensureProfile(profileId);
    ...
    async updateUiState(input) {
        ...
        return this.options.database.updateUiState(parsedInput);
    }
    async prepareToolExecution(input) {
    ...
    async resolveToolExecution(executionId, input) {
        ...
        const result = await runReviewedToolExecution({
            ...
            args: execution.args,
            cwd: execution.cwd
        }, this.recipeRoot, execution.requestedAt);
        return this.options.database.updateToolExecution(executionId, {
    ...
    async streamHermesRequest(input, onEvent) {
        const profile = await this.ensureProfile(input.profileId);
        ...
        try {
            ...
            this.options.database.refreshSessionTitleFromConversation(refreshedSession.id, transcriptContent, conversationalAssistantMarkdown);
            ...
            try {
                ...
                if (activeRecipe &&
                    ...
                    const shouldAutoRetry = !isRetryBuild &&
                        activeRecipe != null &&
            ...
            activeRecipe = this.persistRecipePipeline(requestId, recipePipeline, activeRecipe) ?? activeRecipe;
            if (activeRecipe) {
                await this.emitRecipePipelineStateEvent(activeRecipe, session.id, onEvent, taskFailure.userFacingMessage, {
                    failureCategory: taskFailure.category
                });
            }
            this.appendPersistedRuntimeActivity(profile.id, session.id, requestId, requestPreview, {
            ...
            const systemMessage = this.options.database.appendMessage(ChatMessageSchema.parse({
                id: `message-${randomUUID()}`,
                sessionId: session.id,
                role: 'system',
    ...
    async streamRecipeAction(recipeId, input, onEvent) {
        ...
        if (recipe.primarySessionId !== parsedInput.sessionId) {
            throw new BridgeError(400, 'RECIPE_ACTION_SESSION_MISMATCH', `Recipe ${recipe.id} is attached to session ${recipe.primarySessionId}, not ${parsedInput.sessionId}.`);
        }
        if (recipe.dynamic?.recipeTemplate) {
            ...
            if (context.action.kind !== 'bridge') {
                throw new BridgeError(400, 'RECIPE_ACTION_HANDLER_UNSUPPORTED', `Template action ${context.action.label} with kind ${context.action.kind} is not executable by the bridge.`);
            }
            ...
            switch (handler) {
                ...
                case 'add_event_guest': {
                    const requestedGuestName = typeof parsedInput.formValues.guestName === 'string'
                ...
                case 'append_template_note': {
                    const requestedNote = typeof parsedInput.formValues.note === 'string'
                        ...
                        : typeof parsedInput.formValues.content === 'string'
                            ...
                            : typeof parsedInput.formValues.value === 'string'
                                ...
                                : '';
                    const noteLine = requestedNote || (selectedItems[0] ? `Follow up on ${selectedItems[0].title}.` : `Recipe note added at ${new Date().toLocaleTimeString()}.`);
                ...
                case 'move_template_card_stage': {
                    ...
                    const targetStageId = typeof parsedInput.formValues.targetStageId === 'string' ? parsedInput.formValues.targetStageId.trim() : null;
                ...
                case 'switch_template': {
                    const targetTemplateId = typeof context.action.bridge?.payload?.targetTemplateId === 'string'
                        ? context.action.bridge.payload.targetTemplateId
                        : null;
                    const selectedItem = selectedItems[0];
                    ...
                    if (context.templateState.templateId === 'local-discovery-comparison' && targetTemplateId === 'event-planner') {
                        ...
                        if (!compiled.state || !compiled.definition || compiled.errors.length > 0) {
                            throw new BridgeError(409, 'RECIPE_TEMPLATE_SWITCH_FAILED', compiled.errors.join('; ') || `The ${targetTemplateId} template transition could not be completed safely.`);
                        }
                        nextTemplateState = compiled.state;
                        completionMessage = `Switched the recipe into ${compiled.definition.name}.`;
                        break;
                    }
                    throw new BridgeError(409, 'RECIPE_TEMPLATE_SWITCH_UNSUPPORTED', `Template transition ${context.templateState.templateId} -> ${targetTemplateId} is not supported.`);
                }
                ...
                default:
                    throw new BridgeError(400, 'RECIPE_ACTION_HANDLER_UNSUPPORTED', `Bridge action handler ${handler ?? 'unknown'} is not supported for template recipes.`);
            }
        ...
        if (context.action.kind === 'bridge') {
            ...
            if (handler === 'applet_capability') {
                throw new BridgeError(409, 'RECIPE_EXPERIMENTAL_APPLET_RUNTIME_DISABLED', 'Runtime TSX recipe applets are disabled in production. Retry or refresh uses the DSL-only enrichment path.');
            }
