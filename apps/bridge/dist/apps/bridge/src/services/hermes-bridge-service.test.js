...
import { getRecipeContentEntries, getRecipeContentTab, normalizeRecipeTabs, RECIPE_REFRESH_USER_MESSAGE, RecipeTemplateFillSchema } from '@hermes-recipes/protocol';
import { BridgeDatabase } from '../database';
import { buildRecipeAppletBaseArtifacts } from './recipes/home-recipe-compiler';
import { compileRecipeTemplateState, createRecipeTemplateActionSpec, createRecipeTemplateActionsArtifact, createRecipeTemplateHydrationArtifact, createRecipeTemplateTextArtifact } from './recipes/recipe-template-contract';
...
async function waitForRecipeEnrichment(bridge) {
...
function seedRuntimeReady(database) {
...
function createAttachedRecipe(database, tabs = normalizeRecipeTabs({
    contentFormat: 'table',
    contentData: {
        columns: [
...
function parseStructuredLogDetail(detail) {
    if (!detail) {
        return null;
    }
    try {
...
function listStructuredBuildLogDetails(database, buildId) {
    if (!buildId) {
        return [];
    }
    return database
        ...
        .filter((detail) => Boolean(detail?.recordType));
}
...
function extractPromptSection(prompt, label, nextLabels = []) {
...
function parsePromptSectionJson(prompt, label, nextLabels = []) {
...
function createRecipeTemplateSelectionArtifact(prompt, overrides = {}) {
    const currentTemplate = parsePromptSectionJson(prompt, 'Current template state');
    const templateId = overrides.templateId ?? inferTemplateIdFromPrompt(prompt);
    const currentTemplateId = typeof overrides.currentTemplateId === 'string'
    ...
    return JSON.stringify({
        ...
        confidence: 0.86,
        hints: typeof currentTemplateId === 'string'
            ? { currentTemplateId }
            : undefined
    });
}
function resolveTemplateIdForStagePrompt(prompt) {
...
function selectTemplateFillArtifact(prompt) {
...
function createRecipeTemplateTextArtifactResponse(prompt) {
...
function createRecipeTemplateActionsArtifactResponse(prompt) {
...
function seedFailedBaselineAppletRecipe(database, session, recipe) {
...
describe('HermesBridge recipe entry actions', () => {
    it('removes a single entry from a recipe and keeps the remaining representations aligned', async () => {
        ...
        expect(database
            ...
            .some((event) => event.code === 'RECIPE_ENTRY_REMOVED')).toBe(true);
        database.close();
    });
    it('rejects delete_source entry actions — source deletion must go through a prompt-based template action', async () => {
        const database = createDatabase();
        const { recipe } = createAttachedRecipe(database, normalizeRecipeTabs({
            contentFormat: 'table',
            contentData: {
                columns: [
        ...
        await expect(bridge.applyRecipeEntryAction(recipe.id, {
            profileId: 'jbarton',
...
describe('HermesBridge attached-recipe refresh', () => {
    it('builds refresh prompts from stored recipe context instead of replaying the latest user turn', async () => {
        ...
        const streamChat = vi.fn(async (_options) => ({
...
describe('HermesBridge enrichment supersession (Phase 4)', () => {
    it('stops enrichment between stages when a newer request supersedes the current one', async () => {
        ...
        const streamChat = vi.fn(async (options) => {
        ...
        const generateRecipeTemplateArtifact = vi.fn(async (options) => {
        ...
        await bridge.streamChat({ profileId: 'jbarton', sessionId: session.id, content: 'Find nearby coffee shops.', mode: 'chat' }, () => undefined);
        await waitForRecipeEnrichment(bridge);
        expect(selectionCallCount).toBe(1);
        const postSelectionCalls = generateRecipeTemplateArtifact.mock.calls.filter((call) => call[0]?.stage === 'text' || call[0]?.stage === 'hydrate' || call[0]?.stage === 'actions');
...
describe('HermesBridge Home baseline recipes', () => {
    it('creates a baseline Home recipe, selects an approved template, and promotes a template-filled recipe for structured requests', async () => {
        ...
        const generateRecipeTemplateArtifact = vi.fn(async (options) => {
            ...
            throw new Error(`Unexpected recipe template stage ${options?.stage ?? 'unknown'}.`);
        });
        const events = [];
        const bridge = new HermesBridge({
            database,
        ...
        const queuedCompleteEvent = events.find((event) => event.type === 'complete');
        ...
        const stageTimingLogs = buildLogs
            .map((log) => parseStructuredLogDetail(log.detail))
            .filter((detail) => detail?.recordType === 'recipe_generation_stage_timing');
        const sectionProgressLogs = buildLogs
            .map((log) => parseStructuredLogDetail(log.detail))
            .filter((detail) => detail?.recordType === 'recipe_template_section_progress');
        const stageAttemptLogs = buildLogs
            .map((log) => parseStructuredLogDetail(log.detail))
            .filter((detail) => detail?.recordType === 'recipe_generation_stage_attempt');
        ...
        expect(generateRecipeTemplateArtifact.mock.calls[0]?.[0]?.prompt).not.toContain('Find the best boutique hotels in Dayton for a weekend stay and create a recipe for this.');
        ...
        expect(stageTimingLogs.map((detail) => detail.stage)).toEqual(expect.arrayContaining([
        ...
        expect(sectionProgressLogs.map((detail) => detail.stage)).toEqual(expect.arrayContaining([
            'template_selection',
            'template_text_generation',
            'template_hydration',
            'template_validation'
    ...
    it('completes the request before async recipe generation finishes selecting and filling the template', async () => {
        ...
        const envelope = createDynamicRecipeDataEnvelope();
        const events = [];
        let releaseTemplateSelection;
        const templateSelectionGate = new Promise((resolve) => {
    ...
    it('skips the LLM selection stage when the intent label deterministically maps to a template', async () => {
        ...
        const generateRecipeTemplateArtifact = vi.fn(async (options) => {
            ...
            throw new Error(`Unexpected recipe template stage ${options?.stage ?? 'unknown'} — select stage should have been skipped.`);
        });
        const bridge = new HermesBridge({
            database,
            hermesCli: {
        ...
        expect(generateRecipeTemplateArtifact).not.toHaveBeenCalledWith(expect.objectContaining({ stage: 'select' }));
        ...
        expect(deterministicEvent?.payload?.intentLabel).toBe('hotel shortlist');
    ...
    it('retries structured seed generation, persists raw payload diagnostics, and recovers on a later attempt', async () => {
        ...
        const seedAttemptLogs = structuredLogs.filter((detail) => detail.recordType === 'recipe_generation_stage_attempt' && detail.stage === 'structured_seed_generation');
        const seedRawLogs = structuredLogs.filter((detail) => detail.recordType === 'recipe_generation_stage_attempt_raw' && detail.stage === 'structured_seed_generation');
    ...
    it('retries template selection three times, persists raw failed output, and then fails with attempt history', async () => {
        ...
        const activeBuild = attachedRecipe?.dynamic?.activeBuild ?? null;
        const structuredLogs = listStructuredBuildLogDetails(database, activeBuild?.id);
        const selectionAttemptLogs = structuredLogs.filter((detail) => detail.recordType === 'recipe_generation_stage_attempt' && detail.stage === 'template_selection');
        const selectionRawLogs = structuredLogs.filter((detail) => detail.recordType === 'recipe_generation_stage_attempt_raw' && detail.stage === 'template_selection');
        const generationSummaryLog = structuredLogs.find((detail) => detail.recordType === 'recipe_generation_summary');
    ...
    it('uses same-session correction when template text generation returns invalid schema and the session id is available', async () => {
        ...
        const generateRecipeTemplateArtifact = vi.fn(async (options) => {
            if (options?.stage === 'select') {
                return { assistantMarkdown: createRecipeTemplateSelectionArtifact(options.prompt), runtimeSessionId: 'session-select-1' };
            }
            if (options?.stage === 'text') {
                ...
                return { assistantMarkdown: createRecipeTemplateTextArtifactResponse(options.prompt), runtimeSessionId: 'session-text-2' };
            }
        ...
        await bridge.streamChat({ profileId: 'jbarton', sessionId: session.id, content: 'Find hotels in Chicago and organize them.', mode: 'chat' }, () => undefined);
        await waitForRecipeEnrichment(bridge);
        const attachedRecipe = database.getRecipeByPrimarySessionId('jbarton', session.id);
        const activeBuild = attachedRecipe?.dynamic?.activeBuild ?? null;
        const structuredLogs = listStructuredBuildLogDetails(database, activeBuild?.id);
        const correctionLogs = structuredLogs.filter((detail) => detail.recordType === 'recipe_same_session_correction' || detail.recordType === 'recipe_same_session_correction_result');
        const attemptLogs = structuredLogs.filter((detail) => detail.recordType === 'recipe_generation_stage_attempt' && detail.stage === 'template_text_generation');
...
${JSON.stringify(envelope)}`,
        ...
        expect(getRecipeContentTab(attachedRecipe).content.markdownRepresentation.markdown).toBe('Done.\n\nAdded direct + FWD rules for Plaid effects to SaaS AI.');
...
- Point two`);
    ...
    it('keeps the baseline Home recipe visible when template fill stays invalid after one bounded repair pass', async () => {
        ...
        expect(telemetry.some((event) => event.code === 'RECIPE_TEMPLATE_TEXT_REPAIR_FAILED' && event.payload.failureCategory === 'template_text_repair_failed')).toBe(true);
        expect(generationSummaryLog?.attemptsByStage).toMatchObject({
    ...
    it('persists timeout diagnostics when the bounded template repair stage hits the temporary 90s cap', async () => {
        ...
        const structuredLogs = buildLogs
            .map((log) => parseStructuredLogDetail(log.detail))
            .filter((detail) => Boolean(detail?.recordType));
        const repairTimingLog = structuredLogs.find((detail) => detail.recordType === 'recipe_generation_stage_timing' && detail.stage === 'template_text_repair');
        const generationSummaryLog = structuredLogs.find((detail) => detail.recordType === 'recipe_generation_summary');
        const telemetry = database.listTelemetryEvents({
            profileId: 'jbarton',
            sessionId: session.id,
    ...
    it('attributes empty required-array repair failures to the text stage with specific validation detail', async () => {
        ...
        expect(rawAttemptLogs.some((detail) => detail.stage === 'template_text_generation' &&
            String(detail.errorDetail ?? '').includes('Array must contain at least 1 element(s)'))).toBe(true);
        expect(generationSummaryLog?.attemptsByStage).toMatchObject({
            template_selection: 1,
            template_text_generation: 3,
            template_text_repair: 3
        });
        expect(telemetry.some((event) => event.code === 'RECIPE_TEMPLATE_TEXT_REPAIR_FAILED' &&
            String(event.detail ?? '').includes('Array must contain at least 1 element(s)'))).toBe(true);
        database.close();
    });
    ...
    it('fails cleanly at the actions stage when Hermes returns an actions artifact for the wrong template', async () => {
        ...
        const wrongActionsArtifact = JSON.stringify(createRecipeTemplateActionsArtifact(RecipeTemplateFillSchema.parse(createHotelShortlistTemplateFillArtifact())));
        const generateRecipeTemplateArtifact = vi.fn(async (options) => {
            if (options?.stage === 'select' && typeof options?.prompt === 'string') {
                return {
                    assistantMarkdown: createRecipeTemplateSelectionArtifact(options.prompt, {
        ...
        expect(attachedRecipe?.renderMode).toBe('dynamic_v1');
        expect(getRecipeContentTab(attachedRecipe).content.markdownRepresentation.markdown).toBe('Here is the restaurant shortlist baseline.');
        expect(activeBuild?.phase).toBe('failed');
    ...
    it('updates an existing approved template when selection stays on the same template', async () => {
        ...
        await bridge.retryRecipeAppletUpgradeFromArtifacts('jbarton', session.id, database.getRecipe(recipe.id), 'retry-build', () => undefined);
        await waitForRecipeEnrichment(bridge);
    ...
    it('blocks live-task violations during template generation while keeping the baseline Home recipe active', async () => {
        ...
        const attachedRecipe = database.getRecipeByPrimarySessionId('jbarton', session.id);
        const runtimeRequest = database.listRuntimeRequests(session.id)[0];
        const enrichmentBuild = attachedRecipe?.id != null
            ? database
    ...
    it('retries template generation from persisted artifacts without rerunning live task work', async () => {
        ...
        expect(templateStages.every((stage) => ['select', 'text', 'hydrate', 'actions', 'text_repair', 'actions_repair'].includes(String(stage)))).toBe(true);
    ...
    it('keeps the assistant answer visible when the baseline Home recipe update fails after task success', async () => {
        ...
        vi.spyOn(database, 'createRecipe').mockReturnValueOnce(null);
        const bridge = new HermesBridge({
            database,
            hermesCli: {
                discoverRuntimeProviderState: vi.fn(async () => createRuntimeReadyState()),
        ...
        expect(messages.some((message) => message.role === 'system' &&
            /baseline could not be updated/i.test(message.content) &&
            /assistant answer is still available/i.test(message.content))).toBe(true);
        expect(runtimeRequest?.status).toBe('completed');
    ...
    it('rejects prompt-bound actions for baseline Home recipes', async () => {
        ...
        await expect(bridge.streamRecipeAction(recipe.id, {
            profileId: 'jbarton',
            sessionId: session.id,
    ...
    it('switches a template recipe from local discovery into event planning through the public action stream', async () => {
        ...
        await bridge.streamRecipeAction(recipe.id, {
            profileId: 'jbarton',
            sessionId: session.id,
    ...
    it('emits the complete event before any recipe_event signalling baseline completion', async () => {
        ...
        const bridge = new HermesBridge({
            ...
            hermesCli: {
                discoverRuntimeProviderState: vi.fn(async () => createRuntimeReadyState()),
                streamChat: vi.fn(async (options) => {
                    if (options?.structuredArtifactOnly) {
                        return {
                            assistantMarkdown: JSON.stringify(envelope),
                ...
                generateRecipeTemplateArtifact: vi.fn(async (options) => {
                    if (options?.stage === 'select' && typeof options?.prompt === 'string') {
                        return { assistantMarkdown: createRecipeTemplateSelectionArtifact(options.prompt) };
                    }
                    if (options?.stage === 'text' && typeof options?.prompt === 'string') {
