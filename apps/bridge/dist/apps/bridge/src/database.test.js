...
import { createDefaultRecipeTabs, getRecipeContentEntries, getRecipeContentFormat, getRecipeContentTab, getRecipeContentViewData, isRecipeContentSynchronized, normalizeRecipeTabs } from '@hermes-recipes/protocol';
import { BridgeDatabase } from './database';
const cleanupPaths = [];
function createDynamicRecipeBuildFixtures(recipeId, sessionId) {
    const userPrompt = {
        kind: 'user_prompt',
...
FileNotFoundError: [Errno 2] No such file or directory: '/Users/example/.hermes/profiles/jbarton/scripts/check_mail.py'`,
        ...
        expect(appendedMessages[1]?.content).not.toMatch(/Bridge execution note|Raw system message|python scripts\/check_mail\.py|FileNotFoundError|google-workspace/u);
        database.replaceSessionMessages(session.id, [
            {
...
No such file or directory: /Users/example/.hermes/profiles/jbarton/scripts/check_mail.py`,
        ...
        expect(persistedMessages[1]?.content).not.toMatch(/Bridge execution note|Raw system message|python scripts\/check_mail\.py|FileNotFoundError|No such file or directory|google-workspace|hermes-ui-recipes|"operations"/u);
        database.close();
    });
    ...
    it('generates deterministic local session titles from the first user message for append and replace paths', () => {
        ...
        database.replaceSessionMessages(replacedSession.id, [
            {
    ...
    it('stores synchronized content representations and keeps them stable when only the active view changes', () => {
        ...
        expect(getRecipeContentViewData(getRecipeContentTab(switched), 'table').rows).toEqual(getRecipeContentViewData(getRecipeContentTab(recipe), 'table').rows);
        expect(getRecipeContentViewData(getRecipeContentTab(switched), 'card').cards).toEqual(getRecipeContentViewData(getRecipeContentTab(recipe), 'card').cards);
        database.close();
    });
    it('deleting an attached recipe preserves the session and clears the attachment', () => {
        ...
        expect(database.getSession(session.id)?.attachedRecipeId).toBeNull();
        expect(database.listSessions({ profileId: 'jbarton', page: 1, pageSize: 10, search: '' }).items.some((item) => item.id === session.id)).toBe(true);
        database.close();
    });
    ...
    it('merges a CLI-synced runtime session into the active local session when the runtime id collides', () => {
        ...
        database.replaceSessionMessages('runtime-session-009', [
            {
    ...
    it('persists runtime requests, finalizes unresolved activities, and stores telemetry across reopen', () => {
        ...
        first.appendRuntimeActivity('request-1', session.id, 'jbarton', {
            kind: 'tool',
            state: 'started',
