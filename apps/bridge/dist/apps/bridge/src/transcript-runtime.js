...
function splitAssistantMarkdownBlocks(content) {
...
function isLikelyRuntimeBlobParagraph(paragraph) {
...
function isLikelyRuntimeErrorLine(line) {
    const trimmed = line.trim();
    if (!trimmed) {
        return false;
    }
    return (/^(?:FileNotFoundError|ModuleNotFoundError|PermissionError|IsADirectoryError|NotADirectoryError|SyntaxError|TypeError|ValueError):/u.test(trimmed) ||
...
function isLikelyScriptFence(language, body) {
    ...
    return (/(?:^|[\s(])(?:python|bash|node|ruby)\s+(?:\.\/)?scripts\//iu.test(body) ||
...
function isTechnicalRuntimeLine(line) {
    const trimmed = line.trim();
    if (!trimmed) {
        return false;
    }
    return (/^(?:raw\s+)?(?:system|developer)(?:\s+(?:message|prompt))?:/iu.test(trimmed) ||
...
function extractPromptMarkerIndex(content) {
...
function stripTextRanges(content, ranges) {
    if (ranges.length === 0) {
        return content.trim();
    }
...
function findRecipeFenceEnd(content, searchStart, maxEnd) {
...
export function stripRecipeOperationsFence(content) {
    ...
    for (let index = 0; index < openingMatches.length; index += 1) {
        ...
        const parsedFenceEnd = extractedJson && extractedJson.endIndex > extractedJson.startIndex
...
export function isTechnicalRuntimeContent(content) {
    ...
    return (/^(?:AUTH_[A-Z_]+:|NOT_AUTHENTICATED:|REFRESH_FAILED:|AUTHENTICATED:|PROFILE_EXISTS\b|PYTHON_OK\b|SETUP_OK\b|session_id:|Session:|Query:|Duration:|Messages:|Bridge execution note:|Bridge note:|The active local Recipe for this request is:|Recipe ID:|View type:|Status:|Description:|Metadata:|Data snapshot:)/u.test(trimmed) ||
        ...
        /^Do not include (?:raw tool output|CLI startup banners|CLI diagnostics|tool inventories|JSON blobs|scripts|approvals|turn-limit traces)/u.test(trimmed) ||
...
function sanitizeAssistantBlock(block, options = {}) {
    ...
    if (fencedMatch?.[1]) {
        ...
        if (isLikelyScriptFence(fenceLanguage, fencedBody) ||
...
export function sanitizeAssistantBody(content, options = {}) {
...
export function sanitizeImportedMessageContent(role, content) {
    if (role === 'user') {
...
export function classifyImportedMessageVisibility(role, content) {
    if (role === 'tool') {
        return 'runtime';
    }
...
export function classifyImportedMessageKind(role, content, visibility) {
...
export function normalizeMessageForPersistence(message, previousRequestId = null) {
    ...
    const nextContent = message.role === 'user' && userWasTechnical
        ? trimmedContent
        : message.role === 'assistant' &&
    ...
    return {
        ...
        changed: requestId !== message.requestId ||
            nextContent !== message.content ||
            visibility !== message.visibility ||
            kind !== message.kind
    };
}
export function normalizePersistedSessionMessages(messages) {
...
export function buildRuntimeActivityFromMessage(message) {
    ...
    if (message.role === 'tool') {
        ...
        const label = normalizeActivityDetail(payload?.tool) ??
