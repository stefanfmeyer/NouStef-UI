...
function resolveBoundaryPath(targetPath: string) {
...
function normalizeRequest(request: ToolExecutionPrepareRequest, recipeRoot: string) {
...
export async function runReviewedToolExecution(
  request: ToolExecutionPrepareRequest,
  recipeRoot: string,
  requestedAt: string
): Promise<Pick<ToolExecution, 'status' | 'resolvedAt' | 'stdout' | 'stderr' | 'exitCode'>> {
