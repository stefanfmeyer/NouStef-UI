import type {
  ...
  RecipeActionSpec,
  RecipeAnalysis,
  RecipeAnalysisDataset,
  RecipeAnalysisField,
  RecipeAnalysisRecord,
...
export interface DynamicRecipeIntentHint {
  category: RecipeIntent['category'];
  preferredContentFormat?: RecipeContentFormat;
  label?: string;
}

export interface DynamicRecipeArtifactBuildInput {
  prompt: string;
  requestMode: 'chat' | 'recipe_refresh' | 'recipe_action';
  timestamp: string;
  currentRecipe: Pick<Recipe, 'id' | 'title' | 'description' | 'status' | 'metadata' | 'dynamic'> | null;
  envelope: HermesRecipeDataEnvelope;
  intentHint?: DynamicRecipeIntentHint | null;
}

export interface DynamicRecipeArtifacts {
  userPrompt: RecipeUserPromptArtifact;
  intent: RecipeIntent;
  ...
  fallback: RecipeFallbackState;
  uiSpec: RecipeUiSpec;
  actionSpec: RecipeActionSpec;
  recipeModel: RecipeModel;
  recipePatch: RecipePatch;
  testSpec: RecipeTestSpec;
}

export interface RecipeAppletBaseArtifacts {
  userPrompt: RecipeUserPromptArtifact;
  intent: RecipeIntent;
  rawData: RecipeRawData;
  assistantContext: RecipeAssistantContext;
...
export interface DeriveNormalizedDataOptions {
  datasetLabel: string;
  preferredPresentation: 'table' | 'cards' | 'list' | 'detail' | 'wizard' | 'markdown';
  timestamp: string;
}

type DatasetBuildContext = {
  datasets: RecipeAnalysisDataset[];
};
...
function sanitizeDatasetId(value: string) {
...
function createDatasetId(path: string, context: DatasetBuildContext) {
...
function toCellValue(key: string, value: unknown): RecipeCellValue | null {
...
function getCellText(value: RecipeCellValue) {
  if (value === null) {
    return '';
  }
...
function extractLinksFromValues(values: Record<string, RecipeCellValue>) {
        ...
        return [
          {
            ...
            url: value.href,
            kind: value.kind === 'email' ? ('email' as const) : ('website' as const)
          }
        ];
      }
...
function flattenScalarEntries(value: unknown, prefix = '', values: Record<string, RecipeCellValue> = {}) {
  if (!isRecord(value)) {
    return values;
  }
...
function pickPrimaryTextKey(values: Record<string, RecipeCellValue>) {
...
function pickSubtitleKey(values: Record<string, RecipeCellValue>, excluded: string[]) {
...
function pickDescriptionKey(values: Record<string, RecipeCellValue>, excluded: string[]) {
...
function inferFieldValueKind(key: string, values: Array<RecipeCellValue | undefined>): RecipeAnalysisField['valueKind'] {
  const nonEmptyValues = values.filter((value): value is RecipeCellValue => value !== undefined);
...
function inferFieldRoles(key: string, values: Array<RecipeCellValue | undefined>, totalCount: number) {
  const roles: RecipeAnalysisField['roles'] = [];
...
function recordValuesToAnalysisRecord(rawRecord: Record<string, unknown>, index: number, parentMetadata?: { parentId?: string; parentTitle?: string }) {
  ...
  return {
    ...
    metadata: {}
  } satisfies RecipeAnalysisRecord;
}

function buildAnalysisFields(records: RecipeAnalysisRecord[], path: string) {
  const fieldKeys = Array.from(
    new Set(records.flatMap((record) => Object.keys(record.values))).values()
  );
  ...
  return fieldKeys.map((key) => {
    ...
    } satisfies RecipeAnalysisField;
  });
}

function collectNestedBuckets(records: RecipeAnalysisRecord[], rawRecords: RecordShape[]) {
  const buckets = new Map<string, { value: unknown; parentId: string; parentTitle: string }[]>();
...
function buildCollectionDataset(
  rawRecords: RecordShape[],
  label: string,
  path: string,
  timestamp: string,
  ...
  const defaultView =
    itemCount === 1 && detailFieldKeys.length > 0
      ? 'property_sheet'
  ...
  const dataset: RecipeAnalysisDataset = {
    id: datasetId,
    label,
    path,
    kind:
...
function buildEntityDataset(
  rawRecord: RecordShape,
  label: string,
  path: string,
  timestamp: string,
  context: DatasetBuildContext,
...
function analyzeValue(
  ...
  if (Array.isArray(value)) {
    if (value.every((item) => isRecord(item))) {
      return buildCollectionDataset(value as RecordShape[], label, path, timestamp, context, parentDatasetId, parentFieldKey);
    }

    const normalizedPrimitiveRecords = value.map((item) =>
      isRecord(item)
        ? item
        : {
            value: item
...
function buildSummaryStats(datasets: RecipeAnalysisDataset[]) {
  return datasets
    ...
    .map((dataset, index) => ({
      ...
      tone: 'info' as const
    })) satisfies RecipeSummaryStat[];
}

function mapContentFormatToPresentation(value: RecipeContentFormat | undefined): RecipeIntent['preferredPresentation'] | undefined {
  if (!value) {
    return undefined;
  }
...
function resolvePrimaryPresentationHint(input: DynamicRecipeArtifactBuildInput) {
...
export function analyzeRecipeData(
  rawData: RecipeRawData,
  assistantContext: RecipeAssistantContext,
  options: {
    datasetLabel: string;
    timestamp: string;
    semanticHints?: HermesRecipeDataEnvelope['semanticHints'];
  }
) {
  ...
  const primaryDatasetId =
    options.semanticHints?.primaryDatasetPath
...
function toLegacyPresentation(defaultView: RecipeAnalysisDataset['defaultView']): RecipeNormalizedData['datasets'][number]['preferredPresentation'] {
  switch (defaultView) {
    case 'property_sheet':
      return 'detail';
    case 'timeline':
...
function toNormalizedField(field: RecipeAnalysisField, record: RecipeAnalysisRecord | undefined): RecipeNormalizedField {
...
function toNormalizedItem(record: RecipeAnalysisRecord, fields: RecipeAnalysisField[]): RecipeNormalizedItem {
  return {
    id: record.id,
    title: record.title,
...
export function deriveNormalizedDataFromAnalysis(
  analysis: RecipeAnalysis,
  options: {
    primaryDatasetPreferredPresentation?: RecipeIntent['preferredPresentation'];
  } = {}
): RecipeNormalizedData {
  const datasets = analysis.datasets.map((dataset) => {
    ...
    const preferredPresentation =
      dataset.id === analysis.primaryDatasetId && options.primaryDatasetPreferredPresentation
    ...
    return {
      id: dataset.id,
      label: dataset.label,
      kind: dataset.kind === 'entity' ? ('detail' as const) : ('collection' as const),
...
export function deriveNormalizedDataFromRawData(rawData: RecipeRawData, options: DeriveNormalizedDataOptions): RecipeNormalizedData {
  const analysis = analyzeRecipeData(
    rawData,
    {
...
function buildFallbackSummaryMarkdown(title: string, assistantContext: RecipeAssistantContext, normalizedData: RecipeNormalizedData) {
...
function extractPromptEntities(prompt: string) {
  const entities: RecipeIntent['entities'] = [];
...
function extractPromptFilters(prompt: string) {
  const filters: RecipeIntent['filters'] = [];
...
function resolveIntentPresentation(analysis: RecipeAnalysis, normalizedData: RecipeNormalizedData, contentFormat: RecipeContentFormat | undefined) {
...
function buildRecipeIntent(input: DynamicRecipeArtifactBuildInput, analysis: RecipeAnalysis, normalizedData: RecipeNormalizedData): RecipeIntent {
  ...
  return RecipeIntentSchema.parse({
    ...
    allowOutboundRequests:
      input.envelope.intentHints?.allowOutboundRequests ??
...
function buildFallback(summary: RecipeSummary, assistantContext: RecipeAssistantContext, normalizedData: RecipeNormalizedData): RecipeFallbackState {
  const primaryDataset = normalizedData.datasets.find((dataset) => dataset.id === normalizedData.primaryDatasetId) ?? normalizedData.datasets[0];
...
function buildActionSpec(
  ...
  const primaryDataset = normalizedData.datasets.find((dataset) => dataset.id === normalizedData.primaryDatasetId) ?? normalizedData.datasets[0];
  const actions: RecipeActionDefinition[] = [];
...
function datasetActionIds(actionSpec: RecipeActionSpec) {
  return {
...
function buildDatasetMarkdown(dataset: RecipeAnalysisDataset) {
...
function buildDatasetNodes(
  dataset: RecipeAnalysisDataset,
  actionSpec: RecipeActionSpec,
  options: {
    preferredPresentationHint?: RecipeIntent['preferredPresentation'];
  ...
  const nodes: RecipeUiNode[] = [];
  const preferredPresentationHint =
    dataset.id === options.primaryDatasetId ? options.preferredPresentationHint : undefined;
  ...
  if (dataset.defaultView === 'property_sheet' || dataset.kind === 'entity') {
    const propertyNode: RecipeUiPropertySheetNode = {
  ...
  if (dataset.defaultView === 'timeline' && dataset.timeFieldKey) {
    const timelineNode: RecipeUiTimelineNode = {
  ...
  const collectionNode: RecipeUiCollectionNode = {
    id: `${dataset.id}-collection`,
    kind: 'collection',
    title: dataset.label,
    datasetId: dataset.id,
    display:
      preferredPresentationHint === 'table'
        ? 'table'
  ...
  nodes.push({
    ...
    loadingState: sharedLoadingState,
    errorState: sharedErrorState
  } satisfies RecipeUiDetailSheetNode);
  if (dataset.itemCount > dataset.pageSize) {
    nodes.push({
      id: `${dataset.id}-pager`,
      kind: 'paginator',
...
function buildUiSpec(
  ...
  const overviewNodes: RecipeUiNode[] = [];
...
function buildTestSpec(uiSpec: RecipeUiSpecV2, actionSpec: RecipeActionSpec): RecipeTestSpec {
  const cases: RecipeTestCase[] = [
    {
      id: 'schema-valid',
      kind: 'schema_valid',
      severity: 'critical',
  ...
  const visitNodes = (nodes: RecipeUiNode[]) => {
...
function validateLinkUrl(value: string, isEmailLink: boolean) {
  const trimmed = value.trim();
  if (!trimmed) {
    return false;
  }
...
function collectNodeActionIds(nodes: RecipeUiNode[], actionIds = new Set<string>()) {
  for (const node of nodes) {
    if (
      node.kind === 'collection' ||
      node.kind === 'grouped_collection' ||
...
function findNodeById(nodes: RecipeUiNode[], nodeId: string): RecipeUiNode | null {
  for (const node of nodes) {
    if (node.id === nodeId) {
      return node;
    }
...
function collectNodeIds(nodes: RecipeUiNode[], ids = new Set<string>()) {
  for (const node of nodes) {
    ids.add(node.id);
    if (node.kind === 'section_group') {
...
function collectAllNodes(nodes: RecipeUiNode[], output: RecipeUiNode[] = []) {
  for (const node of nodes) {
    output.push(node);
    if (node.kind === 'section_group') {
            ...
            : new Set([
                ...
                ...input.uiSpec.sections.flatMap((section) => {
                  switch (section.kind) {
                    ...
                    case 'form':
                      return [section.submitActionId, section.cancelActionId].filter((value): value is string => Boolean(value));
                    default:
                      return [];
                  }
                })
...
export function buildRecipeAppletBaseArtifacts(input: DynamicRecipeArtifactBuildInput): RecipeAppletBaseArtifacts {
  ...
  const summary = buildSummary(
    input.envelope.recipe.title || input.currentRecipe?.title || 'Recipe',
    input.envelope.recipe.subtitle || input.envelope.recipe.description || input.currentRecipe?.description,
    analysis,
    assistantContext,
    input.timestamp,
    intent.category
...
export function buildDynamicRecipeArtifacts(input: DynamicRecipeArtifactBuildInput): DynamicRecipeArtifacts {
