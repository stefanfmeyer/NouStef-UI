...
/* eslint-disable @typescript-eslint/no-explicit-any */import type {
  LegacyRecipeDsl,
  RecipeImage,
  RecipeLink,
  RecipeSummaryStat,
...
export interface NormalizeRecipeDslInput {
  baseModel: RecipeModel;
  dsl: RecipeDsl | unknown;
  assistantSummary?: string | null;
}

export interface RecipeDslRepairSummary {
  droppedKeys: string[];
...
export interface NormalizeRecipeDslResult {
  dsl: RecipeAuthoringDsl | null;
  model: RecipeModel | null;
  patch: RecipePatch | null;
  warnings: string[];
  errors: string[];
  repairs: RecipeDslRepairSummary;
}

interface RecipeDslStateEffects {
  activeTabId: string | null;
  collectionSelection: Record<string, string[]>;
  actionState: RecipeModel['state']['actionState'];
  statusMessage: string | null;
}

function createRepairSummary(): RecipeDslRepairSummary {
  return {
    droppedKeys: [],
    aliasMappings: [],
    defaultedFields: [],
...
function registerAlias(repairs: RecipeDslRepairSummary, from: string, to: string) {
  repairs.aliasMappings.push(`${from} -> ${to}`);
}

function registerSynonym(repairs: RecipeDslRepairSummary, from: string, to: string) {
  repairs.normalizedSynonyms.push(`${from} -> ${to}`);
}

function registerDefault(repairs: RecipeDslRepairSummary, field: string) {
  repairs.defaultedFields.push(field);
}

function registerDroppedKey(repairs: RecipeDslRepairSummary, path: string) {
  repairs.droppedKeys.push(path);
}

function mapDatasetDisplay(value: string | undefined): RecipeDslDataset['display'] | undefined {
  if (!value) {
    return undefined;
  }
...
function mapSectionType(
  ...
  type: RecipeDslSection['type'] | null;
  displayHint?: RecipeDslDataset['display'];
  variant?: 'empty_state' | 'error_state' | 'loading_state' | 'skeleton';
} {
  if (!value) {
    return {
      type: null
...
function mapCollectionPreferredView(display: RecipeDslDataset['display'] | undefined): RecipeCollectionPreferredView {
  switch (display) {
    case 'list':
      return 'list';
    case 'cards':
...
function mapSectionKind(
  section: RecipeDslSection,
  datasetDisplay: RecipeDslDataset['display'] | undefined
): RecipeSection['kind'] {
  switch (section.type) {
    case 'dataset':
...
function formatLinksMarkdown(links: RecipeLink[]) {
  if (links.length === 0) {
    return null;
  }
...
function formatNotesMarkdown(notes: string[]) {
  if (notes.length === 0) {
    return null;
  }
...
function coerceLinks(value: unknown) {
  if (!Array.isArray(value)) {
    return [] satisfies RecipeLink[];
  }
...
function coerceMedia(value: unknown) {
...
function coerceStats(value: unknown) {
  if (!Array.isArray(value)) {
    return [] satisfies RecipeSummaryStat[];
  }

  const stats: RecipeSummaryStat[] = [];
  value.forEach((item, index) => {
    ...
    stats.push({
      ...
      value: statValue,
      emphasis:
        item.emphasis === 'primary' || item.emphasis === 'status' || item.accent === 'primary'
...
function coerceActionReferences(value: unknown, repairs: RecipeDslRepairSummary, path: string) {
  if (!Array.isArray(value)) {
    return [] satisfies RecipeDslActionReference[];
  }

  const actions: RecipeDslActionReference[] = [];
  value.forEach((item, index) => {
...
function coerceDatasets(
  ...
  const datasets: RecipeDslDataset[] = [];
  value.forEach((item, index) => {
    if (!isRecord(item)) {
      return;
    }
    ...
    const display = mapDatasetDisplay(
      asString(item.display ?? item.preferredView ?? item.preferredPresentation ?? item.view)
    );
...
function inferSectionType(item: Record<string, unknown>, index: number, repairs: RecipeDslRepairSummary) {
...
function coerceSections(
  ...
  const sections: RecipeDslSection[] = [];
  source.forEach((item, index) => {
    ...
    const entityIds = uniqueStrings(
      asStringArray(item.entityIds).concat(asString(item.entityId) ? [asString(item.entityId)!] : [])
    );
    const section: RecipeDslSection = {
      id,
      ...
      actionIds: uniqueStrings(
        asStringArray(item.actionIds).concat(
          coerceActionReferences(item.actions, repairs, `sections[${index}].actions`).map((action) => action.id)
        )
      ),
...
function coerceTabs(
  ...
  const viewById = new Map(
    views
      ...
      .filter((entry): entry is readonly [string, { sectionIds: string[]; layout: string | undefined }] => Boolean(entry))
  );
  ...
  source.forEach((item, index) => {
    ...
    const sectionIds = uniqueStrings(
      asStringArray(item.sectionIds).concat(view?.sectionIds ?? fallbackSectionIds)
    );
    ...
    tabs.push({
      ...
      sectionIds,
      layout:
        asString(item.layout ?? view?.layout) === 'grid' ||
        asString(item.layout ?? view?.layout) === 'split' ||
        asString(item.layout ?? view?.layout) === 'board' ||
        asString(item.layout ?? view?.layout) === 'tabbed'
          ? (asString(item.layout ?? view?.layout) as string)
          : 'stack',
      metadata: toRecord(item.metadata)
    });
  });

  return tabs as unknown as RecipeDslTab[];
}

function coerceSemanticHints(value: unknown, repairs: RecipeDslRepairSummary) {
  ...
  return {
    ...
    preferredLayout:
      ...
      raw.preferredLayout === 'tabbed'
        ...
        : undefined,
    narrowPaneStrategy:
      raw.narrowPaneStrategy === 'tabs' || raw.narrowPaneStrategy === 'split' ? raw.narrowPaneStrategy : 'stack',
    notes: uniqueStrings(asStringArray(raw.notes))
  } satisfies RecipeAuthoringDsl['semanticHints'];
}

function coerceOperations(
  value: unknown,
  repairs: RecipeDslRepairSummary
): RecipeDslOperation[] {
  if (!Array.isArray(value)) {
  ...
  const operations: RecipeDslOperation[] = [];
  value.forEach((item, index) => {
    if (!isRecord(item)) {
      return;
    }
...
function convertLegacyRecipeDsl(raw: LegacyRecipeDsl, repairs: RecipeDslRepairSummary): RecipeAuthoringDsl {
  const viewById = new Map(raw.views.map((view) => [view.id, view] as const));
  const sections = raw.sections.map((section) => {
    ...
    } satisfies RecipeDslSection;
  });

  const tabs: any[] = raw.tabs
    .map((tab) => {
      const view = viewById.get((tab as any).viewId);
      return {
        id: tab.id,
        label: tab.label,
        ...
        metadata: {
          ...tab.metadata,
          legacyViewId: (tab as any).viewId
        }
      };
    })
  ...
  for (const operation of raw.operations) {
    switch (operation.op) {
      ...
      case 'update_tab':
      case 'create_tab': {
        const view = viewById.get((operation.tab as any).viewId);
        operations.push({
          op: 'update_tab',
          tab: {
            id: operation.tab.id,
  ...
  return {
    ...
    semanticHints: {
      ...
      narrowPaneStrategy:
        raw.semanticHints.narrowPaneStrategy === 'tabs' ? 'tabs' : raw.semanticHints.narrowPaneStrategy,
      notes: raw.semanticHints.notes
    },
...
function buildCanonicalRecipeDsl(
  input: NormalizeRecipeDslInput,
  repairs: RecipeDslRepairSummary,
  warnings: string[],
  errors: string[]
): RecipeAuthoringDsl | null {
  ...
  const hasMeaningfulAuthoringSignal =
    Array.isArray(input.dsl.tabs) ||
  ...
  const sectionsSource =
    Array.isArray(input.dsl.sections) && input.dsl.sections.length > 0
      ? input.dsl.sections
  ...
  const draft: RecipeAuthoringDsl = {
    kind: 'recipe_dsl',
    schemaVersion: 'recipe_dsl/v2',
    ...
    summary,
    status:
      asString(input.dsl.status) === 'idle' ||
      ...
      asString(input.dsl.status) === 'error'
        ? (asString(input.dsl.status) as RecipeAuthoringDsl['status'])
        : asString(recipe.status) === 'idle' ||
          ...
          ? (asString(recipe.status) as RecipeAuthoringDsl['status'])
          : undefined,
    tabs,
    datasets,
    sections,
...
function createDefaultSections(
  ...
  const candidateDatasetIds = uniqueStrings(
    dsl.datasets.map((dataset) => dataset.id).concat(
      dsl.semanticHints.primaryDatasetId ? [dsl.semanticHints.primaryDatasetId] : [],
      baseModel.collections[0]?.id ? [baseModel.collections[0].id] : []
    )
  );
...
function createDefaultTabs(sections: RecipeDslSection[], repairs: RecipeDslRepairSummary) {
...
function applyAuthoringOperations(
  dsl: RecipeAuthoringDsl
): { dsl: RecipeAuthoringDsl; state: RecipeDslStateEffects; warnings: string[] } {
  const warnings: string[] = [];
  const nextDsl = structuredClone(dsl);
  const datasetById = new Map(nextDsl.datasets.map((dataset) => [dataset.id, dataset] as const));
  const sectionById = new Map(nextDsl.sections.map((section) => [section.id, section] as const));
  const tabById = new Map(nextDsl.tabs.map((tab) => [tab.id, tab] as const));
  const state: RecipeDslStateEffects = {
    activeTabId: null,
    collectionSelection: {},
    actionState: {},
    statusMessage: null
...
function filterCanonicalDslReferences(
  dsl: RecipeAuthoringDsl,
  baseModel: RecipeModel,
  repairs: RecipeDslRepairSummary,
  warnings: string[]
  ...
  const datasets = dsl.datasets
    ...
    .map((dataset) => {
      ...
      const validFields = uniqueStrings(
        dataset.fields.filter((field) => collection?.fieldKeys.includes(field) ?? true)
      );
  ...
  return {
    ...
    semanticHints: {
      ...dsl.semanticHints,
      primaryDatasetId:
        dsl.semanticHints.primaryDatasetId && datasetIds.has(dsl.semanticHints.primaryDatasetId)
          ? dsl.semanticHints.primaryDatasetId
          : datasets[0]?.id ?? baseModel.collections[0]?.id
    }
  } satisfies RecipeAuthoringDsl;
}

function buildCollectionMarkdownNote(collection: RecipeCollection, dataset: RecipeDslDataset | undefined) {
  return dataset?.notes[0] ?? (typeof collection.metadata.summary === 'string' ? collection.metadata.summary : undefined);
}

function buildRecipeSectionBody(section: RecipeDslSection) {
  if (section.body) {
    return section.body;
  }
  ...
  if (section.type === 'notes') {
    ...
    const notes = uniqueStrings([section.summary, ...metadataNotes].filter((note): note is string => Boolean(note)));
...
function materializeRecipeModel(
  dsl: RecipeAuthoringDsl,
  baseModel: RecipeModel,
  stateEffects: RecipeDslStateEffects
) {
  const nextModel = structuredClone(baseModel);
  const collectionById = new Map(nextModel.collections.map((collection) => [collection.id, collection] as const));
...
export function normalizeRecipeDsl(input: NormalizeRecipeDslInput): NormalizeRecipeDslResult {
  const warnings: string[] = [];
  const errors: string[] = [];
