...
function capabilityAssertions(capabilities: RecipeAppletCapability[]) {
  return capabilities
    .map((capability) => {
...
function actionAssertions(actionIds: string[]) {
  return actionIds
    .map((actionId) => {
...
export function buildRecipeAppletGeneratedTests(manifest: RecipeAppletManifest): RecipeAppletTestSourceArtifact {
  const dynamicCases: string[] = [
    `  {
    name: 'renders at least one node',
    run({ renderTree }) {
      assert(Array.isArray(renderTree.root) && renderTree.root.length > 0, 'Recipe applet must render at least one node.');
    }
  }`
  ];
  ...
  return {
    ...
    source: `import { assert, defineAppletTests, expectActionDeclared, expectCapability, expectNodeKind } from 'recipe-applet-test-sdk';
