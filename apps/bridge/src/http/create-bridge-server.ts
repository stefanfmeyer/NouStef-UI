  ...
  const server = http.createServer(async (request, response) => {
    ...
    try {
      ...
      if (request.method === 'PUT' && pathname === '/api/soul-md') {
        const { profileId, content } = await readJsonBody<{ profileId?: string; content?: string }>(request);
        if (!profileId) throw new BridgeError(400, 'PROFILE_REQUIRED', 'profileId is required.');
        if (content === undefined) throw new BridgeError(400, 'CONTENT_REQUIRED', 'content is required.');
