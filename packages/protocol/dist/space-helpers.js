import { SpaceCardDataSchema, SpaceCellValueSchema, SpaceContentEntrySchema, SpaceContentEntrySourceSchema, SpaceContentModelSchema, SpaceExtensionTabSchema, SpaceMarkdownDataSchema, SpaceRichTextValueSchema, SpaceTableDataSchema, SpaceTabSchema, SpaceUiStateSchema } from './schemas';
...
function parseSpaceContentFormat(value) {
    return value === 'markdown' || value === 'table' || value === 'card' ? value : null;
}
function truncateSpaceText(value, maxLength) {
...
function normalizeLink(input) {
    ...
    const label = truncateSpaceText(stripMarkdownFormatting((input.label ?? '').trim()) || displayLabelForUrl(url), 120);
...
function normalizeImage(input) {
    ...
    const alt = typeof input.alt === 'string' && input.alt.trim().length > 0 ? truncateSpaceText(input.alt, 160) : undefined;
    return {
        url,
        alt
    };
...
function normalizeRichTextValue(value) {
    ...
    const text = typeof value.text === 'string' && value.text.trim().length > 0
        ? truncateSpaceText(stripMarkdownFormatting(value.text), 200)
        : image?.alt ?? (link ? displayLabelForUrl(link.url) : undefined);
    return SpaceRichTextValueSchema.parse({
        text,
        href: link?.url,
...
function normalizeCellValue(value) {
    ...
    if (typeof value === 'string') {
        ...
        return truncateSpaceText(stripMarkdownFormatting(trimmed), 400);
    }
    if (isRecord(value)) {
        const parsedCell = SpaceCellValueSchema.safeParse(value);
    ...
    return truncateSpaceText(String(value), 400);
}
...
function cloneMarkdownRepresentation(data) {
    return SpaceMarkdownDataSchema.parse({
        markdown: data.markdown
    });
}
function normalizeTableRepresentation(data) {
    const parsed = SpaceTableDataSchema.safeParse(data);
    ...
    return SpaceTableDataSchema.parse({
...
function normalizeCardRepresentation(data) {
    const parsed = SpaceCardDataSchema.safeParse(data);
    const source = parsed.success ? parsed.data : isRecord(data) ? data : {};
    const cards = (Array.isArray(source.cards) ? source.cards : []).map((card, index) => {
        ...
        const metadata = Array.isArray(record.metadata)
            ? record.metadata
                ...
                return {
                    label: truncateSpaceText(stripMarkdownFormatting(item.label), 80),
                    value: truncateSpaceText(stripMarkdownFormatting(item.value), 240),
                    ...(link ? { link } : {})
                };
            })
        ...
        return {
            ...
            title: truncateSpaceText(stripMarkdownFormatting(typeof record.title === 'string' ? record.title : `Card ${index + 1}`), 120),
            description: typeof record.description === 'string' && record.description.trim().length > 0
                ? truncateSpaceText(stripMarkdownFormatting(record.description), 320)
                : undefined,
            eyebrow: typeof record.eyebrow === 'string' && record.eyebrow.trim().length > 0
                ? truncateSpaceText(stripMarkdownFormatting(record.eyebrow), 80)
                : undefined,
            badges: Array.isArray(record.badges)
                ? record.badges
                    .map((badge) => (typeof badge === 'string' ? truncateSpaceText(stripMarkdownFormatting(badge), 60) : ''))
                    .filter(Boolean)
                : [],
            metadata,
            links,
    ...
    return SpaceCardDataSchema.parse({
        cards,
...
function cloneEntrySource(source) {
    ...
    return SpaceContentEntrySourceSchema.parse({
        ...source,
        metadata: isRecord(source.metadata) ? { ...source.metadata } : {}
    });
}
function cloneEntry(entry) {
    return SpaceContentEntrySchema.parse({
        id: entry.id,
        md: entry.md,
...
function entryTitleFromMarkdown(markdown, index) {
    ...
    return truncateSpaceText(title || `Item ${index + 1}`, 120);
}
function createFallbackCard(id, markdown, row, index) {
    ...
    const title = entryTitleFromMarkdown(markdown, index) || truncateSpaceText(firstValue || `Item ${index + 1}`, 120);
    return normalizeCardRepresentation({
        cards: [
            {
                id,
                title,
                description: truncateSpaceText(stripMarkdownFormatting(markdown
                    .replace(/\r\n/gu, '\n')
                    .split('\n')
                    .slice(1)
                    .join(' ')), 240) || undefined,
...
function normalizeEntrySource(rawSource) {
    const parsed = SpaceContentEntrySourceSchema.safeParse(rawSource);
...
function getEntrySectionHeading(entry) {
    if ('sectionHeading' in entry && typeof entry.sectionHeading === 'string' && entry.sectionHeading.trim().length > 0) {
        return truncateSpaceText(entry.sectionHeading.trim(), 120);
    }
    ...
    return rawValue ? truncateSpaceText(rawValue, 120) : undefined;
}
...
function normalizeEntryCandidate(candidate, index) {
    ...
    if (!card) {
        if (!md && Object.keys(row).length === 0) {
            throw new Error('Space content entries must include at least one of md, row, or card.');
        }
...
function convertMarkdownToEntries(markdownData, currentEntries = []) {
    ...
    for (const block of blocks) {
        ...
        if (headingOnlyMatch && (headingOnlyMatch[1]?.length ?? 0) <= 2) {
            pendingSectionHeading = truncateSpaceText(stripMarkdownFormatting(headingOnlyMatch[2] ?? ''), 120) || undefined;
            continue;
        }
...
function parseStructuredField(label, rawValue) {
    ...
    return {
        label: truncateSpaceText(stripMarkdownFormatting(label), 80),
...
function extractStructuredMarkdownEntriesFromBlock(block, pendingSectionHeading) {
    ...
    if (headingMatch) {
        ...
        const normalizedHeading = truncateSpaceText(stripMarkdownFormatting(headingMatch[2] ?? ''), 120);
        ...
        if (fieldMatch) {
            ...
            if (/^(tags?|badges?)$/iu.test(label)) {
                badges.push(...rawValue
                    .split(/[,|/]/u)
                    .map((value) => truncateSpaceText(stripMarkdownFormatting(value), 40))
                    .filter(Boolean));
                continue;
            }
            if (/^(section|category|area|neighborhood)$/iu.test(label)) {
                eyebrow = truncateSpaceText(stripMarkdownFormatting(rawValue), 80);
                continue;
            }
...
function convertMarkdownToCard(markdownData) {
    ...
    if (entries.length > 0) {
        return SpaceCardDataSchema.parse({
            cards: entries.map((entry, index) => ({
                id: `card-${index + 1}`,
                title: truncateSpaceText(entry.title || `Card ${index + 1}`, 80),
                description: entry.description ? truncateSpaceText(entry.description, 240) : undefined,
                eyebrow: entry.eyebrow,
                badges: entry.badges,
                metadata: entry.fields
                    .map((field) => ({
                    label: field.label,
                    value: truncateSpaceText(plainTextCellValue(field.value), 240),
    ...
    const cards = splitMarkdownBlocks(markdownData.markdown)
        ...
        const title = truncateSpaceText(rawTitle, 80) || fallbackTitle;
        return {
            id: `card-${index + 1}`,
            title,
            description: truncateSpaceText(stripMarkdownFormatting(chunk.split('\n').slice(1).join(' ')), 240) || undefined,
            badges: [],
            metadata: [],
...
function parseRepresentationData(format, rawData, fallbackContent) {
    ...
    if (fallbackContent) {
        return getSpaceContentViewDataFromContent(fallbackContent, format);
    }
    return getDefaultRepresentation(format);
}
function getSpaceContentViewDataFromContent(content, format) {
    if (format === 'table') {
...
function recoverContentModel(rawContent, fallbackContent) {
    if (isRecord(rawContent)) {
        if ('format' in rawContent) {
            const legacyFormat = parseSpaceContentFormat(rawContent.format) ?? fallbackContent.activeView;
            return synchronizeSpaceContentModel({
        ...
        if (rawContent.activeView !== undefined ||
            ...
            const sourceView = parseSpaceContentFormat(rawContent.sync?.sourceView) ??
                parseSpaceContentFormat(rawContent.activeView) ??
                fallbackContent.activeView;
            return synchronizeSpaceContentModel({
                currentContent: fallbackContent,
                activeView: parseSpaceContentFormat(rawContent.activeView) ?? fallbackContent.activeView,
                sourceView,
                content: rawContent
            });
        }
    }
    const parsedContent = SpaceContentModelSchema.safeParse(rawContent);
...
function recoverContentTab(rawTab, fallbackTab) {
    const parsedTab = SpaceTabSchema.safeParse(rawTab);
...
function recoverExtensionTab(rawTab) {
    const parsedTab = SpaceExtensionTabSchema.safeParse(rawTab);
    ...
    return {
        id,
        kind,
        label: typeof rawTab.label === 'string' && rawTab.label.trim().length > 0 ? rawTab.label : truncateSpaceText(kind, 32),
...
export function getSpaceContentIntegrityFingerprint(content) {
    return stableStringify({
...
export function synchronizeSpaceContentModel(options = {}) {
    const currentContent = options.currentContent ? SpaceContentModelSchema.parse(options.currentContent) : null;
    if (options.contentData === undefined && options.content === undefined) {
        if (currentContent) {
            return SpaceContentModelSchema.parse({
                ...currentContent,
        ...
        const fingerprint = getSpaceContentIntegrityFingerprint(representations);
        return SpaceContentModelSchema.parse({
    ...
    if (providedContent !== null) {
        const parsedProvidedContent = SpaceContentModelSchema.safeParse(providedContent);
        if (parsedProvidedContent.success) {
            ...
            const representations = synchronizedEntries !== null
                ...
                : buildSynchronizedRepresentations(sourceView, getSpaceContentViewDataFromContent(parsedContent, sourceView), {
                    currentContent: currentContent ?? parsedContent
                });
            const fingerprint = getSpaceContentIntegrityFingerprint(representations);
            const currentFingerprint = currentContent ? getSpaceContentIntegrityFingerprint(currentContent) : null;
        ...
        const normalizedContent = recoverContentModel(providedContent, currentContent ?? createDefaultSpaceContentModel(options.activeView));
...
export function isSpaceContentSynchronized(content) {
    const parsedContent = SpaceContentModelSchema.parse(content);
    const sourceView = parsedContent.sync.sourceView ?? parsedContent.activeView;
    const sourceData = getSpaceContentViewDataFromContent(parsedContent, sourceView);
    const synchronized = synchronizeSpaceContentModel({
        currentContent: parsedContent,
        activeView: parsedContent.activeView,
        sourceView,
        contentData: sourceData
    });
    return getSpaceContentIntegrityFingerprint(parsedContent) === getSpaceContentIntegrityFingerprint(synchronized);
}
export function isSpaceContentMeaningful(content) {
    const parsedContent = SpaceContentModelSchema.parse(content);
...
export function createDefaultSpaceContentModel(format = 'markdown') {
    return synchronizeSpaceContentModel({
        activeView: format
    });
}
export function createDefaultSpaceTabs(format = 'markdown') {
    return [
        {
            id: 'content',
            kind: 'content',
            label: 'Content',
            content: createDefaultSpaceContentModel(format),
            metadata: {}
        }
    ];
}
export function getSpaceContentTab(input) {
    return (getTabsArray(input).find((tab) => tab.kind === 'content') ?? createDefaultSpaceTabs()[0]);
}
export function getSpaceContentActiveView(input) {
    return getSpaceContentTab(input).content.activeView;
}
export function getSpaceContentFormat(input) {
    return getSpaceContentActiveView(input);
}
export function getSpaceContentEntries(input) {
    const contentTab = Array.isArray(input)
        ? getSpaceContentTab(input)
        : 'kind' in input
            ? input
            : getSpaceContentTab(input.tabs);
    return contentTab.content.entries.map((entry) => cloneEntry(entry));
}
export function replaceSpaceContentEntries(input, entries) {
    const currentTabs = getTabsArray(input);
    const currentContentTab = getSpaceContentTab(currentTabs);
    const nextContent = synchronizeSpaceContentModel({
...
export function removeSpaceContentEntries(input, entryIds) {
    const idsToRemove = new Set(entryIds.filter((value) => value.trim().length > 0));
    if (idsToRemove.size === 0) {
        return getTabsArray(input);
    }
    const remainingEntries = getSpaceContentEntries(input).filter((entry) => !idsToRemove.has(entry.id));
    return replaceSpaceContentEntries(input, remainingEntries);
}
export function getSpaceContentViewData(input, format) {
    const contentTab = Array.isArray(input)
        ? getSpaceContentTab(input)
        : 'kind' in input
            ? input
    ...
    return getSpaceContentViewDataFromContent(contentTab.content, view);
}
export function normalizeSpaceUiState(uiState) {
    const requestedActiveTab = typeof uiState?.activeTab === 'string' && uiState.activeTab.trim().length > 0 ? uiState.activeTab.trim() : 'content';
    return SpaceUiStateSchema.parse({
        activeTab: requestedActiveTab === 'todos' ? 'content' : requestedActiveTab
    });
}
export function normalizeSpaceTabs(options) {
    const currentContentTab = options.currentSpace ? getSpaceContentTab(options.currentSpace) : null;
    const currentExtensionTabs = getFutureSafeExtensionTabs(options.currentSpace);
    const defaultTabs = createDefaultSpaceTabs(options.contentFormat ?? currentContentTab?.content.activeView ?? 'markdown');
    if (options.tabs !== undefined) {
        ...
        const parsedTabs = SpaceTabSchema.array().safeParse(options.tabs);
        if (parsedTabs.success) {
            ...
            return [
                {
                    ...contentTab,
                    content: synchronizeSpaceContentModel({
...
export function normalizeLegacySpaceTabs(legacyViewType, legacyData) {
    if (legacyViewType === 'table') {
        return normalizeSpaceTabs({
            contentFormat: 'table',
